...
- Q: Installation resolution model (OQ-1) → A: **Upward directory walk** from the invocation context to the nearest enclosing stack-control config; the **presence of that config file marks the installation root** (git-style). This also settles OQ-5's root-marker: an installation root is the directory containing a stack-control config. Surface-agnostic per FR-026 — the "context" is a directory/root for any host (CLI cwd today, client root later).
- Q: Default location convention (OQ-2) → A: **Split by audience.** Human-facing docs (`ROADMAP.md`, `DESIGN-INBOX.md`) default to the **installation root**; tool-internal stores (backlog store, audit log, config) default under a per-installation **`.stack-control/`** dir (which doubles as the root marker from OQ-1). All locations remain per-file configurable (FR-018).
- Q: Configurable-location granularity (OQ-3) → A: **Both** — the config supports a **base directory** plus **per-file path overrides**. Resolution precedence: per-file override > base directory > audience-split default (OQ-2).
- Q: Audit-log partitioning + managed-set membership (OQ-4) → A: **Two-tier audit log.** Per-feature audit logs are the primary partition (`specs/<feature>/audit-log.md`, co-located with the spec, created by the **feature lifecycle / first govern — NOT installation setup**); per-feature partitioning is the size control (file boundary = feature boundary). One **program-level audit log** (scaffolded by installation setup, default under `.stack-control/`) holds findings not scoped to a single feature. **Installation setup's scaffold set** = config + roadmap + inbox + backlog store + program audit log; operation-products (run dirs, slush/burn-down beyond the backlog, scope-discovery registries) stay lazy. The set is extensible (Principle II) as capabilities migrate in.
- Q: Nested-installation precedence & collision rules (OQ-5) → A: **Nearest enclosing config wins**; a parent installation's scope is its subtree **minus any nested child installations' subtrees** (git/`tsconfig` nesting semantics). Setup **fails loud and refuses** any configured location that escapes its installation's scope or collides with another installation's file — no silent sharing (Principle V).
...
1. **Given** a project with no stack-control working files, **When** the operator runs setup, **Then** a governed roadmap, a design inbox, a backlog store, a program-level audit log, and a stack-control config all exist and are well-formed. (Per-feature audit logs are created later by the feature lifecycle, not by installation setup — FR-027.)
...
- **FR-001**: Setup MUST scaffold, into a stack-control installation, the installation-level governed working files the verbs require to operate without hand-authoring (Clarification 2026-06-09): a governed roadmap, a design inbox, a backlog store, a **program-level audit log**, and a stack-control config. Operation-produced artifacts (governance run directories, slush/burn-down beyond the backlog, scope-discovery registries) are NOT pre-scaffolded by setup — they are created lazily-and-announced by the verb that produces them (FR-016). The managed set is extensible (Principle II): a capability that needs a new installation-level working file adds it to setup's set as it migrates in.
- **FR-027**: The audit log MUST be **two-tier** (Clarification 2026-06-09): (a) a **per-feature audit log** is the primary partition — one per feature, created by the *feature* lifecycle (feature creation, or lazily-and-announced on that feature's first govern run) at a configured per-feature path pattern (default `specs/<feature>/audit-log.md`, co-located with the spec), bounding size by aligning the file boundary with the feature boundary; (b) a single **program-level audit log** per installation, scaffolded by installation setup, holds findings not scoped to a single feature. Installation setup scaffolds ONLY the program-level log; the config records both the program-log location and the per-feature-log path pattern (shared contract with the feature lifecycle and the read-side discovery).
...
- **FR-018**: An adopter MUST be able to configure the location of each governed working file independently. The config MUST support a **base directory plus per-file path overrides** (Clarification 2026-06-09), resolved with precedence **per-file override > base directory > audience-split default** (FR-019). Setup MUST create each working file at its resolved location and record that location in the installation config.
- **FR-019**: When a working file's location is not configured, setup MUST use a documented project-relative default and record the resolved location in config. Defaults are **split by audience** (Clarification 2026-06-09): human-facing docs (`ROADMAP.md`, `DESIGN-INBOX.md`) default to the installation root; tool-internal stores (backlog store, audit log, and config itself) default under a per-installation `.stack-control/` directory.
...
- **FR-021**: The setup, config, and resolution model MUST support multiple independent stack-control installations within a single repository, each with its own config and its own isolated set of working files. An installation root is marked by the presence of a stack-control config in a directory (Clarification 2026-06-09); the installation governs that directory's subtree, **excluding any nested child installations' subtrees**. Nesting is permitted (e.g. a repo-root program installation plus per-package installations).
...
- **FR-023**: A governed verb MUST deterministically resolve which installation it operates on by **walking up from the invocation context to the nearest enclosing installation config** (Clarification 2026-06-09), MUST operate only on that installation's working files, and MUST fail loudly when the walk reaches the filesystem/repo boundary without finding a config (never an arbitrary pick, never a fallback to a bundled copy). The "invocation context" MUST be defined surface-agnostically (FR-026) — a CLI working directory today, a client-supplied root in future.
- **FR-024**: Setup MUST **fail loudly and refuse** (Clarification 2026-06-09) any configuration in which a working-file location would be shared by two installations, or would escape its own installation's scope (into a parent, child, or sibling installation) — never silently allowing cross-installation interference. When installations nest, the nearest enclosing config wins (FR-023) and the parent's scope excludes the child's subtree (FR-021).
...
- **Installation**: an independent stack-control unit scoped to a subtree of a repository, **rooted at the directory containing its config** (Clarification 2026-06-09). Owns one config and one isolated set of governed working files. A repo may contain N installations; a verb belongs to the nearest enclosing one (upward walk); setup operates on exactly one per invocation. (This monorepo's `plugins/stack-control/` is itself an installation — the dogfood.)
- **stack-control installation config**: the single configuration artifact for one installation (its presence marks the installation root), recording a base directory plus per-file location overrides for every governed working file, including the program-level audit-log location AND the per-feature audit-log path pattern (FR-027). The authoritative input to read-side document discovery. The create-side (this feature) writes it; the read-side resolves it.
...
- **Audit log (two-tier)**: **per-feature audit logs** (primary, one per feature, created by the feature lifecycle, co-located with the feature's spec at `specs/<feature>/audit-log.md`) bound size by aligning the file with the feature boundary; a **program-level audit log** (one per installation, scaffolded by installation setup, default under `.stack-control/`) holds findings not scoped to a single feature. Other governance/scope-discovery working files (run directories, slush/burn-down beyond the backlog, scope-discovery registries) are operation-produced and created lazily by their verbs, not by setup.
...
- **Configurable locations with project-relative defaults**: every governed working file's location is independently configurable; when unset, audience-split defaults apply (Clarification 2026-06-09) — human docs at the installation root, internal stores under `.stack-control/`. The `.stack-control/` directory (already used in code for grammars) is the config home and thus the installation root marker (OQ-1).
...
## Open Questions *(all resolved — Session 2026-06-09)*

These were the genuine forks the operator owned; captured rather than silently defaulted (constitution Principle II). **All five resolved** in the `/speckit-clarify` session — see the Clarifications section for the canonical answers; retained here as a resolution ledger:

- **OQ-1 — Installation resolution model**: ✅ RESOLVED (Session 2026-06-09) — upward directory walk from the invocation context to the nearest enclosing config; config presence marks the root (FR-023). Surface-agnostic per FR-026.
- **OQ-2 — Default location convention**: ✅ RESOLVED (Session 2026-06-09) — audience-split: human docs (`ROADMAP.md`, `DESIGN-INBOX.md`) at the installation root; internal stores (backlog, audit log, config) under `.stack-control/` (FR-019).
- **OQ-3 — Configurable-location granularity**: ✅ RESOLVED (Session 2026-06-09) — both: base directory + per-file overrides, precedence per-file > base > default (FR-018).
- **OQ-4 — Managed-set membership**: ✅ RESOLVED (Session 2026-06-09) — installation setup scaffolds config + roadmap + inbox + backlog + **program-level** audit log; per-feature audit logs come from the feature lifecycle (FR-027); operation-products (run dirs, slush/burn-down, scope-discovery registries) stay lazy; set extensible (FR-001).
- **OQ-5 — Nested-installation precedence & collision rules**: ✅ RESOLVED (Session 2026-06-09) — nesting permitted; nearest config wins; parent scope excludes child subtrees; configured-location collisions/escapes are refused with a fail-loud error (FR-021/FR-023/FR-024).
