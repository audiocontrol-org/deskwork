...
- **FR-006**: Every spawn MUST settle into exactly one recorded terminal state — completed, timed-out, spawn-failed, killed-no-liveness, or killed-external (terminated by a signal the wrapper did not send — an external/out-of-band kill) — persisted in the run artifacts and distinguishable downstream.
...
- **Spawn terminal state**: the single settled outcome of one model invocation — completed | timed-out | spawn-failed | killed-no-liveness | killed-external — plus enforcement state (enforced | unenforced) and liveness state (monitored | unmonitored).
