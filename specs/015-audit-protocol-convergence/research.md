...
**Outcome (live calibration, 2026-06-12)**: the hypothesis held. On a 12.4 KB per-phase payload sonnet met all three bars — latency **224 s** (< 300 s derived timeout; ~10× under the 2226 s big-unit figure, confirming the payload-size coupling this decision rests on), 4 file:line-anchored depth-findings (one corroborating an opus finding), 4 tool calls + zero repo mutations under plan-mode. **Operator decision (2026-06-12): PROMOTED to the default fleet** (opus + codex + sonnet — three lanes), rather than parking it at opt-in. The "broader sample" the override-profile-first path called for is gathered as **live field evaluation across real change sets**, not pre-promotion calibration runs — a deliberate trade chosen for the cross-model-diversity payoff. Evidence + watch-item (latency at >~23 KB): quickstart-results.md § US5.
