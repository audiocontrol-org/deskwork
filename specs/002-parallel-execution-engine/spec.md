...
- **Reconcile conflict** between two parallel tasks' changes: the engine dispatches the conflict resolution as a task to a capable backend (given both tasks' intent) and applies it; if a resolver task can't produce a resolution, that's a task failure (FR-019: retry once, then quarantine, continue). After assembly, iff any conflict was resolved, a single whole-solution audit-barrage runs over the run branch (FR-022) and its findings flag the branch for promotion-time review. The engine never silently picks a winner, never halts the run, and never waits for the operator (FR-007 / FR-022 / FR-021).
...
- **FR-022**: The reconcile-conflict audit MUST run **exactly once per run, over the ENTIRE assembled run integration branch** (never per conflict site), and **only when one or more conflicts were resolved** during assembly — a zero-conflict assembly runs no conflict audit. It uses **cross-model audit-barrage** (slice-001 governance, applied concretely; Principle II — not abstracted into a validator port until a second validator exists). Its findings are **recorded** in the run record and the normal finding machinery; a run whose whole-solution audit surfaces blocking findings still completes (unattended, FR-021) but the run branch is **flagged for operator review before promotion**. *(Because the verdict is recorded, not consumed to gate individual merges, this is simply the one-way post-run governance seam of FR-017/FR-018 made mandatory when conflicts occurred — it introduces no engine→governance coupling. Validating the assembled whole also catches cross-resolution interactions a per-site audit would miss, at a fraction of the cost.)*
...
- **Run record**: the per-run artifact (per-task backend attribution, status incl. retried/failed/quarantined, conflict-resolution diffs, the whole-solution audit verdict when run, output location / branch ref).
