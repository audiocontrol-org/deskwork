...
`govern --mode implement` builds a barrage payload that excludes the feature's own audit-log (and governance bookkeeping surface) and other features' untracked scaffolds from the fold — while still folding the feature's own untracked files and newly-added source modules (FR-008 as amended by AUDIT-20260611-01). Multi-round governance loops can reach a clean zero-finding floor because no round re-reads the previous round's prose as if it were repository code.
...
- **FR-008** (US5, amended by AUDIT-20260611-01): The untracked fold MUST exclude files under other features' roots and the feature's own audit-log; everything else — the feature's own files AND non-feature files (e.g. newly-added untracked source modules) — MUST fold in. Each other-feature exclusion MUST be announced and recorded in the payload's `skippedOtherFeature` ledger. (The original "include only files belonging to the feature under audit" inclusion form silently dropped untracked source files — the feature root is the spec/docs directory, not the feature's code — excluding exactly the surfaces the fold exists to audit.)
...
- **SC-007**: The layout probe — `grep -rn '001-IN-PROGRESS' plugins/stack-control/src --include='*.ts'` excluding the shared resolver and tests — shows zero path *constructions* (fail-loud error-message/comment strings naming both layouts are compliant 013 loudness and exempt — research.md R7), and scope-export + scope-widen evidence both work against a `specs/NNN-slug` fixture.
