...
- [X] T032 [P] Create `plugins/stack-control/README.md` (purpose, install per repo plugin conventions, the three front-door verbs, the rehomed governance extension; no rot-prone version strings — link the releases page per `.claude/rules/documentation.md`)
- [X] T033 [P] Update the slice-001 completion note + any doc citing the old `plugins/dw-lifecycle/spec-kit/deskwork-governance` source path (informational; no behavior change — governance-extension.md § Move mechanics step 4). **DONE:** fixed the genuinely-stale forward-looking install command in the governance README (source `plugins/stack-control/spec-kit/deskwork-governance/README.md` + its installed copy, re-synced via `specify extension add` from the new home). Historical records (DEVELOPMENT-NOTES, audit-log, slice-001 specs, the 003 spec's own `git mv` instructions) describe the past correctly and were intentionally NOT rewritten (`.claude/rules/documentation.md`). Confirmed the hook command invokes the SOURCE govern.sh, so the previously-stale installed `govern.sh` copy was never executed (no live impact; re-sync removed dead drift).
- [X] T034 Self-hosting proof (SC-005 / FR-009 / quickstart Scenario F): through the front door in-session, `/stack-control:extend` advance Feature 2's spec (`specs/002-parallel-execution-engine`) toward runnable (or `/stack-control:define` a new spec), then `/stack-control:execute` it via native Spec Kit with governance firing — demonstrating the door drives subsequent stack-control development. **DONE (lightweight variant — operator decision 2026-06-05: prove the front door without building Feature 2):** ran the full arc on a throwaway spec — `define` (spec-check `spec=yes`) → `extend` (+plan → +tasks, spec-check tracking each) → `execute-check` (fail-loud before runnable → `runnable` after) → native execution of one trivial task → `after_implement` governance fired automatically (2/2 models, **0 manual barrage invocations**), findings routed to a throwaway slug so the real audit-log stayed clean. Throwaway artifacts removed afterward (all untracked; project dogfood-cleanup rule); real feature audit-log unchanged (0 open). The full-on Feature-2 proof was explicitly deferred by the operator.
- [X] T035 Run the full quickstart.md (Scenarios A–F) end-to-end; confirm SC-001..006 coverage. **DONE:** A (validate + `stackctl version` 0.37.0), B (extension enabled from new home, old path gone), C (execute-check fail-loud/pass), D (governance smoke green — last session), E (dw-lifecycle suite 2704 green + isolation guard), F (self-hosting proof — T034, lightweight variant). SC-001..006 covered.
