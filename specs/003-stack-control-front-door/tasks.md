...
- [ ] T034 Self-hosting proof (SC-005 / FR-009 / quickstart Scenario F): through the front door in-session, `/stack-control:extend` advance Feature 2's spec (`specs/002-parallel-execution-engine`) toward runnable (or `/stack-control:define` a new spec), then `/stack-control:execute` it via native Spec Kit with governance firing — demonstrating the door drives subsequent stack-control development. **PENDING OPERATOR SCOPE DECISION:** this proof drives native `/speckit-implement` over a *separate* feature (Feature 2, the parallel execution engine), which would begin implementing it — a scope call the operator owns, not an autonomous step inside Feature 1's implement loop.
- [ ] T035 Run the full quickstart.md (Scenarios A–F) end-to-end; confirm SC-001..006 coverage. **PARTIAL:** Scenarios A/B/C/D/E exercised this program (A: validate + `stackctl version`; B: extension enabled from new home; C: execute-check fail-loud/pass; D: governance smoke green — last session; E: dw-lifecycle suite green + isolation guard). Scenario F == T034 (pending the scope decision above).
