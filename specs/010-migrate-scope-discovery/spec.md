...
- **OQ-1 — Migration semantics confirmation.** Confirm vendor/copy (dw-lifecycle keeps a working copy until wholesale retirement) over a destructive move. Assumed: vendor/copy. Impact: whether dw-lifecycle's tests/baselines can break during this feature (they must not).
- **OQ-2 — Founding-increment boundary.** Does the founding increment that must ship first = the per-codebase clone-detector core (US1 + US2) with discovery agents / dispatch wrapper / registries / install (US3–US6) and the deferred wiring (US7–US8) as captured-but-later increments behind the same feature? Or is the full surface intended as one delivery? Capture-mode keeps all of it in this spec; this question only sets the v1 ordering/boundary. Assumed: clone-core-first (US1/US2 = P1).
- **OQ-3 — Codebase-boundary mechanism.** Confirm "per-codebase" = nearest-enclosing 009 installation (excluding nested children). Alternatives: explicit per-plugin `path` config, or a `roots` list in scope-discovery config. Assumed: 009 installation model.
- **OQ-4 — Per-component port-vs-rebuild.** Which components (if any) warrant a native rebuild rather than a port-and-generalize (e.g. anything coupled to deskwork's state machine / studio / stage vocabulary)? Assumed: port-and-generalize default; record exceptions per FR-004.
...
- **OQ-6 — Adopter-facing install machinery surface.** The original design shipped opt-in hook scaffolds + agent-prompt mirrors; per `.claude/rules/enforcement-lives-in-skills.md`, stack-control enforcement fires from skill bodies + CLI verbs, never installed git hooks. Confirm the migrated install machinery drops the hook-install verbs entirely (no `install-scope-discovery-hooks` analog) in favor of skill-body + CLI-verb wiring. Assumed: drop hook-install machinery.

## Captured for future expansion *(inherited from the 2026-05-24 design — not built in this increment)*

These were captured-as-future in the original canonization design and are restated so they are not lost; they are explicitly NOT part of this migration's build:
