...
- **No [NEEDS CLARIFICATION] markers:** genuine forks were captured in the `Open Questions` section with documented informed-default assumptions rather than inline markers, per capture-mode (Constitution Principle II). `/speckit-clarify` (Session 2026-06-09) resolved OQ-1, OQ-2, OQ-3, OQ-4, OQ-6; OQ-5 (install-drift advisory home) is deferred to `/speckit-plan` (low-impact P3, genuinely open home decision).
