...
On `--apply`, curate **reorders first, then archives** (it writes the live document for the reorder, then composes `archive --apply`). The **durability promise** (FR-010) holds: an interrupted or failed `curate --apply` **never silently loses or corrupts content** — any inconsistency is recoverable via version control (revert + re-run) and detectable via the coherence check below. The contract does **not** promise the whole operation is atomic; the write/recovery protocol is an implementation concern.
