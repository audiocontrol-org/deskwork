...
On `--apply`, curate first asserts the **committed-working-tree precondition** (FR-006) **once, at entry** — the target live document **and** its sibling archive MUST be committed (clean); an uncommitted target **fails loud** ("commit or stash `<path>` before --apply") with **zero writes**. Having passed that single entry guard, it then **reorders first, then archives** (it writes the live document for the reorder, then composes `archive --apply`) as a **single already-guarded operation**: the composed `archive` step does **NOT** re-assert the precondition (curate already did at entry), so curate's own intermediate reorder write — which dirties the working tree — never causes the archive step to self-fail. The **durability promise** (FR-010) holds: an interrupted or failed `curate --apply` **never silently loses content** — *because the entry guard ensured a clean pre-op target* — any inconsistency is recoverable via version control (revert to the last commit, restoring the exact pre-op state, + re-run) and detectable via the coherence check below. The contract does **not** promise the whole operation is atomic; the write/recovery protocol is an implementation concern.
...
- `2` — usage/config error, ungovernable document, parse failure, identifier-invariant violation, or uncommitted target on `--apply` (the fail-loud cases).
