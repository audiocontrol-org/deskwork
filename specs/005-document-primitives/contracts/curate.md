...
On `--apply`, curate first asserts the **committed-working-tree precondition** (FR-006) — the target live document **and** its sibling archive MUST be committed (clean); an uncommitted target **fails loud** ("commit or stash `<path>` before --apply") with **zero writes**. It then **reorders first, then archives** (it writes the live document for the reorder, then composes `archive --apply`). The **durability promise** (FR-010) holds: an interrupted or failed `curate --apply` **never silently loses content** — *because the precondition ensured a clean pre-op target* — any inconsistency is recoverable via version control (revert to the last commit, restoring the exact pre-op state, + re-run) and detectable via the coherence check below. The contract does **not** promise the whole operation is atomic; the write/recovery protocol is an implementation concern.
