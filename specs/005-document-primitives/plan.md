...
**Durable write/recovery protocol is an implementation concern.** The spec (FR-006/FR-008/FR-010) states only the durability **promise** — an interrupted or failed `--apply` never silently loses or corrupts content; because governed documents are version-controlled, any inconsistency is recoverable (revert + re-run) and detectable via the `curate` coherence NOTICE. The concrete write ordering across the live document + archive file, any temp-file/rename strategy, and crash-handling are **NOT specified in the spec** — they are pinned here and by the contracts + RED tests. The `archive-engine.ts` / `unarchive-engine.ts` / `curate-engine.ts` modules own the write path; the RED suite includes a simulated-interruption test asserting (a) no silent content loss, (b) the `curate` coherence check reports the resulting mismatch as a NOTICE, and (c) a version-control revert restores the pre-operation state. The spec deliberately does not promise two-file atomicity (mechanically impossible across two files), so the implementation is free to choose any write strategy that upholds the promise.
