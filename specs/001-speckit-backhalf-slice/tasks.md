...
- [ ] T006 [US1] Write the FAILING integration smoke at `scripts/smoke-governance-after-implement.sh` that, given a sample diff, runs the governance command and asserts (a) a new run-dir under `.dw-lifecycle/scope-discovery/audit-runs/`, (b) `INDEX.md` lists ≥2 model lanes, (c) findings appended to `docs/1.0/001-IN-PROGRESS/pluggable-lifecycle-providers/audit-log.md`. Run it; confirm it FAILS (RED) because `govern.sh` is a skeleton.
...
- [ ] T010 [US1] Make the smoke GREEN: run `scripts/smoke-governance-after-implement.sh` and confirm run-dir + ≥2 lanes (claude+codex) + findings lifted (SC-001/002/004).
