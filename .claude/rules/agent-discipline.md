...
A feature's lifecycle has two halves: shipping the work and closing it. Across cycles, the shipping half wins structurally — there are gates that refuse to let work ship without tests / review / verification, and natural rewards (PR merged, version bumped, tag pushed) at every shipping waypoint. The closure half has historically had no equivalent. Issues stayed open across releases because nothing structurally walked them. Workplan TBD markers shipped to merge because nothing structurally refused them. Parked branches accumulated in the branch-list noise because nothing structurally surfaced them. Stale worktrees (post-merge feature checkouts whose work has shipped) accumulated in the operator's `~/work/<project>-work/` directory because nothing structurally walked them either — the failure mode at the bottom of the agent-driven implementation flow (a session opened against a worktree whose work shipped two releases ago, silently re-implementing what's already in main) is captured at [#347](https://github.com/audiocontrol-org/deskwork/issues/347).
...
| `/dw-lifecycle:session-end` hygiene capture | End-of-session — writes a structured hygiene-observations + next-session-recommendation block into `DEVELOPMENT-NOTES.md`. The block always emits (even on the "no signals" branch) so session-start always sees a written record from the prior session. The block now carries four streams — commit markers, workplan TBDs, issues referenced this session, and stale worktrees flagged this session (Phase 11). |
...
| `/dw-lifecycle:debt-report` | On-demand operator invocation — cross-source snapshot of three of the four debt streams (issues + workplan TBDs + parked branches). The read-only baseline the mutating verbs operate on. |
| `/dw-lifecycle:worktree-report` | On-demand operator invocation — fourth-stream read for stale + orphan worktrees. Sibling of `:debt-report`. Surfaces nine per-criterion staleness signals + a verdict + a recommended disposition. |
| `/dw-lifecycle:triage-issues`, `:promote-deferrals`, `:archive-branch`, `:dismantle-worktrees` | On-demand operator-triggered mutations against the snapshots. Batched-proposal contract (`propose` writes a JSON file; operator fills in dispositions; `apply` validates all-or-nothing then dispatches) keeps the closure work auditable. `:dismantle-worktrees` composes with `:archive-branch` via `--archive-first` so the branch contents are preserved as a named tag before the worktree is removed. |
...
- When a stale worktree surfaces in `/dw-lifecycle:worktree-report` (or in `:session-end-hygiene`'s "Dismantle stale worktrees:" recommendation): pick one of the dispositions in `:dismantle-worktrees propose` (dismantle / archive-then-dismantle / prune-orphan / skip), then `:dismantle-worktrees apply`. Use `archive-then-dismantle` when the branch has novel commits worth preserving — the worktree gets removed and the branch contents go into an `archived/<branch>-<date>` tag. This is the structural mechanism that closes the `:#347` stale-branch-sessions failure mode at its source: every shipped feature's worktree gets a structural prompt to dismantle, instead of relying on the operator to remember.
