...
| `/dw-lifecycle:session-end` hygiene capture | End-of-session — writes a structured hygiene-observations + next-session-recommendation block into `DEVELOPMENT-NOTES.md`. The block always emits (even on the "no signals" branch) so session-start always sees a written record from the prior session. The block now carries four streams — commit markers, workplan TBDs, issues filed this session, and stale worktrees flagged this session (Phase 11). |
