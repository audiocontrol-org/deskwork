...
**Scope of the protocol — point fixes are NOT spec-driven feature execution.** The full chain + per-phase governance applies to **spec-driven feature execution** (a roadmap feature taken through `/stack-control:define` → `/stack-control:execute`). It does **NOT** apply to a **point solution or targeted bug fix** — a small, well-understood, directly-picked-up fix (e.g. a roadmap `impl:fix/<slug>` item with the fix already described). For those, the correct path is RED-first TDD → **code review** → commit → push → ship; **do not run `govern`/audit-barrage**, do not author a spec, do not manufacture a phase chain. Operator, 2026-06-19: *"I don't care about governance for point solutions and targeted bug fixes."* This is NOT an offroad — it is the protocol correctly scoped to feature work; running heavyweight governance on a one-file bugfix is its own friction (the per-phase friction the burndown exists to reduce). When unsure whether something is a "feature" or a "point fix," that ambiguity is itself a scope question the operator owns — ask. (Issue-closure discipline is unchanged: a point fix still isn't "fixed" until verified in a formally-installed release.)

**Code review IS run as a matter of course on point fixes and burn-downs — it is NOT part of the skipped governance.** Operator, 2026-06-26: *"we should do code reviews as a matter of course on point fixes and burn-downs."* The heavyweight cross-model audit-barrage is skipped for point fixes (above); the **lightweight built-in `/code-review`** (a few finder angles + adversarial verify over the branch diff) is **not** — run it as a standing step of every point-fix / burn-down tranche before reporting it done, without waiting to be asked. It is cheap, it sits on TOP of the green deterministic floor (it is NOT a substitute for `tsc` + suite green), and it earns its keep: the 2026-06-26 H2 liveness-window clamp was a real latent bug a green floor passed and `/code-review` caught. This is the point-fix analogue of the audit-barrage's stochastic defense-in-depth — scaled down to the built-in reviewer, never skipped. Apply the findings (RED-first for any real bug) before the tranche is "done."
