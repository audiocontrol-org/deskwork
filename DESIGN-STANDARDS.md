...
- 2026-05-29 — Graphical-entries Phase 7 group review surface design accepted (Tasks 7.3 + 7.4 folded). Picked **Direction B** (composed multi-lane default with list toggle) over Direction A (flat-list-with-sticky-summary-strip) and Direction C (stacked-no-toggle). The group review surface renders members as a miniature multi-lane swimlane composition reusing the Phase 5 swimlane primitive scoped to the group's member set; a section-head toggle flips to flat list. Empty-members fallback degrades to the group's `artifactPath` content or a centered empty-state CTA. Picked **Direction 1** (vertical pull-tab on the row's left edge) over Direction 2 (kicker line above slug) and Direction 3 (trailing inline chip near `⋮`) for the "Member of:" affordance on member entry rows in the lane dashboard. The kraft-color pull-tab carries a member-count badge; tap expands inline with parent group(s) as clipboard-copy back-links. Mirrors the `.er-marginalia-tab` / `.er-outline-tab` precedent per `.claude/rules/affordance-placement.md`. Group-vs-member-stage divergence detection is deferred to doctor (Task 7.5.3's `group-all-members-cancelled` rule is on the workplan; no on-surface divergence indicator in v1). Decision brief at `docs/studio-design/ACCEPTED/2026-05-29-group-review-surface/brief.md`; rejected alternatives at `docs/studio-design/REJECTED/2026-05-29-group-review-surface-alternatives/brief.md`. Implementation lands as Phase 7 Tasks 7.3 + 7.4 commits.
