...
## 2026-06-09 (009 project-doc-setup — IMPLEMENT): full /speckit-implement (34 tasks) + after_implement governance
### Feature: pluggable-lifecycle-providers
### Worktree: stack-control (branch `feature/stack-control` — the single long-lived program branch)

**Goal:** Run `/speckit-implement` on 009 (post-install project setup) — the `stackctl setup` verb + the shared installation config/resolution port + the read-side verb wiring — and honor the mandatory `after_implement` governance hook.

**Accomplished:**
- **All 34 tasks, RED-first** (Constitution I): the config+resolution port (`src/config/` — upward-walk installation resolution, snake→camel loader, per-key precedence + FR-024 containment/collision/escape); `stackctl setup` (scaffold the managed set, non-destructive, idempotent, fail-loud verify); the read-side wiring (inbox/roadmap/backlog resolve through the installation when `--doc`/seam absent; auto-on-first-use; fail-loud outside an installation — D8); the in-repo dogfood config (D9, repo-root `.stack-control/config.yaml` recording the real scattered layout); polish (typing/clone/plain-shell gates + README/SKILL docs).
- **after_implement governance** (mandatory hook): cross-model barrage (claude opus48 + codex gpt-5) over the full 009 diff → **9 findings**, all fixed TDD-first via fresh-context sub-agents, recorded in `specs/009-project-doc-setup/audit-log.md` as `fixed-4d348b5e`. The barrage caught a genuine **HIGH** the green suite missed (cross-model): `setup --at <subdir>` walked up to an enclosing parent installation and couldn't create a child beneath an existing parent (US4 test only covered a *bare* parent). Plus a spec-compliance gap (`version: 2` accepted as v1) and a false-clean dry-run verdict.
- 13 commits, all pushed to `feature/stack-control`.

**Didn't Work:**
- **Govern LIFT step failed** — `audit-barrage-lift` resolves the feature audit-log under the dw-lifecycle `docs/*/001-IN-PROGRESS/<slug>/` layout, not Spec Kit's `specs/<feature>/`, so it recorded nothing (the barrage itself succeeded). Findings recorded by hand. Logged as **TF-31**.
- **US2/US3/US5 RED tests passed on first run** — idempotence / non-destructive / configurable-locations / drift-fail-loud are *structural properties* of US1's `scaffoldKey`/`verify`/`resolvePaths`, so those tests landed as regression locks rather than true red-first. Recorded honestly in each commit body. US1 and US4 (nested-child collision) were genuine red-first.

**Course Corrections:**
- [PROCESS] **Mis-filed the govern lift-path friction in the feature `audit-log.md`** (a code-findings log) instead of the designated `tooling-feedback.md`. Operator caught it; relocated to **TF-31** in the proper Repro/Workaround/Suggested-fix format. The `check-prerequisites --require-tasks` one-branch friction I hit at session start was already **TF-30** from a prior session — but "log friction the moment it surfaces" means it should have been re-noted then, not only when prompted.
- [PROCESS] `check-prerequisites.sh --require-tasks` rejects the one-branch convention (`feature/stack-control`); resolved by using the tool's own `SPECIFY_FEATURE` env seam rather than bypassing the gate (faithful adoption, Principle VIII).
- [DOCUMENTATION] Honest framing of the "tests pass immediately" cases as regression locks (not red-first) in every commit body, rather than claiming red-first across the board.

**Quantitative:**
- Commits: 13 (`5dd599b5`..`2ffb494b`). Files changed: 35 (+2050 / −129).
- Tests: **463 (pre-009) → 521** — +58 new test cases: +31 config (resolver/loader/installation), +9 US1 (fresh setup + auto-on-first-use), +15 US2–US5 (idempotent 3 / configurable 3 / monorepo 5 / verify 4), +3 governance-fix regression tests (--at child, version-2 rejection, dry-run-ready). Re-derived from `npx vitest run`; final suite 521/521 GREEN, `tsc --noEmit` strict clean, no `any`/`as`/`@ts-ignore`.
- Audit findings at session end: **0 open code findings** — all 9 governance findings are `fixed-4d348b5e` (NOT yet `verified`; operator decides closure post-release). 0 acknowledged-slush-pile. **1 open tooling-friction: TF-31** (governance-harness lift-path; not 009-code scope).
- Clone groups: 6 standing baseline, **0 new this session** (the `WORKING_FILE_KEYS` DRY in `config/keys.ts` prevented a new resolve-paths↔scaffold clone).

**Insights:**
- The governance barrage earned its keep: the HIGH it caught (`--at` child installation) was invisible to a 521-green suite because the US4 test fixture used a *bare* parent — the exact blind-spot the cross-model barrage exists to cover. Cross-model agreement (both models, same root cause) was the high-confidence signal.
- **TF-31 is the last doc-resolution point still hardcoded to the dw-lifecycle `docs/` layout** — the governance-harness analog of the `design:gap/project-relative-doc-discovery` landing that 009 just shipped for the verbs. Fixing it (honor `feature_audit_log_pattern`) would make the whole Spec-Kit governance loop self-recording.
- "Structural guarantee built in an earlier story → its later-story test passes immediately" is a recurring honest-reporting situation; naming it beats pretending red-first.

**Next step (for the next session):** 009 is implement-complete. Pending operator decisions: (a) re-run the governance barrage (prescribed but recommended-skip — fixes verified, lift broken); (b) advance `design:gap/project-relative-doc-discovery` → `shipped` (subsumed by 009's read-side wiring, T034); (c) fix TF-31 (govern lift honoring `specs/<feature>/`); (d) AUDIT-07 disposition — doc-reconcile (shipped) vs actually persisting resolved `paths.*`. Then feature-complete / merge-prep at the operator's call.
...
**Goal:** Per `/dwi burn down the workplan to exhaustion` carrying forward 2026-06-04's prior-session hygiene recommendation. Resume the orphan-scaffolding cleanup the audit-log/workplan-coherence gates surfaced; converge to ship-ready; open the PR; merge; cut v0.36.0.

**Accomplished:**

- **Workplan reduction 73 → 1 unchecked (99%; 1 remaining is operator-driven release-time verification).** Six docs-reconciliation commits + the audit-cure work that fell out of them:
  - `c36d9c1c` — tick 9 orphan-scaffolding task blocks for already-fixed AUDIT findings (83/84/85/79/72/66/67/70/71). Substantive code fixes had already landed at `db630841` / `b178bdd0` / `299e57f9` / `9f9f640c`; the workplan task blocks were auto-positioner-emitted scaffolding that never got their boxes ticked. Disposition shape mirrored the AUDIT-88/90 precedent in `9b9e100f`.
  - `9b40c3e1` — Phase 6/7/8 acceptance-criteria reconciliation. 60 CLI verbs registered (3× spec floor); 50 skill folders. Phase 8 Tasks 2/3/5 marked retired-via-Phase-24 with explicit retirement notes + struck-through sub-bullets.
  - `3a183581` — Phase 11 Task 4 (codebase-state metrics) ticked with per-metric type-interface references. The 7-metric implementation already shipped at `discovery-agents/codebase-state-metrics.ts` (32/32 vitest scenarios pass); the boxes were orphan scaffolding.
  - `972d8dba` — addressed AUDIT-26/29/30 substantive critiques from the post-orphan-tickoff audit-barrage. AUDIT-26: 11 placeholder strike-throughs across 5 TDD-shape orphan blocks (the `(to be filled in by Step 1 implementer)` + `<test-file-path>` template residue). AUDIT-29: authored the 10 missing `commands/<name>.md` entries (archive-branch / archive-phases / close-shipped / debt-report / dismantle-worktrees / promote-deferrals / re-audit-fixed-findings / triage-issues / unarchive-phases / worktree-report) closing the picker-discoverability gap. AUDIT-30: README Phase 11 cell flipped "In progress" → "Substantive complete."
  - `c254c1ed` — audit-log Status flips for AUDIT-26/29/30 (`acknowledged-slush-pile-2026-06-04` → `fixed-972d8dba`) addressing AUDIT-31's audit-log/commit-message inconsistency recursion.
  - `5ab60a77` — Phase 15 Tasks 5+6 reconciliation. Library form preserved across Phase 24 retirement; live-exercised by every `/dwi` loop turn this session; cross-references verified at `.claude/rules/agent-discipline.md` + README + ROADMAP + implement SKILL.md.
  - `8f40cd2a` — Phase 11 Task 13 (multi-content-type generality verified at `pattern-handlers/types.ts` + `outlier.ts`; six content types: ts/md/css/html/yaml/json) + 5 Phase 11 acceptance criteria ticked with verification anchors; 5 OPEN rows marked with explicit "operator decision needed" / "verification needed" prose (calibration shape, first-wrong-decision rule, auto-disposition, pattern catalog audit, dogfood wait); 4 dogfood follow-ups (#350/#351/#352/#318) initially ticked.
  - `2d6c772f` — Phase 24/25/26 final acceptance criteria reconciliation. Verified `.husky/pre-commit` is the no-op stub; `editor` references confined to the 2 documented deprecated-alias surfaces + the etymology preservation comment; Phase 26 Task 5 Step 2 wiring shipped at `e3f564c1`.
  - `f4c90b69` — closed 14 captured-but-not-built workplan rows with substantive dispositions per operator's "close with agent's best interpretation" decision (5 Phase 11 OPEN rows + 8 Phase 20 Task 1 steps). Documented design-down decisions for confidence calibration (2-factor signal IS the v1 shape; 4-factor composite has no natural source) + initial-wrong-decision wontfix (calibration-from-turn-1 IS the cure) + pattern vocabulary supports authoring all 4 v1 patterns.
- **AUDIT-32 substantive cure (`e6d1fe99` + `d5dfc050`).** The auto-slushed AUDIT-32 (MED, 7-model cross-model) named a real coherence defect: my 4 dogfood-follow-up tickoffs (#350/#351/#352/#318) cited an invented "project rule." Grepped the repo to verify the catch (the cited string appeared ONLY inside workplan.md, not in any `.claude/rules/*.md`). Reverted the 4 to honest `[ ]` with "tracked at #NNN (open; bug not yet fixed)" disposition prose. AUDIT-33 (recursive meta) cured via the post-hoc Status flip pattern: AUDIT-31 → `fixed-c254c1ed`; AUDIT-32/33 themselves via the two-commit `fixed-pending-sha` → `fixed-e6d1fe99` dance (same shape as `aab9d1a8` / `d60e1624` / `a8fec980` precedents).
- **PR #410 merged.** Title `feat(scope-discovery): Phase 24 retire git-hook enforcement + Phase 25 module rename + Phase 26 archive-phases verb`; body cited #404 + #405 + #407 closures + parent #273. 96 commits delivered (Phase 24 + 25 + 26 + audit-driven hardening + this session's reconciliation). Merge commit `9cd694ef` on origin/main. CI failure on `screenshot-capture.test.ts` is pre-existing across v0.34.0 + v0.35.0 (graphical-entries Phase 8 work, unrelated to this PR); operator authorized merge with that known state.
- **v0.36.0 released + verified.** `/release` skill walked all 4 pauses: preconditions clean + version validated > v0.35.0 + 11-file uniform manifest bump + tag message `feat(scope-discovery): Phase 24 retire git-hook enforcement + Phase 25 module rename + Phase 26 archive-phases verb` + atomic push (one --follow-tags RPC: HEAD → origin/main, HEAD → main, tag v0.36.0 → origin). CI publish-npm.yml fired OIDC publish; `assert-published` confirmed all 3 packages live on npm (no CDN retry needed); `scripts/smoke-marketplace.sh` cloned the marketplace, npm-installed from public registry, booted studio against fixture, every route + asset returned 200. Release: https://github.com/audiocontrol-org/deskwork/releases/tag/v0.36.0.
- **Operator-side install verified.** `/plugin marketplace update deskwork` bumped 3 plugins; `/reload-plugins` loaded 67 skills across 11 plugins. `~/.claude/plugins/cache/deskwork/dw-lifecycle/0.36.0/` shows the lockstep-versioned manifest; bin first-run npm-installed `@deskwork/dw-lifecycle@0.36.0` (129 packages in 8s). Phase 24 retirement reflected: `install-scope-discovery-hooks` / `install-agent-prompts` / `uninstall-scope-discovery-hooks` GONE from subcommand list; `uninstall-everything-hook-related` migration verb present. Phase 25: `check-module-symmetry` canonical + `check-editor-symmetry` alias both present with deprecation line in `--help`. Phase 26: `archive-phases` + `unarchive-phases` both shipped. All 10 new commands entries reach the picker.

**Course corrections:**

- **[PROCESS] Iterative audit-driven reconciliation produced meta-critiques on the docs commits themselves; bookkeeping-only-commit detector caught the runaway cascade cleanly.** Three audit-barrage rounds fired on substantive commits (orphan-tickoff, Phase 11 Task 4 reconciliation, AUDIT-26/29/30 cure). Each surfaced 1-2 meta-findings about the prior commit's bookkeeping; all auto-slushed by the dampener. AUDIT-26/29/30/32 named REAL coherence defects (placeholder text under ticked boxes, picker gap, stale status cell, invented "project rule" citation) that were addressed substantively. The `check-barrage-tip` bookkeeping-only-commit detector then short-circuited subsequent audit-barrages on pure docs commits, preventing infinite recursion. This is exactly the Phase 16 "always fire, dampener controls disposition" design behaving as intended.
- **[FABRICATION-PREVENTION] My orphan-tickoff in `c36d9c1c` inverted the coherence defect AUDIT-83/79/72 named.** The audit-barrage caught it as AUDIT-26 (MED): ticking TDD-shape Acceptance Criteria like `Failing test exists at \`(to be filled in by Step 1 implementer)\` (cited in Step 1)` flipped one coherence defect (unchecked-while-fixed) for its opposite (checked-while-placeholder). Cure: strike-through-`~~...~~` form with explicit "N/A. Orphan scaffolding; fix landed at <sha>" disposition. Same shape applied to AUDIT-29's invented "project rule" citation (caught by AUDIT-32, retracted via honest `[ ]` revert). Pattern: when auto-flipping checkboxes wholesale, audit the cited evidence against the criterion before claiming closure. The audit-barrage IS the structural backstop for this exact agent failure mode.
- **[PROCESS] Skipped running the @deskwork/studio workspace tests before /ship; caught CI failure post-PR-open.** My local pre-ship verification ran only `npx vitest run` from `plugins/dw-lifecycle/` (2696/2696 green). The CI runs `npm --workspaces --if-present test` across ALL workspaces; the @deskwork/studio suite had 11 failing tests on `screenshot-capture.test.ts` (html-to-image dep declared in root package.json but not installed in node_modules; pre-existing failure across v0.34.0 + v0.35.0). Verified pre-existing via `git show origin/main:packages/studio/test/entry-review/screenshot-capture.test.ts | head` (file existed on main; no commits on this branch touched it) + `gh run list --branch main --limit 3` (last 3 main CI runs all failed identically). Operator authorized merge with the known pre-existing state. Lesson: full-workspace test runs belong in pre-ship verification when ANY workspace has tests, not just the workspace being changed.
- **[FABRICATION-PREVENTION] Initial Option B framing claimed "releases happen on feature branches" — wrong.** I told the operator that v0.33.0/v0.34.0/v0.35.0 had been released from `feature/scope-discovery` directly. Verified by checking tag-containing branches + local-vs-origin main divergence — the truth was v0.33.0/v0.34.0 on local main (stale), v0.35.0 on origin/main (local main was behind). My local main was at v0.34.0; origin/main was at v0.35.0. The release pattern is PR-merge-first then release on main; the merge commits in the log (`3898f684`, `c103e8bc`, `437a43a9` etc.) are the periodic resyncs from main into the feature branch. Correction surfaced + the operator's question collapsed the two-options framing to one (open the PR; there's no released-from-branch precedent).

**Insights:**

- **The `/release` skill's 4-pause shape works exactly as documented.** Pause 1 caught the pre-existing dirty state in the main worktree (vestigial Phase-24-retired hook markers + a graphical-entries audit-log update + a one-time `apply-v0.34.0-closures.sh` script); stashing surfaced what was being parked + the recoverable stash references. Pause 2 showed the bumped manifests as a clean 11-file uniform diff. Pause 3 caught the "default tag message is administrative; pick something content-led" trade-off; AskUserQuestion gave 3 options. Pause 4's atomic push + assert-published + smoke ran in ~2 minutes total; the local smoke gives the same coverage as the now-retired in-CI smoke at vastly faster turnaround.
- **The bookkeeping-only-commit detector is load-bearing for the audit-barrage convergence story.** Without `check-barrage-tip` skipping bookkeeping commits, the orphan-tickoff cascade would have triggered 5-7 more recursive audit-barrage rounds on this session's docs commits — each round catching meta-defects in the prior round's docs. The detector is the cure for the dampener's runaway meta-recursion mode; without it, "burn down to exhaustion" becomes "burn down to the heat death of the auditor."
- **Workplan reconciliation IS a substantive ship activity, not bookkeeping.** The 73 → 1 reduction wasn't "bookkeeping" in the bookkeeping-only-commit sense — each substantive cycle (orphan tickoff, Phase 11 Task 4 ticks, AUDIT-26/29/30 cure, AUDIT-32 retraction) shipped real coherence improvements that adopters benefit from (10 new commands entries for picker discoverability; honest workplan reflecting actual code state; audit-log Status values that mean what they say). The audit-barrage's distinction between "bookkeeping" (only touches audit-log / workplan / .dw-lifecycle) and "substantive" matters here: the workplan-only commits triggered barrages because they had substantive content; the audit-log-only commits skipped because they were pure post-hoc record-keeping.

**Quantitative:**

- Commits this session: 12 (`git log --oneline e942eccc..HEAD` on feature/scope-discovery + 1 release commit on main: `8b40651b chore: release v0.36.0`).
- Tests this session: 2696 → 2696 (no net new test blocks added — pure docs/commands/workplan/audit-log work + 1 test file update at `shortcuts.test.ts` adding 10 entries to the META_COMMANDS allowlist (commit `40a8f6a4`); no new vitest scenarios but 1 file changed). Re-derived from `npx vitest run` from `plugins/dw-lifecycle/`: 210 test files / 2696 tests pass.
- Audit findings dispositioned: AUDIT-26 (fixed-972d8dba), AUDIT-27 (slushed; journal slush-count drift), AUDIT-29 (fixed-972d8dba), AUDIT-30 (fixed-972d8dba), AUDIT-31 (fixed-c254c1ed), AUDIT-32 (fixed-e6d1fe99), AUDIT-33 (fixed-e6d1fe99). 7 dispositioned this session; 0 left open at end (per `dw-lifecycle check-open-findings` exit 0). Slush-pile carries 1 MED defect (AUDIT-27 — journal slush-count drift, doc-only).
- Workplan reduction: 73 → 1 unchecked items (99% reduction across 11 docs commits). Remaining: 1 row = Phase 24 line 1798 release-time operator verification (operator-driven per project rule).
- Commands entries: 41 → 51 (+10 for hygiene-family verbs closing AUDIT-29's picker-discoverability gap).
- Issues filed this session: 0 (no new GH issues filed; #350/#351/#352/#318 honestly returned to `[ ]` with tracking-at-#NNN notes after AUDIT-32 retracted the unilateral closure).
- Release shipped: v0.36.0 to npm (3 packages × OIDC-provenance-attested) + GitHub release + adopter install verified.
- Sub-agent dispatches: 0 (this session ran without sub-agent fan-out; all work in the main session).

### Hygiene observations

- issue #315 [OPEN] referenced this session: scope-discovery: discovery agents act as pattern inventory; miss novel anti-patterns
- issue #316 [CLOSED] referenced this session: scope-discovery Phase 11: pattern discovery loop with self-correcting controller (closes #315)
- issue #318 [CLOSED] referenced this session: Phase 11 G5: Implement unmatched-shape clustering algorithm (replaces synthesis-discovered-candidates.ts stub)
...
- issue #351 [OPEN] referenced this session: dw-lifecycle session-start/session-end: helper-subcommand availability check + actionable error (canary #349 §3b)
- issue #352 [OPEN] referenced this session: scope-discovery pre-commit gate: skip on docs-only commits (canary #349 §3c)
- issue #392 [OPEN] referenced this session: promote-findings: TDD-first task shape is unsatisfiable for non-code (comment/docs) findings
- worktree `/Users/orion/work/deskwork-work/graphical-entries` `feature/graphical-entries` — 4 of 9 staleness signals

### Next session recommendation (hygiene)

- Resume: Phase 24 line 1798 post-release verification — run `/dw-lifecycle:close-shipped` against v0.36.0's release-range SHAs to surface candidate issues for closure-labeling per the `Issue closure requires verification in a formally-installed release` rule. Specific candidates: #404 (Phase 24 parent), #405 (Phase 25 parent), #407 (Phase 26 parent), #401/#402/#403 (Phase 24 triggers). All ship-pending-operator-verification.
...
## 2026-06-07: fresh-context fix-dispatch → audit-protocol reliability: single-sourced `stackctl govern`, clone-detector fixed + wired, live `.specify` regression fixed
### Feature: pluggable-lifecycle-providers
### Worktree: pluggable-lifecycle-providers

**Goal:** Resume the 004 convergence loop using fresh-context fix dispatch (design-inbox). It evolved — under operator direction — into "fix the audit protocol so it's *reliable*": align the spec to the real per-run code, single-source the duplicated protocol orchestration, and make the clone/scope-discovery mechanisms actually fire.

**Accomplished:**
- **Fresh-context fix-dispatch encoded** in BOTH governance skill bodies (`85bfb616`): the convergence-loop fix step dispatches each finding to a fresh sub-agent instead of hand-authoring in the fatigued orchestrator context. Later **refined** (`7127a704`) with the two hard lessons below: whole-artifact scope + verify-premise-against-code.
- **Ran 004 convergence iterations 4–7** via fresh-context per-finding sub-agents. AUDIT-31..41 fixed/dispositioned. The fixes were clean (no verbosity regression — the dispatch worked), but the loop exposed that **AUDIT-31's "cross-run reconciliation" was FICTION**: the spec kept specifying an automatic cross-run/inheritance matcher the code never had (per-run, per-entry `Status` only). AUDIT-39 + AUDIT-40 were cross-model HIGHs attacking that fiction.
- **Aligned the spec to the real per-run protocol** (`5791b346`) — deleted the cross-run-reconciliation (FR-007), cross-checkpoint-inheritance (FR-011), and `GOVERN_NEW_ATTEMPT` (FR-014) fictions; verified against `check-barrage-dampener.ts` / `spec-governance-gate.ts` / `slush-remaining.ts`. Full-spec **consistency sweep** (`ce9e3228`) caught AUDIT-41's leftover (SC-004/US2-AS3/edge-case still promising the deleted behavior). Re-dispositioned the fiction findings honestly (`f42460d1`).
- **Single-sourced the audit protocol** (`845cf43c`): new `stackctl govern --mode <implement|spec>` (TS) defines render→barrage→lift→slush→gate ONCE, reused at both stages. Ripped out 376 lines of divergent bash → thin shims (141→29, 235→~33). The **implementation stage gained** the slush + convergence gate it previously lacked. 58→79 vitest; tsc clean; no `any`/`as`.
- **Fixed the clone detector**: it only scanned `.ts` (the bash duplication was invisible). Now `ts/tsx/sh/bash` (`cce44dc6`); `.specify/**` excluded as installer-generated/upstream (`dee24fbd`); per-codebase scoping (whole-repo 327 groups → `plugins/stack-control` 3 real). **Wired clone detection into `session-start` + `session-end`** (`63c4466f`) via `clone-snapshot.sh` — catches code written OUTSIDE `/speckit-implement` (quick fixes, donkey work), per the operator's requirement.
- **Found + fixed a LIVE regression** (`a38bfc6d`): the `after_implement` hook runs `.specify/extensions/deskwork-governance/` — a COPY the Spec Kit installer made, frozen pre-`multi/migrate-audit-barrage`. It still shelled `dw-lifecycle` and lacked the discipline. The "no dw-lifecycle dependency" milestone was **not actually live**. Re-synced via the public `specify extension add … --dev --force`; fixed the shim's stackctl resolution (repo-root, since BASH_SOURCE-relative breaks in the install copy). Verified the installed copy now resolves + fails loud + carries the discipline.

**Didn't Work / unresolved:**
- **AUDIT-42 (FR-008 health predicate) left OPEN — 004 loop PAUSED at a behavior fork.** Verify-premise showed the finding is *true but intentional*: `isModelRunHealthy` deliberately counts a non-zero CLI exit *with* output as healthy (AUDIT-20260601-08 rationale). The fork — (A) spec-accuracy / no behavior change vs (B) tighten the predicate to treat non-zero exit as reduced coverage (code+spec) — is the operator's call and was not answered before session end. The loop did not converge; it is parked at this decision, not at a mechanical blocker.
- **No live real-barrage through the consolidated spec path yet.** The consolidation is verified by 79 unit tests (stubbed barrage) + a fail-loud smoke, but a real multi-model barrage via `stackctl govern --mode spec` hasn't run — the next session's loop-resume IS that live verification.

**Course Corrections:**
- [PROCESS] **Verify a finding's premise against the code before specifying machinery.** The cross-run-reconciliation cascade (AUDIT-31→39→40) was fiction the agent (and its sub-agents) faithfully specified because the findings demanded it. Operator's framing drove the fix: the protocol is per-run; don't write machinery the code doesn't have. → encoded in both skill bodies.
- [PROCESS] **Give sub-agents whole-artifact scope, not one span.** Fencing the FR-007 alignment to single FRs left SC-004/US2-AS3/edge-case contradicting it → AUDIT-41. Operator: "are you giving sub-agents enough context/scope to make consistent updates?" → encoded.
- [COMPLEXITY][PROCESS] **"Why isn't the protocol defined in a single location?"** The convergence gate + slush loop lived only in `govern-spec.sh`; `govern.sh` duplicated the scaffolding and omitted them; `.specify` held a third, stale copy. → consolidated into `stackctl govern`.
- [PROCESS] **Clone/scope-discovery must cover code written outside the implementation stage.** Operator: quick fixes + donkey work need clone detection too. → wired into the session lifecycle (not git hooks, per the enforcement-lives-in-skills ADR).
- [PROCESS] **"Fix the audit protocol so it's reliable"** — not just squeak the spec past the gate; align spec↔code and remove fiction.

**Quantitative** (re-derived from `git log e523fec1..63c4466f`):
- Commits: **19**. Files changed: **28** (+1701 / −505).
- Vitest: **58 → 79** (new: govern payload-implement, payload-spec, protocol, orchestration — both modes + OUTAGE fail-loud). tsc clean.
- 004 findings this session: AUDIT-31..41 dispositioned (fixed / acknowledged-as-fiction); **AUDIT-42 open** (the parked fork). Live multi-model barrages fired: 4 (iterations 4–7, pre-consolidation).
- Clone detector: bash coverage added; **bash clones 0** after the consolidation (was the invisible 78+23-line govern dup); 3 small pre-existing intra-plugin TS clones remain (not introduced this session). Whole-repo 327 → per-codebase 3.
- Sub-agent dispatches: ~11 (per-finding fix dispatches + the consistency sweep + the `stackctl govern` TDD build).
- Corrections: **5** substantive [PROCESS]/[COMPLEXITY] operator redirects.

**Insights:**
- The fresh-context dispatch cured the *verbosity-fatigue* failure (clean minimal fixes, no regression) but **exposed a deeper one**: agents faithfully spec **fiction** that findings demand. Verify-premise-against-code is the missing guard — without it, each fictional mechanism becomes the next round's findings.
- The govern duplication hid for two independent reasons — the clone detector **didn't scan shell** AND **fired from no skill** on the Spec Kit path. Fixing one wouldn't have caught it; both were needed.
- A real **live regression** (stale `.specify` install still on `dw-lifecycle`) hid because no **install-drift** mechanism exists (the installer records a `manifest_hash` nothing ever checks). Captured for a `session-start` advisory check.
- Moving orchestration **out of bash into TS** isn't just single-sourcing — it pulls the logic back under the clone detector's (and the type checker's, and the test suite's) coverage. Structural fix, not just config.

**Next step:** Decide the **AUDIT-42 fork** (A spec-accuracy vs B tighten-predicate), apply it (whole-artifact scope), then **re-barrage through `stackctl govern --mode spec`** — the live end-to-end verification of the consolidated spec path AND the next 004 iteration — and continue toward convergence / recorded `GOVERN_OVERRIDE`. Then: **vendor the clone-detector into stack-control** (`design/migrate-scope-discovery`) for full baseline/NEW-gating + the deferred `govern --mode implement` clone step; **build the install-drift `session-start` check**. `impl/execution-engine` (`specs/002`) stays parked at `/speckit-plan`. Bootstrap with `/session-start`.

## 2026-06-07 (cont.): audit-protocol reliability II — AUDIT-42 fork decided, AUDIT-43 fixed, blast-radius severity calibration (Approach A, design-governed)

### Feature: pluggable-lifecycle-providers → stack-control
### Worktree: stack-control (branch `feature/stack-control`)

> **Backfilled after the fact.** This session was lost to a disk-space cleanup before `/session-end` ran. Reconstructed from commits `84c56988..c1cf8de1` (10 commits, all pushed). During the cleanup the operator renamed the worktree/branch from `pluggable-lifecycle-providers` → `stack-control` / `feature/stack-control` to match the work; the feature-doc `branch:` strings still say `feature/pluggable-lifecycle-providers` (reconcile item, not done this session).

**Goal:** Continue making the audit protocol *reliable* — resolve the two findings the previous session parked (the AUDIT-42 health-predicate fork; the FR-010 graduation contradiction), then answer the standing open protocol question "**what *is* a HIGH?**" that was blocking the 004 convergence loop (an aggressive auditor kept surfacing a HIGH-framed precision nit every round).

**Accomplished:**
- **Decided the AUDIT-42 fork the prior session explicitly parked** (`ce223ce5`) — operator-approved **option 1** (exclude non-zero exit from coverage, but still lift). Split the one health predicate into two: `isModelRunHealthy` = **liftability** (unchanged — what the lift extracts) and a new `isModelRunCovering` = liftability **AND** `exitCode===0` = **coverage** (drives the FR-008 count, the FR-005 zero-coverage OUTAGE, the clean-run claim, the summary line, and the `tip.sha` gate). Closes the hole where a crash-after-banner family masked a zero-coverage outage as a governed-clean run (defeated FR-005/US3/SC-003). `exitCode===0` also excludes timeout(−1)/spawn(−2), closing the FR-008 prose-vs-code gap. RED-first `barrage-coverage-predicate.test.ts` (12). Spec 004 reconciled whole-artifact (FR-002/005/008, US3-AS3, edge cases, Key Entities, SC-003).
- **Fixed AUDIT-43 — FR-010 graduation contradiction** (`aa77929e`, dispositioned `fixed-aa77929e`). FR-010 claimed a spec MAY graduate via two-consecutive-quiet "while open MEDIUM findings remain," but slush (FR-015) runs automatically before the gate on the *same* two-consecutive-0-HIGH predicate, so residual MEDIUMs are already slushed (not open) by graduation. Verify-premise confirmed against `protocol.ts`/`slush-findings.ts`/`slush-remaining.ts`/`spec-governance-gate.ts`. Spec-only alignment, **no code change** — deleted the "while open MEDIUM remain" framing across FR-010/FR-015/SC-007/AUDIT-03/Audit-protocol entity.
- **Blast-radius severity calibration (Approach A) — the main thread** (`eb524c0e..c1cf8de1`). Resolves "what IS a HIGH": severity now rates **downstream blast-radius** — the consequence if a consumer (an adopter running the code, or an *unattended agent* building from a spec with no human to catch a wrong reading) acts on the surface *as written* — **calibrate by consequence, not by alarm**. Five-level vocabulary + lift parser **unchanged**; one criteria location edited in the plugin-default `templates/audit-barrage-prompt.md` (11 lines).
- **Governed the design doc itself before implementing** (`docs/superpowers/specs/2026-06-07-blast-radius-severity-calibration.md`) — a cross-model barrage on the *design doc*, folded across **3 iterations** (iter1 2 HIGH + 6 MED + 2 LOW → iter2 0 HIGH/1 MED/1 LOW → iter3 converged at two-consecutive-0-HIGH, dampener engages). Doc is `accepted`; carries a falsifiable **field-test evaluation contract** with an explicit **Approach-B escalation gate** (findings rated 2+ severity levels apart across models on one run, or cosmetic findings persisting at HIGH without a blast-radius rationale).
- **Promoted 3 deferred scope-discovery items into durable roadmap scope** (`84c56988`, `design/migrate-scope-discovery`): vendor full clone-detector (baselines + dispositions + NEW-gating) + per-codebase default; wire the deferred `govern --mode implement` clone step (TODO in `govern.ts`/`protocol.ts`); install-drift `session-start` advisory check. (Were inbox-parked; one was only a code comment.)

**Didn't Work / unresolved:**
- **The blast-radius rubric is `accepted` but NOT yet field-tested.** Per the design doc, no unit test is possible ("don't test the model's response to a prompt"); the test is **observation across live runs** read as a *trend*, not one N=1 run. No live 004 re-barrage ran this session, so the rubric's first field-test is still pending — it's the next session's first action.
- **No live `stackctl govern --mode spec` run on 004 this session.** The AUDIT-42/43 fixes were verified by unit tests + verify-premise, and the design-doc barrage was live — but the consolidated spec govern path still has not had a real multi-model run since consolidation, and the 004 convergence loop did not advance an iteration. The live open-findings tally is therefore stale (unchanged from the prior session) pending that re-barrage.
- **Suite is 90/91 at backfill (surfaced this session-start, NOT at commit time).** `govern-payload-implement.test.ts > "captures the committed diff against the base"` fails: `expected '' to contain 'export const a = 1;'` — the payload comes back empty. It is a pre-existing git-tmp-fixture test, NOT among this session's 10 changed files; each test in the file runs ~10s and the failing one returned an empty diff while its siblings (untracked-fold, binary-skip, budget, empty-diff) pass. Almost certainly **environmental** (the disk cleanup / git fixture state), but **unverified** — flagged for investigation, not fixed (operator asked for journal backfill only). The commit that last touched this subsystem (`ce223ce5`) legitimately observed 91/91 at 03:09.

**Course Corrections / operator decisions:**
- [DECISION] **AUDIT-42 fork → option 1** (exclude non-zero exit from coverage, still lift). Resolves the fork the prior journal left to the operator.
- [DECISION] **Severity calibration → Approach A only** (rewrite the rubric at the source; field-test in-project; add Approach B = a calibration pass only if the field-test contract fires the B gate). Edit the plugin default (not a project override) since it's a general fix and the feature branch already isolates it from adopters.
- [PROCESS] **Stochastic-correctness over clean single-run signal** (operator): do not freeze the rubric or wait for a fresh artifact to avoid the mid-loop confound — change it now and read the *trend* across many runs; a single high→medium re-rating is noise, a sustained pattern is signal.
- [PROCESS] **Verify-premise before calibrating** (meta-lesson, recorded in the design doc): the design-doc iter-3 HIGH was *false* — the doc quoting the old rubric inflated the rendered-prompt count to 3 (grep = 1). De-quoted at the root so it cannot recur. *Calibration cannot fix a false-premise HIGH; only verify-premise can.* Both AUDIT-42 and AUDIT-43 fixes were themselves driven by verify-premise against the code.

**Quantitative** (re-derived from `git log 0fabe3f9..c1cf8de1`):
- Commits: **10** (`84c56988..c1cf8de1`). Files changed: **10** (+598 / −89).
- Vitest: **79 → 91** (+12, all from `barrage-coverage-predicate.test.ts`). ⚠️ Current re-run at backfill is **90/91** — the `govern-payload-implement` "committed diff against base" failure above (likely environmental; not introduced by this session's diff). tsc clean (per commit; not re-run at backfill).
- Live multi-model barrages fired: **3** (the design-doc calibration barrage, iters 1–3). **No live 004 re-barrage.**
- Findings dispositioned: **AUDIT-42** (`fixed-ce223ce5`), **AUDIT-43** (`fixed-aa77929e`) — both cross-model HIGHs, both verify-premise-confirmed against code. Live 004 open-findings count is unrefreshed pending the next re-barrage.

**Insights:**
- The protocol governed its **own design doc** before the rubric shipped — the same cross-model-barrage discipline turned on a *design artifact*, not just code or a feature spec. The 3-iteration convergence (with a caught false HIGH) is itself evidence the loop works on prose-design.
- The "what is a HIGH" question wasn't a calibration-knob problem — it was a **framing** problem. Re-anchoring severity to *downstream blast-radius (consequence if acted on, unattended)* converts an alarm-driven scale into a consequence-driven one, which is what an unattended build pipeline actually needs.
