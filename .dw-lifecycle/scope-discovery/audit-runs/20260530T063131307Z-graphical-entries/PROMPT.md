...
Phase 3 of graphical-entries: Lane data model + config loader + entry schema delta. Introduces LaneConfig type + Zod schema (id, name, pipelineTemplate, contentDir); loadLaneConfig(id, projectRoot) reads .deskwork/lanes/<id>.json with refusal on missing files (no fallback per project rule); listLaneConfigs(projectRoot) walks the directory. Cross-validation: lane's pipelineTemplate must resolve via the Phase 2 template loader. Extends EntrySidecar with lane: string (required after migration) + artifactKind: 'markdown' | 'html-mockup' | 'single-file-html' | 'image' (required after migration). bootstrapDefaultLaneIfMissing migrates legacy single-site projects (default lane bound to editorial template). detectArtifactKind classifies on-disk artifacts. Schema-broadening cascade through journal events + downstream readers. Audit focus: schema invariants, loader refusals, migration correctness, type-safety, edge cases in artifact detection.

## Commit subjects in the audited range

8fa6477 feat(graphical-entries): Phase 3 — Lane data model + entry schema delta
...
+    lines: 23
     members:
-      - packages/core/src/doctor/repair.ts:22:40
-      - packages/core/src/doctor/validate.ts:170:198
-    disposition: pending
-    reason: null
+      - packages/core/src/doctor/repair.ts:23:45
+      - packages/core/src/doctor/validate.ts:173:205
+    disposition: keep-with-reason
+    reason: pre-existing duplication of artifactPathForStage between doctor/repair.ts and doctor/validate.ts; the file-internal comments document the intentional copy to avoid a cross-import cycle; Phase 4 will replace both with template-driven path resolution
   - id: 2b7d648d9381
     lines: 14
     members:
@@ -236,6 +236,20 @@ clones:
...
diff --git a/packages/core/src/doctor/migrate.ts b/packages/core/src/doctor/migrate.ts
index 3c5a6b2..0f7240a 100644
--- a/packages/core/src/doctor/migrate.ts
+++ b/packages/core/src/doctor/migrate.ts
@@ -215,22 +215,52 @@ export async function migrateCalendar(
...
diff --git a/packages/core/src/doctor/repair.ts b/packages/core/src/doctor/repair.ts
index cf97894..56f802a 100644
--- a/packages/core/src/doctor/repair.ts
+++ b/packages/core/src/doctor/repair.ts
@@ -1,7 +1,7 @@
...
@@ -17,9 +17,10 @@ export interface RepairResult {
...
@@ -34,6 +35,10 @@ function artifactPathForStage(projectRoot: string, slug: string, stage: Stage):
     case 'Blocked':
     case 'Cancelled':
       return null;
+    default:
...
diff --git a/packages/core/src/doctor/validate.ts b/packages/core/src/doctor/validate.ts
index af60d09..6873ccd 100644
--- a/packages/core/src/doctor/validate.ts
+++ b/packages/core/src/doctor/validate.ts
...
+function artifactPathForStage(projectRoot: string, slug: string, stage: string): string | null {
   switch (stage) {
     case 'Ideas':
       return join(projectRoot, 'docs', slug, 'scrapbook', 'idea.md');
@@ -182,6 +185,10 @@ function artifactPathForStage(projectRoot: string, slug: string, stage: Stage):
     case 'Blocked':
     case 'Cancelled':
       return null;
+    default:
...
index 80bac3d..1eeac1f 100644
--- a/packages/core/src/index.ts
+++ b/packages/core/src/index.ts
@@ -26,3 +26,4 @@ export * from './content-index.ts';
...
diff --git a/packages/core/src/iterate/history.ts b/packages/core/src/iterate/history.ts
index 1f16e43..6e89f32 100644
--- a/packages/core/src/iterate/history.ts
+++ b/packages/core/src/iterate/history.ts
@@ -16,12 +16,18 @@
  */
...
diff --git a/packages/core/src/journal/read.ts b/packages/core/src/journal/read.ts
index e94aeb1..defd292 100644
--- a/packages/core/src/journal/read.ts
+++ b/packages/core/src/journal/read.ts
...
+    }
     ...
     events.push(e);
diff --git a/packages/core/src/lanes/bootstrap.ts b/packages/core/src/lanes/bootstrap.ts
new file mode 100644
index 0000000..24e6c9e
--- /dev/null
+++ b/packages/core/src/lanes/bootstrap.ts
@@ -0,0 +1,124 @@
+/**
...
+}
diff --git a/packages/core/src/lanes/detection.ts b/packages/core/src/lanes/detection.ts
new file mode 100644
index 0000000..c80b238
--- /dev/null
+++ b/packages/core/src/lanes/detection.ts
@@ -0,0 +1,78 @@
+/**
...
index 86f4efc..e9069aa 100644
--- a/packages/core/src/pipelines/types.ts
...
@@ -24,13 +79,49 @@ const SUCCESSOR: Record<Stage, Stage | null> = {
   Cancelled: null,
 };
...
+const ArtifactKindEnum = z.enum(['markdown', 'html-mockup', 'single-file-html', 'image']);
 ...
 export const EntrySchema = z.object({
   // Identity
@@ -42,14 +133,29 @@ export const EntrySchema = z.object({
   source: z.string(),
...
diff --git a/packages/core/test/journal/append-read.test.ts b/packages/core/test/journal/append-read.test.ts
index 3168455..3db6062 100644
--- a/packages/core/test/journal/append-read.test.ts
+++ b/packages/core/test/journal/append-read.test.ts
...
diff --git a/packages/core/test/lanes/bootstrap.test.ts b/packages/core/test/lanes/bootstrap.test.ts
new file mode 100644
index 0000000..20ff513
--- /dev/null
+++ b/packages/core/test/lanes/bootstrap.test.ts
@@ -0,0 +1,196 @@
+/**
...
+  });
+});
diff --git a/packages/core/test/lanes/detection.test.ts b/packages/core/test/lanes/detection.test.ts
new file mode 100644
index 0000000..41e3403
--- /dev/null
+++ b/packages/core/test/lanes/detection.test.ts
@@ -0,0 +1,108 @@
+/**
...
+  });
+});
diff --git a/packages/core/test/lanes/types.test.ts b/packages/core/test/lanes/types.test.ts
new file mode 100644
index 0000000..d7b8e4d
--- /dev/null
+++ b/packages/core/test/lanes/types.test.ts
@@ -0,0 +1,116 @@
+/**
...
+      currentStage: '',
       ...
       createdAt: '2026-04-30T10:00:00.000Z',
       updatedAt: '2026-04-30T10:00:00.000Z',
diff --git a/packages/core/test/sidecar/write.test.ts b/packages/core/test/sidecar/write.test.ts
index bc31b91..783a2f7 100644
--- a/packages/core/test/sidecar/write.test.ts
+++ b/packages/core/test/sidecar/write.test.ts
