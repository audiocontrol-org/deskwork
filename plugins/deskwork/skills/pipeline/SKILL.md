...
2. Run the matching subcommand via `deskwork pipeline <verb> [args...]`:
   ...
   - **`delete <id>`** removes a project-override template JSON. Refused for plugin presets (use `customize pipeline <id>` to create an override instead). Refused when any lane has `pipelineTemplate === <id>`, unless `--reassign-lanes-to <other-id>` is passed — in which case every dependent lane is re-bound to `<other-id>` (which must itself resolve) before the doomed JSON is unlinked. Stage compatibility between the old and new template is the operator's problem; doctor surfaces entries whose `currentStage` isn't valid on the new template. Any `<id>-renames.json` migration sidecar at `<projectRoot>/.deskwork/pipelines/migrations/<id>.json` is unlinked alongside the template to keep a future `pipeline create <same-id>` from inheriting a stale audit trail. The dependent-lane rewrites are NOT atomic across lanes: if a lane write fails partway through, re-running the same `delete --reassign-lanes-to <other-id>` is safe — already-rebound lanes are re-written with the same content (idempotent at the data level), and the iteration continues with the remaining lanes.
