...
### `mark-fixed` / `mark-acknowledged` verb — a mechanical writer for the audit-log `fixed-`/`acknowledged-` transitions

- **Surfaced:** 2026-06-08, while answering "how does audit-log status get updated?" (post-#432 gate work).
- **Context:** The audit-log `Status:` line has three writers' worth of transitions but only two are automated: `audit-barrage-lift` stamps `open`, and `slush-findings` flips `open ↔ acknowledged-slush-pile-<date>`. The two most meaningful transitions — `open → fixed-<sha>` (defect actually resolved) and `open → acknowledged-<reason>` (dispositioned false-premise/matches-spec) — have **no verb**; they ride on a hand-edit (Edit/Write tool) by the agent/operator. Failure modes: a fabricated or stale `<sha>`, a forgotten flip (fix lands in code, entry stays `open`), or a typo'd disposition. **Stakes reframed by #432:** the gate no longer reads `Status:` for graduation (it counts raw-surfaced severity), so this is now **record/audit integrity**, not gate correctness — real but lower-stakes than when a forgotten flip blocked graduation via the (now-removed) union.
- **Idea:** A `stackctl mark-fixed --feature <slug> --id <AUDIT-id>` (+ `mark-acknowledged --id … --reason "…"`) verb that locates the finding by ID, **refuses unless it is currently `open`**, defaults `<sha>` to `git rev-parse HEAD` so **the sha is real by construction** (can't stamp a non-existent commit), and writes the line atomically (the same `atomicWriteFile` path `slush-findings` uses). Gives `fixed-`/`acknowledged-` the same mechanical writer `slush` already has — every transition then has a verb, no hand-edit. Fold the call into the govern fix-loop (the agent already dispatches one sub-agent per finding; add "after the fix commits, run `mark-fixed --id <AUDIT-id>`"). **Optional backstop (separate entry-worthy):** a `check-audit-dispositions` reconciliation verb (every `fixed-<sha>` sha exists in history / commit references the finding-ID; flag fixed-in-code-but-still-`open`) fired from a skill body (session-end / govern close), never a git hook (`.claude/rules/enforcement-lives-in-skills.md`).
- **Provisional home:** `multi/migrate-audit-barrage` (the audit-barrage + protocol are mid-migration into stack-control; the finding state machine moves with them). **Triage note:** check whether dw-lifecycle already has an analogous `check-*`/status-set verb to **vendor rather than reinvent** before building.
- **Status:** **captured** (awaiting triage).
