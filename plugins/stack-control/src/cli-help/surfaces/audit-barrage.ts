    ...
    build: () =>
      buildFlatSurfaceCommand({
        ...
        flags: [
          ...
          { name: 'background', description: 'detach the run into its own session and return immediately with a handle (decouples a long govern pass from the foreground Bash-tool timeout); poll --status' },
          { name: 'status', description: 'report a background run and relay its gate verdict (running: exit 75; completed: the govern exit code; crashed: 2)' },
          { name: 'handle', arg: 'id', description: 'target a specific background run for --status (default: newest)' },
