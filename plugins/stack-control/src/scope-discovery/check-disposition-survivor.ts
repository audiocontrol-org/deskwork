/**
 * plugins/stack-control/src/scope-discovery/check-disposition-survivor.ts
 *
 * Pre-commit gate. Fails the commit when any clone-group's disposition
 * reverts from a protected non-pending state (`keep-with-reason`,
 * `refactor`, `ignore-with-justification`) back to `pending` without
 * operator acknowledgment via `--allow-disposition-loss`.
 *
 ...
 *
 * Per `.claude/rules/enforcement-lives-in-skills.md`, this verb is the
 * primitive an adopter can wire into their own pre-commit hook; the
 * plugin does not ship the hook wiring.
 *
 ...
 * `resolveBaselinePath` and expressed repo-relative for `git show`.
 * `--baseline` overrides (repo-relative path). No cwd-relative
 * `.dw-lifecycle` default.
 *
 * Invocation:
 *   stackctl check-disposition-survivor
 *   stackctl check-disposition-survivor --allow-disposition-loss
 *   stackctl check-disposition-survivor --baseline <path>
 *   stackctl check-disposition-survivor --head-ref <ref>
 *
 * Exit codes:
...
export interface Cli {
  ...
  /** `--baseline` override (repo-relative); null = per-codebase default. */
...
function printHelp(): void {
  process.stdout.write(
    [
      'stackctl check-disposition-survivor [options]',
      '',
...
/**
 * Resolve the repo-relative baseline path used both for the `git show`
 * spec and the working-tree read. An explicit `--baseline` is used
 * verbatim (repo-relative); otherwise the per-codebase installation's
 * baseline is resolved and re-expressed relative to --repo.
 */
function resolveBaselineRel(cli: Cli): string {
  if (cli.baseline !== null) return cli.baseline;
  const abs = resolveBaselinePath({ startDir: cli.repoRoot, override: null });
  return relative(cli.repoRoot, abs);
}
 ...
 *   "HEAD:.stack-control/scope-discovery/clones.yaml"  — HEAD's version
 *
...
export async function runCheck(cli: Cli): Promise<number> {
  let baselineRel: string;
  try {
    baselineRel = resolveBaselineRel(cli);
  } catch (err) {
    process.stderr.write(
  ...
  const headSpec = `${cli.headRef}:${baselineRel}`;
  ...
  const absPath = resolve(cli.repoRoot, baselineRel);
  ...
  try {
    ...
    process.stderr.write(
      `check-disposition-survivor: failed to parse HEAD's ${baselineRel}: ${errorMessage(err)}\n`,
    );
    return 2;
  }
  try {
    ...
    process.stderr.write(
      `check-disposition-survivor: failed to parse working-tree ${baselineRel}: ${errorMessage(err)}\n`,
    );
    return 2;
  }
  ...
  reportTransitions(transitions, baselineRel);
  return 1;
}
