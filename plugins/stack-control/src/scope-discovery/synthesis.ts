/**
 * plugins/stack-control/src/scope-discovery/synthesis.ts
 *
...
import type { DiscoveredCandidateCluster } from './discovery-agents/types.js';
import type { ManifestDiscoveredCandidate } from './synthesis-types.js';
...
export async function synthesize(input: SynthesizeOptions): Promise<SynthesisOutput> {
  ...
  // DETERMINISTIC discovered-candidate surfacing (010 decoupling).
  //
  // The dw-lifecycle synthesizer sourced `discovered_candidates:` from
  // an LLM-mediation pass (`mediate()` / `toManifestSection` in
  // `mediation/mediation.js`). That module belongs to the
  // audit-orchestration loop — a SEPARATE feature that is NOT being
  // migrated into stack-control — so the import is severed here.
  //
  // The in-scope deterministic discovery path already produces the
  // novel-shape clusters: `pattern-matrix.ts` runs
  // `clusterUnmatchedShapes()` (synthesis-discovered-candidates.ts) and
  // emits the result on `AstGrepMatrixFindings.discoveredCandidates`.
  // We fold THOSE clusters into the manifest. This preserves FR-017 —
  // a run that found novel/unmatched shapes is NOT reported all-clear,
  // because the deterministic clusters surface in
  // `discovered_candidates:` (and regime_holdouts / codebase_state_metrics
  // independently carry their own candidate signal). No LLM, no mediation.
  const discoveredCandidates = deriveDiscoveredCandidates(partitioned.ast);
...
/**
 * Fold the deterministic unmatched-shape clusters produced by the
 * pattern-matrix agent (`clusterUnmatchedShapes`) into the manifest's
 * `discovered_candidates:` section. Returns null when no AST finding
 * carried any cluster — the section is OPTIONAL in the schema, so it
 * is omitted entirely in that case.
 *
 * Each agent finding may carry its own cluster list; we concatenate
 * across findings and de-duplicate by stable cluster id (two findings
 * from the same run won't normally collide, but the id is stable by
 * member set, so a dedupe keeps the manifest clean if they do).
 */
function deriveDiscoveredCandidates(
  astFindings: ReadonlyArray<AstGrepMatrixFindings>,
): ReadonlyArray<ManifestDiscoveredCandidate> | null {
  const clusters: DiscoveredCandidateCluster[] = [];
  const seen = new Set<string>();
  for (const finding of astFindings) {
    const found = finding.discoveredCandidates;
  ...
  // Stable order: member count desc, then cluster id asc.
  clusters.sort((a, b) => {
  ...
  return clusters.map((c) => ({
    cluster_id: c.id,
    summary: c.shapeSummary,
    member_count: c.memberCount,
    exemplar_files: c.members,
  }));
}
 ...
 * catalog files are present under .stack-control/scope-discovery/ (the
...
async function gatherCodebaseStateMetrics(
  ...
  const configDir = resolve(args.repoRoot, '.stack-control/scope-discovery');
