/**
 * plugins/stack-control/src/scope-discovery/check-anti-patterns.ts
 *
 ...
 * the `stackctl` CLI. Adopters who want a pre-commit gate run the
 * subcommand from a hook that fires on staged .ts/.tsx changes.
 *
 ...
 *
 * Usage:
 *   stackctl check-anti-patterns [--root <path>] [--registry <path>]
 *                                    [--quiet] [--json]
 *
 * Exit codes:
...
const DEFAULT_REGISTRY_NAME = 'anti-patterns.yaml';
...
function printHelp(): void {
  process.stdout.write(
    [
      'stackctl check-anti-patterns [options]',
      '',
      'Options:',
      '  --registry <path>  Override registry path (default: <installation>/.stack-control/scope-discovery/anti-patterns.yaml)',
...
export async function scan(input: ScanInput): Promise<ScanResult> {
  ...
  const scanRootAbs = resolve(input.scanRoot);
  assertCanonicalFilesExist(activeEntries, scanRootAbs);
  const files = await listSourceFiles(scanRootAbs);
  const findings: Finding[] = [];
  for (const file of files) {
    // Path against which `excludes_paths:` entries match: scan-root-
    // relative, POSIX-form. Matches how findings are rendered in the
    // report, so an operator copying a flagged path into
    // `excludes_paths:` works as-is.
    const relPath = toPosix(relative(scanRootAbs, file));
 ...
 *
 * The path resolves against the scan root — matches how
 * `isPathExcluded` compares the canonical to each candidate file
 * (scan-root-relative POSIX), so an entry that passes this check will
...
function assertCanonicalFilesExist(
  ...
  for (const entry of entries) {
    ...
    const abs = resolve(scanRootAbs, entry.canonicalFile);
    if (!existsSync(abs)) {
      throw new Error(
...
export async function main(
  ...
  try {
    ...
    if (!scope.registryExists && scope.registryIsDefault) {
      result = { findings: [], filesScanned: 0, entriesScanned: 0 };
    } else {
  ...
  const out = opts.json
    ? reportJson(result, scanRoot)
    : reportText(result, { quiet: opts.quiet, displayRoot: scanRoot });
