/**
 * plugins/stack-control/src/scope-discovery/check-refactor-preconditions.ts
 *
 * Gate that mechanically enforces the refactor-precondition protocol on
 * commits whose message names one or more clones.yaml entries
 * (`Closes clones.yaml <id>`). Layers four runtime checks on top of the
 * parse-time validateRefactorPreconditions:
 ...
 *   (d) parse-time errors surface verbatim
 *
 * Per `.claude/rules/enforcement-lives-in-skills.md`, this verb is the
 * primitive an adopter can wire into their own commit-msg hook; the
 * plugin does not ship the hook wiring. Silent on commits without a
 * marker.
 *
 ...
 * `resolveBaselinePath`. `--baseline` overrides (resolved relative to
 * --repo). No cwd-relative `.dw-lifecycle` default.
 *
 * Invocation flags:
...
export interface Cli {
  ...
  /** `--baseline` override; null = use the per-codebase default. */
...
async function readCommitMessage(cli: Cli): Promise<string> {
  ...
  // run manually for diagnostic (a commit-msg hook always supplies --commit-msg-file).
...
/**
 * Resolve the absolute baseline path: the `--baseline` override resolved
 * relative to --repo, or the per-codebase default via resolveBaselinePath.
 */
function resolveBaselineAbs(cli: Cli): string {
  if (cli.baselinePath !== null) return resolve(cli.repoRoot, cli.baselinePath);
  return resolveBaselinePath({ startDir: cli.repoRoot, override: null });
}

async function loadBaseline(cli: Cli): Promise<ClonesYaml> {
  const path = resolveBaselineAbs(cli);
  let text: string;
  try {
    ...
    if (isEnoent(e)) {
      throw new Error(
        `clones.yaml baseline not found at ${path}. ` +
          `Refactor commits require a tracked baseline; check the ` +
          `.stack-control/scope-discovery/clones.yaml exists and is committed.`,
      );
    }
    throw e;
  }
    ...
    return [
      preconditionError(
        ...
        `Either remove the marker (this commit does not close a clone) or ` +
          `add the entry to .stack-control/scope-discovery/clones.yaml first. Marker ` +
    ...
    return [
      preconditionError(
        ...
        `Update the entry's disposition to 'refactor' AND supply the ` +
          ...
          `tests_proof — see .stack-control/scope-discovery/README.md §Refactor Preconditions).`,
      ),
    ];
  }
  ...
  if (!parseCheck.ok) {
    return parseCheck.errors.map((detail) =>
      preconditionError(
        ...
        '<schema>',
        detail,
        `Fix the entry in .stack-control/scope-discovery/clones.yaml; see ` +
          `.stack-control/scope-discovery/README.md §Refactor Preconditions for field shapes.`,
      ),
    );
  }
