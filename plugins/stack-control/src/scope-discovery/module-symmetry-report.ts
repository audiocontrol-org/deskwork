/**
 * plugins/stack-control/src/scope-discovery/module-symmetry-report.ts
 *
 ...
 * `.stack-control/scope-discovery/editor-symmetry.md` when `--write` is
 ...
 * `.stack-control/scope-discovery/` (not the legacy
 * `docs/scope-discovery/` location).
 */
export const ARTIFACT_PATH = '.stack-control/scope-discovery/editor-symmetry.md';
  ...
  if (matrix.rows.length === 0) {
    ...
    lines.push(
      'Each refactor commit that PROMOTES a primitive to a shared location ' +
        ...
        'Run `stackctl check-module-symmetry --write` to refresh this ' +
...
function intro(moduleRoot: string): string {
  return (
    ...
    '`.stack-control/scope-discovery/adopter-manifests.yaml`; columns are ' +
      ...
      if (cell.status === 'missing' && cell.expected === 0) {
        ...
        lines.push(
          `- ${glyph} **${module ?? '<unknown>'}**: ` +
            ...
            `(${cell.holdouts} holdout${cell.holdouts === 1 ? '' : 's'}). ` +
            `Run \`stackctl check-adopters\` for the per-file list.`,
        );
      }
    }
    lines.push('');
