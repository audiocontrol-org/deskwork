/**
 * plugins/stack-control/src/scope-discovery/discovery-agents/codebase-state-metrics.ts
 *
...
import {
  computeDiscoveredCandidateRate,
  computeDispositionLatency,
} from './codebase-state-metrics-late.js';

// The input-contract types live in codebase-state-metrics-input.ts
// (010 T034 split). Re-export them here so existing consumers
// (codebase-state-metrics-gather.ts, synthesis.ts) keep their import
// paths (`from './codebase-state-metrics.js'`) unchanged.
export type {
...
} from './codebase-state-metrics-input.js';
import type {
  ComputeInput,
  CatalogEntryObservation,
} from './codebase-state-metrics-input.js';
...
// Late-metric thresholds (discovered-candidate rate + disposition
// latency) live in codebase-state-metrics-late.ts alongside the
// computations that use them (010 T034 split).
...
// Metrics 6 (discovered-candidate rate) + 7 (disposition latency) live
// in codebase-state-metrics-late.ts (010 T034 split). The public entry
// point below calls the imported helpers.
