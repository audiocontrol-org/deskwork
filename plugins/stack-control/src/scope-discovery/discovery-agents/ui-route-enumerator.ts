/**
 * plugins/stack-control/src/scope-discovery/discovery-agents/ui-route-enumerator.ts
 *
 ...
 * `.stack-control/scope-discovery/router-strategies/<id>.ts`) are
 * deferred to a later phase — only the bundled defaults ship today.
 *
 * CLI:
 *   tsx plugins/stack-control/src/scope-discovery/discovery-agents/ui-route-enumerator.ts \
 ...
 *
 * FR-018 — the bundled router-strategy registry is the DEFAULT, not a
 * hard-coded constant: callers (and tests) may pass an explicit
 * `strategies` array to override the detection set. The `scope-inventory`
 * subcommand calls with one argument, so it transparently gets
 * `DEFAULT_STRATEGIES`. A project that ships a custom strategy supplies
 * it here. (Project-config-file strategy loading is a later phase; the
 * override seam itself is the FR-018 contract.)
 */
export async function enumerateUiRoutes(
  input: DiscoveryAgentInput,
  strategies: ReadonlyArray<RouterStrategy> = DEFAULT_STRATEGIES,
