/**
 * plugins/stack-control/src/scope-discovery/discovery-agents/adopter-manifest-checker.ts
 *
 ...
 * wrapper when `.stack-control/scope-discovery/adopter-manifests.yaml`
 * exists. Standalone CLI runs (`stackctl check-adopters`) keep
...
export async function checkAdopterManifests(
  ...
  // Internal library scan — ScanInput is just (registryPath, scanRoot); the
  // CLI-only concerns (quiet/json/gateMode) are not part of the lib contract.
