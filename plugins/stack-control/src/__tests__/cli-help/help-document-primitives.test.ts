// US1 help-probe — document-primitives family (028; FR-001/002; SC-001).
// Every verb (archive / unarchive / curate) and every sub-action emits a
...
import { DOCUMENT_PRIMITIVES_VERBS } from '../../cli-help/surfaces/document-primitives.js';

const surface = buildSurfaceFrom(DOCUMENT_PRIMITIVES_VERBS);

describe('document-primitives --help (028 US1)', () => {
  it('builds the surface (completeness + mediation guards pass)', () => {
    expect(surface.length).toBe(DOCUMENT_PRIMITIVES_VERBS.length);
  });
