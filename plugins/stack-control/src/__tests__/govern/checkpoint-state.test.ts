...
describe('phase checkpoint persistence', () => {
  ...
  it('marks a checkpoint stale when the scope fingerprint changes', () => {
    ...
    expect(isCheckpointFresh(fresh, computeScopeFingerprint(root, ['src/a.ts']))).toBe(true);
    writeFileSync(join(root, 'src', 'a.ts'), 'export const a = 2;\n', 'utf8');
    expect(isCheckpointFresh(fresh, computeScopeFingerprint(root, ['src/a.ts']))).toBe(false);
    rmSync(root, { recursive: true, force: true });
  });
