/**
 * plugins/stack-control/src/__tests__/scope-discovery/batch-dispose.fixtures.ts
 *
 * Shared fixture + IO helpers for `batch-dispose.test.ts` (ported from
 * dw-lifecycle, 010 US2). Extracted so the test file stays under the
 * 300-500 line cap and so future scenarios reuse the same
 * fixture-builder / IO-capture surface.
 *
 * The helpers here intentionally know about:
 *   - OS-tmpdir-scoped fixture directories (per-scenario, per-run id).
 *     The vitest harness uses `mkdtemp` under the OS tmpdir so concurrent
 *     test workers don't collide on shared paths. The fixture clones.yaml
 *     is passed via `--clones` (absolute); batch-dispose's per-codebase
 *     resolver preserves the absolute override.
...
export async function makeFixture(label: string): Promise<Fixture> {
  const dir = await mkdtemp(join(tmpdir(), `sc-batch-dispose-${label}-`));
 ...
 * catches a post-write mutation (the failure mode this exists to
 * prevent).
 */
