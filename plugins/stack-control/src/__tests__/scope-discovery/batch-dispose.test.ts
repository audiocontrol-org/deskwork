/**
 * plugins/stack-control/src/__tests__/scope-discovery/batch-dispose.test.ts
 *
 * Adversarial scenarios for `batch-dispose` (ported from dw-lifecycle,
 * 010 US2). Plants synthetic clones.yaml fixtures, drives the CLI both
 * programmatically (via `runBatchDispose`) and as a subprocess (via the
 * `stackctl batch-dispose` dispatcher), and proves:
 *
...
describe('batch-dispose — core scenarios', () => {
  ...
  it('unknown id fails HARD with exit 2 + no writes + cites refresh-baseline prereq', async () => {
    ...
    try {
      ...
      // The unknown-id error MUST cite the refresh-baseline prereq so the
      // operator's recovery path is discoverable.
      expect(stderr).toContain('stackctl check-clones --refresh-baseline');
...
describe('batch-dispose — subprocess (cli dispatcher)', () => {
  it('boots via `stackctl batch-dispose` and reports Applied:1 + Verified:Y', async () => {
