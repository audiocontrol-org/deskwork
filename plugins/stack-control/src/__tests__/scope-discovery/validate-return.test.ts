/**
 * plugins/stack-control/src/__tests__/scope-discovery/validate-return.test.ts
 *
 * Tests for the `stackctl validate-return` CLI bridge that exposes
...
describe('validate-return CLI assembler', () => {
  ...
  it('project override REJECTS its own phrase', async () => {
    await mkdir(join(tmp, '.stack-control', 'scope-discovery'), { recursive: true });
    await writeFile(
      join(tmp, '.stack-control', 'scope-discovery', 'forbidden-deferral-phrases.yaml'),
      'phrases:\n  - "absolutely-not"\n',
      'utf8',
    );
    const response = [
