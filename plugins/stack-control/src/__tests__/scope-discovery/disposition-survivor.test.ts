/**
 * plugins/stack-control/src/__tests__/scope-discovery/disposition-survivor.test.ts
 *
 * Adversarial scenarios for `check-disposition-survivor` (ported from
 * dw-lifecycle, 010 US2). The gate compares HEAD's version of the
 ...
 *
 * Each scenario plants a tiny git repo in a tmpdir that is ALSO a
 * stack-control installation (carries `.stack-control/config.yaml`), so
 * the gate's per-codebase baseline resolution finds the baseline under
 * `.stack-control/scope-discovery/clones.yaml`. It commits a baseline to
 * HEAD, modifies the working tree, and drives the gate via the cli.ts
 * dispatcher with cwd inside the installation.
 */
...
// Per-codebase baseline path (010 generalization): the gate resolves the
// nearest-enclosing installation's `.stack-control/scope-discovery/clones.yaml`.
const BASELINE_RELPATH = '.stack-control/scope-discovery/clones.yaml';
...
/** Plant a tmpdir with `git init`, a `.stack-control` installation marker,
 * a HEAD-committed baseline, and the baseline path resolved for the
 * working-tree write step. */
async function makeGitFixture(
  label: string,
  initialDoc: ClonesYaml,
): Promise<GitFixture> {
  const dir = await mkdtemp(join(tmpdir(), `sc-dispsurv-${label}-`));
  ...
  // Installation marker so resolveBaselinePath finds this dir.
  await mkdir(join(dir, '.stack-control'), { recursive: true });
  await writeFile(join(dir, '.stack-control', 'config.yaml'), 'version: 1\n', 'utf8');
