/**
 * plugins/stack-control/src/__tests__/scope-discovery/dispose-clone.test.ts
 *
 * Tests for the dispose-clone wrapper (ported from dw-lifecycle, 010 US2).
 * Asserts:
 *
 ...
 *
 * The pass-through scenarios write the fixture clones.yaml under a
 * `.stack-control` installation and pass it via `--clones` (per-codebase
 * model: batch-dispose resolves the override relative to the enclosing
 * installation; the explicit absolute path wins).
 */
...
/**
 * Build a `.stack-control` installation under a tmp dir and place the
 * clones.yaml inside its scope-discovery dir, so batch-dispose's
 * per-codebase baseline resolution accepts the `--clones` override.
 */
async function makeFixture(label: string): Promise<Fixture> {
  const dir = await mkdtemp(join(tmpdir(), `dispose-clone-${label}-`));
  await mkdir(join(dir, '.stack-control', 'scope-discovery'), { recursive: true });
  await writeFile(join(dir, '.stack-control', 'config.yaml'), 'version: 1\n', 'utf8');
  return {
    dir,
    clonesPath: join(dir, '.stack-control', 'scope-discovery', 'clones.yaml'),
    async cleanup() {
