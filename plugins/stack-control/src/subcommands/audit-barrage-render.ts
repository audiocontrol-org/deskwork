/**
 * plugins/stack-control/src/subcommands/audit-barrage-render.ts
 *
 ...
 *        }
 *
 *   2. `stackctl audit-barrage-render --feature <slug> --vars-file <path>
...
const USAGE = [
  'Usage: stackctl audit-barrage-render',
  '    --feature <slug>',
  '    --vars-file <path>',
  '    [--output <path>]',
  '    [--repo-root <path>]',
  ...
  'at .stack-control/audit-barrage-prompt.md takes precedence',
  'over the plugin default), and writes the rendered prompt. The output is',
  'the input file expected by `stackctl audit-barrage --prompt-file`.',
  '',
].join('\n');
