...
> **Read this first.** This is the *why* behind stack-control. Every foundational document (constitution, roadmap, plugin README, the succession rule) points here so that every new developer and every fresh agent session inherits the same grounding. If a design choice or piece of work doesn't trace back to the thesis below, that's the signal to stop and reconsider.
...
- Program structure + work items: [`ROADMAP.md`](./ROADMAP.md) (the heading-keyed program roadmap; the earlier prose `stack-control-roadmap.md` is retired)
...
- Out-of-sequence idea capture: the governed [`DESIGN-INBOX.md`](./DESIGN-INBOX.md) via the native [`/stack-control:inbox`](./skills/inbox/SKILL.md) verb (the interim `design-inbox.md` convention retired when `design/insight-capture` shipped)
