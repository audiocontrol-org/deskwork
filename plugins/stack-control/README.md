...
| `stackctl capability list [--json]` | Agent-facing discovery (026 US2): each capability's interface, mediated identities, and policies, read from the single registry — the API spec IS this listing. |
| `stackctl capability reconcile [--at <dir>] [--json]` | Report-only US3 backstop: flag spec-execution state the graduate gate would refuse (a missing/stale per-phase checkpoint, a non-phased tasks.md). Never mutates. |
| `stackctl mediate-check --surface <bash\|skill> --identity <str> --session <id> [--at] [--json]` | The vendor-neutral mediation decision (026 US1): permit/refuse/usage via exit 0/1/2; refuse names the front door. Pure-read. |
| `stackctl front-door enter --capability <id> --session <id>` / `exit --token <tok> --session <id>` | The session-keyed marker writer the front-door skills bracket their backend drive with, so a sanctioned call is permitted (others are refused). |

## Capability-interface mediation (026)

stack-control is the **agent-facing capability API**: the only sanctioned way to reach a swappable backend is through a capability interface (a `/stack-control:*` skill). A plugin-shipped Claude Code **PreToolUse interceptor** (`hooks/hooks.json` → `bin/intercept`) refuses a RAW fronted-backend call — a direct `backlog …` in Bash, or a raw `/speckit-*` skill — at the point of invocation, naming the interface to use instead. A call made by a front-door skill is permitted because the skill sets a session-keyed marker first (`front-door enter`/`exit`); raw reach-around refused **and** mediated calls permitted is what makes the interface the only path. The v1 capabilities — `backlog`, `spec-definition`, `spec-execution` — and the identities each fronts live in ONE declarative registry (`src/capability/registry.ts`); adding a backend is a registry entry, not new adapter code. The interceptor is best-effort by design (FR-017); the load-bearing guarantee is the per-phase graduate gate (bypassed work cannot graduate — surfaced by `capability reconcile`). Cross-vendor: the decision core branches on capability/identity, never vendor; the Codex adapter (Bash-only) calls the same `mediate-check` verb.
...
**What does NOT belong in the pile — tooling friction routes outward.** The pile is the burn-down queue for work **this installation can fix**. A defect in a tool the project *consumes* — `stackctl` itself, Spec Kit, any upstream — is **not** backlog material: no amount of project work closes it, so each one pollutes the selection queue with unactionable items. Route it outward — [file a GitHub issue on the tool's repo](#reporting-tool-defects-and-friction) (reliably cross-project, reviewable). If friction was already captured locally before it was recognized as upstream, mark the local item `done` with a `filed-upstream` label + the issue URL in its notes — the record survives, the selection queue stays honest. This is the fourth intake decision, really an **anti-intake**: found work that belongs to a *different* project flows out, not in (`import-github` only snapshots this project's own issues inward).
