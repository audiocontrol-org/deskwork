...
4. After the open findings are addressed, **re-run the governance pass** (re-diff → re-barrage) and repeat **until the govern pass reports the gate OPEN (exit 0)** — the FR-010 convergence rule is satisfied. The gate is a single boolean you **obey**, not a verdict you interpret (#432): govern **exit 0 = gate OPEN (done)**, **exit 1 = gate BLOCKED (keep going)**, **exit 2 = fatal**. Once the gate is OPEN, **stop** — do not keep looping, and do not re-derive "done" from finding counts (the gate decides OPEN on what the recent run(s) raw-surfaced, FR-010; the count of still-open findings has no bearing). When several findings stem from one root change, run a consistency sweep over the whole change surface first. Always instruct each sub-agent to **use the Write/Edit tool to persist its changes to disk.**
