...
- analyze-clean: yes
- part-of: multi:feature/lifecycle-industrialization
- depends-on: multi:feature/unskippable-workflow-protocol
- design-approved: yes
...
Stays `planned`: **build a properly-architected, standalone dashboard application to REPLACE the janky in-process prototype.** A **validation build** was merged to main via [PR #529](https://github.com/audiocontrol-org/deskwork/pull/529) (2026-07-20, design record `docs/superpowers/specs/2026-07-19-fleet-dashboard-design.md`) — a plane-embedded, read-only live view built OUTSIDE the governed lifecycle to validate the 036/037 plane end to end. **That throwaway is not the governed feature and its architecture is explicitly REJECTED** (operator decision 2026-07-21): it runs IN-PROCESS with the control plane, yet is a client-side SPA that calls the plane's own authed `/v1/*` API over HTTP — and, having no credential of its own, it **injects a stolen sidecar telemetry token snapshotted once at plane startup** (`src/dashboard/serve.ts`, `[...acceptedTokens.keys()][0]`). So the dashboard authenticates *as a sidecar* to read state that lives in its own process, and its very operation is coupled to whether/when a sidecar connected (blank until an instance enrolls before startup — see backlog TASK-476/477). **The real dashboard is an OUT-OF-PROCESS application** (its own package/repo/deploy, NOT embedded in the plane process) that **consumes the control-plane `/v1/*` API as a first-class external client and authenticates like every other consumer, with its own credential — never a borrowed telemetry token.** This requires a companion API-side piece the plane does not yet have: a **consumer/read credential tier** (distinct from enrollment credentials and per-instance telemetry tokens — the read side currently reuses telemetry tokens, which is the root defect that forced the theft), so an external reader can authenticate to the fleet/instances/runs read + command API. Surface scope: fleet table of commandable runs, tabbed instance drawer, live deltas (SSE), historical views. Re-enters at `design` and runs the full lifecycle; the `2026-07-19-fleet-dashboard-design.md` sketch is superseded on the architecture axis (its plane-embedded / plane-as-only-reader framing is the rejected model), though its surface sketch (columns, drawer tabs, delta-not-full-push) remains a starting point. The in-process validation build under `plugins/stack-control/src/dashboard/` is to be **removed** when this ships. Cut from `design:feature/fleet-control-plane`'s scope by operator decision 2026-07-16; supersedes the removed `multi:feature/control-plane-frontend` (2026-06-07).
