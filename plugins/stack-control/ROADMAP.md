...
- part-of: multi:feature/lifecycle-industrialization
- ref: TASK-242
Give the roadmap CLI a graph-mutation surface for EXISTING nodes so clustering never forces hand-editing the governed ROADMAP.md. Two parts: (1) edge-mutation verbs — add-edge / remove-edge for part-of and depends-on on an existing node (dry-run then --apply, graph-revalidating: refuse cycle/dangling/self/dup, zero-write-on-failure); (2) a one-move roadmap cluster (group) convenience that creates-or-reuses a parent epic + attaches part-of on N existing children (+ optional --chain to wire a depends-on sequence) atomically — the literal 'group the cluster' operation an operator asks for in words. Motivated by the offing dogfood (TASK-242): clustering three existing nodes under a new parent had no CLI path; the agent had to hand-edit ROADMAP.md, contradicting its own 'do not hand-edit' header. BROADER than sibling impl:gap/roadmap-reparent-verb (TASK-137, which only MOVES an existing edge); operator to decide whether to fold reparent into this node or keep them separate. Promoted from TASK-242.
