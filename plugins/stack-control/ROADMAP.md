...
- part-of: multi:feature/lifecycle-industrialization
- ref: TASK-289
Burn down the per-phase governance friction surfaced while implementing 027 — the dominant cost was govern tooling, not the feature code. Headline defects: TASK-289 (O(n^2) shared-file checkpoint staleness — a later phase editing an earlier phase's file re-stales its checkpoint, forcing repeated re-governance and ad-hoc overrides; the structural fix is fingerprinting per-phase HUNKS not whole files, or a govern-at-end mode for shared-file features); the audit-barrage severity NON-DETERMINISM (HIGH oscillated 2->0->2 and LOW->HIGH on identical code, defeating the convergence dampener and forcing overrides on phases 4+6); TASK-263 (per-phase scoping derives the payload from tasks.md backtick paths, so a file split out during implementation — e.g. cluster.ts — is excluded from its own audit, and the no-grounding claude lane then raises FALSE HIGHs it cannot disconfirm). Also fold in the 027 code residuals: TASK-288 (promote the no-grounding claude-lane fix to the shipped default), TASK-290 (test ! hygiene), TASK-291 (roadmap SKILL.md cluster doc), TASK-292 (uniform list-flag stray-comma handling + dead branch), TASK-293 (rewriteEdgeLine fence-awareness). OFFING-TEAM ADOPTER FRICTION folded in (imported from GitHub #487/#488, now closed): TASK-295 (CUSTOMER-BLOCKING — govern's advisory clone step hardcodes --format typescript,tsx, so on a non-TS adopter repo, e.g. offing's Bash/PHP/WordPress change-runbook, jscpd matches zero files and the throw aborts govern BEFORE the language-agnostic cross-model barrage runs, making /stack-control:execute unusable on any non-TS adopter — the clone step must be language-aware or non-fatal when it finds nothing); TASK-294 (tooling-feedback guidance should route adopter friction to GitHub issues against audiocontrol-org/deskwork, not a local tooling-feedback.md the maintainers never see — overlaps TASK-16). Promoted from the 027 govern dogfood + the offing adopter dogfood.
