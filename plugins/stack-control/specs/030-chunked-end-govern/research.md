...
2. **If still oversized — sub-split** the cluster's files (hunk-level within a single oversized file per Edge case "single file larger than the envelope") into envelope-sized sub-chunks; **record a `split-cluster` marker** with the coverage caveat; recover dropped cross-sub-chunk coverage via the chunk-manifest + the seam pass (R7).
