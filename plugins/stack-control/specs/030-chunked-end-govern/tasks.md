...
- [ ] T086 [US9] Decompose `govern.ts` and the `payload-implement.ts` successors under the 500-line cap (completes T064); make T077 pass. (FR-022, SC-007)
  - REMAINING: `subcommands/govern.ts` (1038) + `govern/protocol.ts` (555) still exceed the 500-line cap (the only 2 RED in T077 now; `payload-implement.ts` already removed from the glob by T085). `govern.ts` requires splitting `runGovern` (~611 lines) into arm-modules (implement-arm end-govern drive, spec-arm convergence loop, override short-circuit) threading a shared context, plus extracting the BarrageVars builders (`buildImplementVars`/`buildSpecVars` + helpers) into `govern/govern-vars.ts`. `protocol.ts` needs a ~56-line extraction (e.g. `reportFleetStatus` + fleet helpers into `govern/protocol-fleet.ts`, moving `GovernProtocolError`/`GovernTerminalKind`/`BarrageVars` into a `govern-protocol-types.ts` to avoid a circular import). Pure file-size refactor, no behavior change — verified by the full suite + T077.
