...
- [ ] T086 [US9] Decompose `govern.ts` and the `payload-implement.ts` successors under the 500-line cap (completes T064); make T077 pass. (FR-022, SC-007)
  ...
  - REMAINING (govern.ts): `subcommands/govern.ts` (~1031) is the ONLY T077 RED left. Plan: split `runGovern` (~607 lines) into focused functions — the override short-circuit (~125 lines), the implement-arm end-govern drive (~100), and the spec-arm convergence loop (~110) — each taking an explicit context object for the shared locals (installation, repoRoot, slug, flags, requireModels, laneCapabilities, barrageBin, featureRoot, excludeRoots, excludePaths); plus extract the CLI helpers + BarrageVars builders (`parseFlags`/`pick`/`resolveBarrageBin`/`tail`/`resolveAuditLogExcerpt`/`resolveSpecPath`/`formatScopeExclusionSummary`/`buildImplementVars`/`buildSpecVars`/`resolveGovernExcludePaths`/`resolveGovernFeatureRoot`/`preflightNegotiatedFleet`) into `govern/govern-vars.ts` (repoint the 2 tests that import `buildImplementVars`/`buildSpecVars`/`formatScopeExclusionSummary`). Pure file-size refactor, no behavior change — verified by the full suite + T077.
