...
- **Single file larger than the envelope** (e.g. a large generated or hand-written file whose own diff exceeds the envelope after trim): handled by sub-split at the hunk level within the file, with a `split-cluster`/`split-file` marker; never FATALs.
