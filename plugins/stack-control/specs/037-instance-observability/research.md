...
All five are pinned in `liveness-constants.ts` (or the nearest existing constants seam) with a RED test asserting each value + the derived state boundaries, mirroring 036's constants-pinned-by-tests pattern.
...
**Decision:** add `host`, `path`, `sessionId` to `EventEnvelope`. **`host`/`path` are derived INSIDE `constructEnvelope`, not passed via `EnvelopeInput`** — `constructEnvelope` takes the `installationRoot` and derives `host:path` via `deriveInstanceId` (D8) itself, so **every** event carries identity by construction and FR-011 cannot be violated by a producer that forgets to stamp (analyze finding M2). `sessionId: string | null` IS on `EnvelopeInput` (caller-supplied, nullable — same shape as `runId`): the emit site reads it from the machine-local `current-session` record and passes it (null when no session is open). Add matching `require*` lines in `validateEnvelope` (`event.ts:206-234`). `schemaVersion` increments (additive fields; old events read `sessionId: null`). `bumpSchema` handled per 036's existing schema-version discipline. Do **not** change `callerKind` handling in this feature except where a producer is genuinely a long-run (out of scope here — observability, not the run producer).
...
**Decision:** emit `phase.entered` as a **fail-open side emission at the subcommand level** — immediately after the committed `applyTransition` in `emitAdvance` (guarded on `apply === true`), carrying `{phase: t.to, from: t.from, item: r.item.identifier}` (all in scope) with the wall-clock from a threaded `Clock`. `currentBearing` is **derived** as `{phase, item}` in the registry — so the seam carries **no separate bearing/compass snapshot** (analyze L2), which keeps this hot-ish path free of the compass resolver. This is the single instrumentation seam for the whole design→spec→execute→govern timeline (FR-012). **Do NOT add telemetry to `EFFECT_VERBS`** — the phase emit is a side effect of advancing, not a governed effect; keeping it at the subcommand preserves the transition-engine's narrow Effect/Transition/git-only contract (Principle IV, division of labor). The emit is wrapped fail-open so a telemetry failure never fails a real phase advance.
...
- **New:** `machine-state/instance-id.ts` (host:path deriver), `machine-state/current-session.ts`, `plane/instance-registry.ts`, `plane/http/instance-api.ts`, a `liveness-constants.ts` seam (D1), and a `workflow`/subcommand phase-emit hook (D4).
