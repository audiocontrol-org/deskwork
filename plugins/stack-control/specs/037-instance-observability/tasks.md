...
- [~] T051 [tier:balanced] (RECONCILED — not a 037 bug) Unexercised axes: the runs facet returns `{"runs":[]}` (the dogfood never drives a real `execute`/`govern`, so no run exists) and `liveness`/`connection` are always `live`/`attached` (the dogfood is faster than the 90s/10m windows). Extend the dogfood (or a longer-running variant) to drive a real run and to observe a `live`→`stale`→`gone` transition, so those axes are actually dogfooded. Also confirm `connection` reflects real uplink presence vs. the current liveness-approximation (D1 says they are distinct axes). RECONCILED: `connection`/`liveness` are now real, distinct, heartbeat-driven axes (T050) and idle-stays-live is demonstrated (dogfood S5); the empty runs facet is 036's unwired run-lifecycle producer (TASK-470), OUT of 037's instance-observability scope — the 037 runs facet correctly filters an empty run registry. A full `stale`->`gone` over the 10-min horizon is impractical to dogfood but is unit-tested. No 037 work remains here.
