...
- **FR-029**: The `governing → shipped` exit criterion MUST be decided by the **impl-govern** convergence record (recorded ∧ converged) — required and mechanical; it MUST NOT infer convergence from tasks-completion or any agent assertion. The `specifying → implementing` signal MUST, **by default**, derive from `speckit-analyze`-clean (spec-chain completion), read mechanically from an `analyze-clean:` marker recorded on the roadmap node by the spec chain (a `node-marker` criterion, the same recorded-fact shape as `design-approved:`) — NOT from a subjective judgment at evaluation time and NOT from a persisted analyze-result store; the **spec-govern** convergence record is an **opt-in** stricter gate (used only when the operator runs `govern --mode spec`), never default-required while spec audit-barrage is parked (workflow-policy decision 2026-06-16). The spec-govern gate MUST be re-enableable as default without re-design (FR-028 mechanism retained).
...
- **Analyze-clean signal (resolved 2026-06-16, analyze U1)**: `speckit-analyze` writes no artifact of its own, so the default `specifying → implementing` gate reads a recorded `analyze-clean:` **marker on the roadmap node** — set by the spec chain after a clean analyze, checked by a `node-marker` criterion. This mirrors the `design-approved:` precedent (operator/chain records the fact; the gate checks it) rather than introducing a new analyze-result store. The opt-in spec-govern path instead reads the spec-govern convergence record (`record-converged`).
