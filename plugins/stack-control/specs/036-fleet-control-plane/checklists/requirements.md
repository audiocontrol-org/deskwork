...
**On "scope is clearly bounded".** Bounded, but deliberately **not cut**. Per Constitution Principle II and `.claude/rules/agent-discipline.md` § *Capture mode vs scope mode*, this spec captures everything known or knowably-implied; no "YAGNI" / "deferred" / "not in v1" cuts were inserted. The design record's ~20 open questions are carried forward in full as the **Plan-Time Contracts** section (PT-001…PT-014) — deferred to plan time *by design decision*, to be pinned by RED tests, never dropped. Story priorities P1–P6 denote task ordering within ONE delivery, not shippable slices; this is stated explicitly in the spec because the template's default reading (each story an independent MVP) contradicts the project's no-partial-delivery rule.
