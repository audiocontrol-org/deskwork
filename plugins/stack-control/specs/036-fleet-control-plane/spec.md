...
This feature adds a **control plane**: a network-reachable service that stack-control installations report into from wherever they run, and a dashboard aggregating that state into a live operator view with active control over each run.
...
**Why this priority**: This is the constraint the whole comms architecture exists to satisfy, and the foundation every other story stands on. If emission can tax the CLI, the feature is a net harm regardless of how good the dashboard is.
...
### User Story 2 - See the whole fleet at a glance (Priority: P2)

An operator opens the dashboard and sees every commandable run across every host in one live view — what it is, where it is, what it is doing, how far along, and whether it is healthy — without switching terminal context.

**Why this priority**: This is the feature's headline value and the reason the telemetry exists.

**Independent Test**: Register commandable runs from multiple installations on multiple hosts, confirm each appears as exactly one row with correct attributes, and confirm the view updates live as runs progress.

**Acceptance Scenarios**:

1. **Given** commandable runs are active across several hosts, **When** the operator opens the dashboard, **Then** an initial snapshot renders one row per commandable run, each showing instance, compass, status, progress, model, git, and reconciliation state.
2. **Given** the dashboard is open, **When** a run progresses, **Then** the view updates via a delta describing only what changed — never a full registry push per telemetry event.
3. **Given** short-lived verbs are being invoked constantly, **When** the operator views the fleet table, **Then** those invocations do **not** appear as rows, because the table means "runs you can act on" — while their timing data remains available.
4. **Given** a run is selected, **When** the operator opens its detail drawer, **Then** tabs for overview, artifacts, execution, governance, timings, and reconciliation are populated for that run.
5. **Given** two installations correspond to the same repository, **When** the fleet is displayed, **Then** grouping metadata permits grouping them without that metadata ever being treated as identity.

---

### User Story 3 - Act on a run and always know what happened to the command (Priority: P3)

An operator pauses, resumes, cancels, pushes config to, or requests reconciliation of a run from the dashboard, and can always tell what happened to that command — whether it was accepted, delivered, received, applied, rejected, failed, expired, or superseded. "Sent" is never presented as "applied".

**Why this priority**: Active control is the difference between a dashboard and a control plane. The promise that the operator always knows a command's fate is what makes it trustworthy.
...
1. **Given** a run is executing, **When** the operator issues `pause`, **Then** the command is tracked with a plane-generated identifier and its requested-versus-applied distinction is visible, because pause is cooperative.
...
4. **Given** a command is delivered but never applied, **When** the operator views it, **Then** it is shown as delivered — not complete — because stream replay position is not command status.
5. **Given** a command is never delivered before its expiry, **When** it expires, **Then** it enters a visible terminal expired state that says so loudly rather than vanishing.
...
8. **Given** the operator issues a fleet-wide action, **When** it is applied, **Then** it fans out non-atomically and is presented as such — reporting targets, accepted, and unavailable, with per-instance command state shown — and `Cancel all` requires confirmation.
...
The operator can trust the fleet view's claims. It distinguishes disconnection from death, uplink failure from archive failure, and requested from applied. It never presents a run as healthy when it is uncommandable, and never silently drops history while implying completeness.
...
4. **Given** a run's sidecar connection is lost while the run continues executing, **When** the fleet is viewed, **Then** the run is shown honestly as temporarily uncommandable rather than as healthy.
...
7. **Given** events are retried and may arrive more than once, **When** they are ingested, **Then** duplicates are removed by event identifier, live registry state never regresses to an older sequence, late events are stored durably rather than discarded, and sequence gaps are surfaced diagnostically.
...
### User Story 5 - Read history without amplifying the capped resource (Priority: P5)

The operator browses historical runs and phase timings — how long design, spec, execution, and governance actually took — and those reads are served without amplifying transactions against the read-capped durable store.
...
**Independent Test**: Drive repeated and varied historical reads and confirm capped-store read transactions stay flat rather than scaling with dashboard traffic.

**Acceptance Scenarios**:

1. **Given** the operator requests a historical view, **When** it is served, **Then** it comes from artifacts the plane derived and cached, and the browser never contacts the content-delivery layer.
...
4. **Given** live runs are displayed, **When** their state is read, **Then** it is served from the plane's in-memory registry, because cloud reads are confined to finalized run artifacts.
...
6. **Given** an operator inspects phase durations across the fleet, **When** they view timings, **Then** design, spec, execution, and governance phase durations are available as historical record.
...
- **The sidecar exits while holding an un-flushed spool.** Must not lose spooled telemetry.
...
- **The dashboard offers access to an artifact on a filesystem the browser cannot reach.** A remote dashboard cannot open a local path.
...
- **FR-013**: Only long-running interruptible runs (`execute`, `govern`) MUST register as commandable fleet instances. The fleet table means "runs you can act on".
- **FR-014**: CLI usage and timing data across every verb MUST be available without those invocations appearing as rows in the fleet table.
...
- **FR-030**: The dashboard MUST derive one summary status for display from those separate axes.
...
- **FR-039**: Every event MUST carry `installationSequence` — the sidecar's outbound emission order — used for transport diagnostics, gap detection, and spool restoration.
...
- **FR-042**: The plane MUST deduplicate by `eventId`, MUST NOT regress live registry state from an older sequence, MUST store late events durably rather than discarding them, and MUST surface sequence gaps diagnostically.
...
- **FR-062**: Fleet-wide actions MUST be fan-out, not atomic, and MUST be presented as such — reporting targets, accepted, and unavailable, with per-instance command state shown. `Cancel all` MUST require confirmation.

### Functional Requirements — Durable store and read path

- **FR-063**: Telemetry MUST be stored as immutable per-event objects, laid out as `{bucket}/runs/{installationId}/{runId}/events/{invocationSequence}-{eventId}.json`, plus a finalized `summary.json` written once at run end.
- **FR-064**: The sequence in the object path MUST be `invocationSequence`, named explicitly.
...
- **FR-068**: The plane MUST be the only reader of the delivery layer. The browser MUST NEVER contact it.
- **FR-069**: The plane's history queries MUST be canned and low-cardinality so the edge cache actually hits. Arbitrary caller-driven ranges, filters, and pagination that generate near-unique cache keys are prohibited.
- **FR-070**: There MUST be no browser-held credential for the delivery layer, because there is no browser-to-delivery-layer path.
- **FR-071**: The plane MUST derive dashboard-visible artifacts and cache them to avoid re-reading per request. A cold cache MUST re-read through the delivery layer, which is cached, and MUST NOT touch the capped store.
...
- **FR-080**: The dashboard MUST source everything from the plane — live state and history alike.
- **FR-081**: The dashboard MUST receive initial state via a REST snapshot, then live updates as deltas (instance upserted/removed, command updated, store health). A full registry push per telemetry event is prohibited.
- **FR-082**: The fleet table MUST show one row per commandable run with instance, compass, status, progress, model, git, reconciliation, and actions.
- **FR-083**: The instance detail drawer MUST be tabbed: overview, artifacts, execution, governance, timings, reconciliation.
- **FR-084**: Historical views MUST be served from artifacts the plane derived and cached.
- **FR-085**: Design, spec, execution, and governance phase durations MUST be available as historical record.
- **FR-086**: Roadmap and backlog reconciliation state MUST be surfaced per instance.
...
- **Fleet Instance**: The dashboard-visible projection of a commandable run, carrying separate connection, liveness, and execution status axes plus a derived summary status.
- **Derived Artifact**: A plane-computed, cached, revisioned dashboard view over finalized run data.
...
- **SC-003**: An operator managing runs across multiple hosts sees every commandable run in one view without switching terminal context — 100% of commandable runs appear.
...
- **SC-006**: Every command an operator issues resolves to an observable terminal state, and no command is ever displayed in a state stronger than what actually occurred — 0 overclaims.
...
- **SC-008**: Repeated and varied historical reads do not increase read transactions against the capped durable store — read transactions stay flat as dashboard traffic scales.
...
- **SC-015**: Duplicate and reordered event delivery produces no incorrect registry state — the dashboard never walks backward from a later state to an earlier one.
- **SC-016**: An operator can determine which hop is degraded from the surfaced health state alone, with no ambiguity between uplink and archive failure.
