...
- **Matcher mechanism (CORRECTED — Phase-3 audit claude-01/02)**: an earlier draft of this note claimed the Skill matcher had to be a skill-name regex. **That was wrong** and contradicted both this feature's own contract (`interceptor-hook.md`: `"matcher": "Skill"`) and the decision core (which branches on `tool_name === 'Skill'` and reads `tool_input.skill_name`). The official docs confirm the PreToolUse `matcher` filters on the **TOOL NAME** (documented examples: `Bash`, `Edit|Write`, `mcp__.*`). So the matcher is the constant tool name **`"Skill"`** (parallel to `"Bash"`); a skill-name regex would be matched against the tool name `Skill`, never the skill name, and the hook would never fire — rendering all skill-surface interception inert. ALL skill-name filtering happens in `interceptDecision` (registry-driven), so `hooks.json` carries no skill list at all. This **also** dissolves the FR-011 drift concern (there is no derived list in `hooks.json` to drift — it is a constant; the registry is the single source the decision core reads).
  - **T015/T018 acceptance check (the remaining empirical unknown)**: the docs are SILENT on whether the Skill tool is named `Skill` and whether PreToolUse fires for it at all (research D2 assumes yes; the live spike that would confirm was harness-blocked). So T018 MUST carry an explicit live gate — *the registered `"Skill"` hook actually fires and DENIES a raw `/speckit-implement` end-to-end in a live session, and `tool_input.skill_name` carries the name* — before skill-surface mediation is claimed working. If PreToolUse does not fire for the Skill tool, the spec-* surfaces fall to the Approach C backstop (research D3 contingency).
