...
- [X] T038 OQ-4: add the operator-facing close surface — added `skills/close/SKILL.md` (`/stack-control:close`), a thin operator-facing surface driving `stackctl roadmap advance <id> --to closed` (dry-run cascade plan → operator confirm → `--apply`), with the self-hosting "validate the installed release" prompt at the confirm step (operator judgment, NOT a workflow criterion — install-agnostic, FR-017); skill body + verb, never a git hook (per `enforcement-lives-in-skills.md`). No new fronted op (it drives the existing `roadmap advance` verb): `check-front-door` stays green (65 ops, the +1 vs 64 is the Phase-4 `roadmap/resolves` sub-action); the skill invokes only real verbs so the C2d phantom-op probe is satisfied.
- [X] T039 Ran quickstart scenarios A–H — each is exercised end-to-end against on-disk fixtures (real CLI + real `backlog` binary) by a passing test; the marquee terminal-stage flow was additionally smoke-driven live with `./bin/stackctl` against this repo's installation (status → `next: closed`; compass `--intent close` → on-course; `advance --to closed` dry-run → cascade plan, ROADMAP.md unchanged). Results table recorded in quickstart.md § Validation results. Full suite 2570 passed / 0 failed.
