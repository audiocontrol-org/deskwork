...
- **Alternatives**: `--json` codex stream extractor (**rejected** — a larger parser change with no current need: the calibrated `model_reasoning_summary=detailed` stderr pulses deliver codex liveness today. Re-open only as a fresh design decision if that mechanism fails its named acceptance condition — reasoning-summary pulses stop arriving within the liveness window on a supported model); dropping opus (rejected — composition is a separate calibration-backed decision; opus stays, calibrated, escalate only if it can't meet the envelope).
