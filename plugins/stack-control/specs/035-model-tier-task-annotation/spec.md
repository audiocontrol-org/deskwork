...
- Q: When an installation has no configured `tier_map`, what should the tasks-authoring step do? → A: Print a loud advisory naming the missing `tier_map` and the config path, and emit each task with an explicit unresolved sentinel `[tier:UNSET]` that the existing `resolve-tiers` floor rejects at execute — no invented label, no silent default, generation not blocked. **(analyze F1, 2026-07-08):** in the genuinely-absent-`tier_map` state the floor's *actual* error is `no-map` (resolveTier's check order is no-tier → no-map → unknown-tier), which fires regardless of label; the `[tier:UNSET]` sentinel's job is to make the gap *visible in `tasks.md`*, not to be the thing that trips the refusal. Tests for this path (T005/T013) MUST assert the `no-map` category for the absent-map case and reserve `unknown-tier` for the map-present-but-label-absent case.
