...
### Session 2026-06-11 (clarify — three design forks resolved)

The three scope-shaping forks were resolved by operator delegation (operator: *"do it — your recommendations sound right"*). Each answer is encoded in the named requirement:

- Q (FR-001, the convergence crux): which severity de-inflation mechanism? → A: **(A) cross-lane severity-agreement floor + (C) adjudication layer; (B) deferred.** Cluster severity is computed by ≥2-lane agreement, not max-of-cluster; single-model findings keep their own severity (004 FR-003 preserved); an adjudication pass re-scores the residual single-lane inflations on blast-radius / reachability / fix-debt. Per-lane severities preserved on disk (FR-002).
- Q (FR-007): incremental audit unit granularity? → A: **per-phase (tasks.md phase grouping); whole-feature `after_implement` governance composes from the already-converged phases** (re-audits only changed/cross-cutting code). Per-task and per-commit rejected.
- Q (FR-011): model re-admission bar? → A: **latency-within-derived-timeout + finding-depth + on-task**, admitting sonnet to an **operator-selectable override profile** first (mirroring the fable thoroughness override); default-fleet promotion is a later, separately-recorded decision.
...
- **FR-001** *(severity de-inflation — the convergence crux)*: The system MUST stop a single lane's severity label from inflating a cross-lane cluster's gate-counted severity, so that the convergence dampener's two-consecutive-raw-0-HIGH branch is reachable when the only HIGHs are single-lane inflations. The per-lane severities MUST be preserved on disk (recoverable), not collapsed to a single max at lift. Real cross-lane-agreed HIGHs MUST still block (no suppression of genuine signal). **Resolved (clarify 2026-06-11): mechanism (A) as the mechanical floor + (C) as the richer adjudication layer; (B) deferred.** (A) The cluster's gate-counted severity is computed by **cross-lane severity agreement, not max-of-cluster**: a cluster (≥2 lanes flagged the same root cause) is gate-counted at a severity level only when **≥2 covering lanes rate it at that level or higher** — so a cluster one lane rated HIGH and another rated MEDIUM is gate-counted MEDIUM, while a cluster ≥2 lanes rate HIGH stays HIGH and blocks. A **single-model** finding (only one lane flagged it) keeps that lane's severity unchanged (004 FR-003 preserved — a single-model HIGH still blocks; de-inflation applies ONLY to severity-disagreement *within* a cluster, never to single-model findings, and never below a one-covering-lane quorum). (C) An **adjudication pass** re-scores the residual single-lane inflations that (A) cannot resolve — consistency-seam / fix-code findings — on the finding's own blast-radius prose, reachability through the public path, and fix-debt classification, feeding the gate a calibrated severity recorded with its basis. (B) the gate-side plateau / diminishing-returns detector is **deferred** to a follow-on as a gate-reason telemetry signal, not part of this feature's primary mechanism.
...
- **FR-007** *(incremental audit unit)*: The system MUST support auditing a unit of work smaller than the whole feature, scoping the barrage payload to that unit's diff and audit record, governed by the same convergence protocol (FR-001/FR-004). **Resolved (clarify 2026-06-11): per-phase granularity.** The audit unit is a completed **tasks.md phase** (the natural existing grouping — bounded payload, moderate invocation count, aligned with the spec's own structure; per-task is rejected as too fine — highest invocation count + most cross-task-seam re-audit churn; per-commit is rejected as unaligned with the spec structure and variable in size). Whole-feature `after_implement` governance **composes from the already-converged phase units**: it still runs as the final safety net but does NOT re-audit a phase whose code is unchanged since its unit-audit converged — converged phases are carried; changed and cross-cutting code is re-audited.
...
- **FR-011** *(model fleet re-admission)*: The system MUST support re-evaluating previously-excluded models (sonnet specifically) on the smaller-unit payloads under mechanical read-only, and MUST record the admit/reject decision with its calibration evidence (no silent fleet changes). **Resolved (clarify 2026-06-11): admit to an operator-selectable override profile first, not the default fleet.** sonnet is re-calibrated on a representative per-phase payload under `--permission-mode plan`; the re-admission bar is (1) **latency** within its derived timeout (with the 014 margin) at the target per-phase payload size; (2) **finding depth** — produces verified findings (live-probe behavior), not zero-depth informational output (the haiku exclusion reason); (3) **on-task** — zero mutations (mechanically guaranteed by plan-mode) and no runaway tool-loop. Meeting the bar admits sonnet to an **operator-selectable override profile** (mirroring the existing fable thoroughness override), with the admit/reject decision + evidence recorded. Promotion to the **default** fleet is a later, separately-recorded decision after the override profile proves out. **Recorded outcome (2026-06-12): the live calibration met all three bars (quickstart-results.md § US5), and by operator decision sonnet was PROMOTED to the default fleet** (opus + codex + sonnet) — the broader sample is gathered as live field evaluation across change sets rather than additional pre-promotion calibration.
