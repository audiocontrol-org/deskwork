...
| `liveness_window_seconds` | int > 0 | when signal ≠ `none` | staleness threshold; default 60 in shipped template. REFUSED when signal = `none` — an unmonitored lane has no window to honor (AUDIT-20260611-14) |
...
| Field | Type | Rules |
|---|---|---|
| `terminalState` | `completed` \| `timed-out` \| `spawn-failed` \| `killed-no-liveness` \| `killed-external` | exactly one, set at settle, single source of downstream truth (FR-006); `killed-external` = terminated by a signal the wrapper did not send (AUDIT-20260611-13) |
...
spawn attempt ──spawn error (ENOENT/E2BIG/…)──▶ spawn-failed
   ...
   ▶ running
       ...
       ├─ external signal, no wrapper kill ──▶ killed-external
       ├─ staleness > liveness window ──kill──▶ killed-no-liveness
       └─ budget elapsed ───────────────kill──▶ timed-out
```

Race rule: the first settle wins (existing `finish()` single-settle); a `close` arriving before a kill timer fires records `completed` — unless the close itself carries a non-null signal with no wrapper kill in flight, which records `killed-external` (out-of-band kill: OOM killer, external SIGTERM/SIGKILL; AUDIT-20260611-13). `timed-out` and `killed-no-liveness` are disjoint by construction — the watchdog disarms once the timeout kill begins, and vice versa.
