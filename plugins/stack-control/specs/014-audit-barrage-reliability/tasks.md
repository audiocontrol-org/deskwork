...
- [X] T020 [US3] RED then GREEN: govern convergence-loop fleet status in plugins/stack-control/src/govern/ — round status lines include fleet report; 0-HIGH over a degraded fleet annotated; dampener/models-attempted counters count only `completed` lanes (US3 scenario 3)
