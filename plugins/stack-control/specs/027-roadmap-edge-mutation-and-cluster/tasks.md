...
- [X] T014 [US2] Implement `cluster(parentId, children, {chain, summary, apply})` in `src/roadmap/cluster.ts` (split out of `src/roadmap/mutations.ts` for cohesion, CHK025) composing the `mutations.ts` primitives (`buildSection`/`appendEdge`/`reassemble`) inside one build→revalidate→write (FR-007..015).
