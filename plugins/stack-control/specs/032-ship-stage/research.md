...
- **Decision** (REVISED per analyze F1, operator-confirmed 2026-06-23): Model the post-govern span as ordered phases `governing → merging → validating → closed`. **`shipped` is the recorded STATUS, not a derived phase.** Derive each phase from recorded signals so derived phase == recorded status by construction:
  ...
  - **`validating`** — derived from recorded **status == shipped** (a new `status-is` derive predicate over the recorded status, NOT the old `record-converged impl`). Work = the adopter-defined validation (default: operator-confirm). The `validated` marker is the `validating → closed` GATE, NOT a derive discriminator.
  - **`closed`** — unchanged: by-name terminal (`derive: never`, status == closed).
- **Rationale**: Modeling BOTH a `shipped` and a `validating` phase over the single `status: shipped` (discriminated by the `validated` marker) **deadlocks** — once validated, the derive-walk returns `shipped` while `close` fires from `validating`, leaving `closed` unreachable (analyze finding F1). Collapsing to a single post-merge phase (`validating`) keyed on `status-is shipped` eliminates the deadlock while preserving the operator-approved first-class `validating` waypoint. `graduate` records `status: shipped` at merge (so `shipped` means merged, FR-006) and lands in phase `validating`; every post-merge derivation keys on recorded status (+ the `validated` marker as the close gate) — the exact source the close gate (`roadmap advance --to closed`) reads — making the TASK-445 divergence impossible (FR-008).
- **Alternatives considered**: (a) **Keep both `shipped` + `validating` phases via a distinct `validating` roadmap status** — preserves a 5-phase chain but adds status vocabulary + a second post-merge status write; rejected (heavier; operator chose the collapse). (b) **Keep both phases discriminated by the `validated` marker** — the original encoding; rejected: it deadlocks `closed` (F1). (c) **Keep `record-converged impl` as the post-merge derive** — this IS the TASK-445 bug; rejected (it is what we are removing).
- **New grammar surface**: a derive predicate over the recorded status (`status-is <status>`) added to `DERIVE_KINDS` (with evaluation in `phase-derivation.ts`), plus reuse of the existing generic `approval-marker validated` for the close gate. `phase:shipped` is deleted (clean break). Final encoding pinned RED-first in execute.
