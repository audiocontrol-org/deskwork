...
- roadmap add has no flag for the item's prose body/description. Every roadmap item carries a one-line description, but 'roadmap add' only accepts --status/--scope/--depends-on/--part-of/--deferred-until/--spec/--ref. Adding design:feature/fleet-dashboard required: roadmap add --apply, then a hand-edit of ROADMAP.md to append the prose line, then 'roadmap order' to revalidate. The ROADMAP.md header sanctions this ('For an edit that has no verb yet: edit this file directly, then run stackctl roadmap order'), so it is a known seam -- but the body is not an exotic edge, it is on every node. Suggested fix: --body/--description flag on roadmap add.
- Verb-name discoverability: guessed 'roadmap show <id>' (not a subaction; known set is next/blocked/blocks/order/graph/add/advance/...) and 'backlog add' (the verb is 'backlog capture'). Both failed loud with the known-subaction list, which is the right behavior and recovery was instant -- capturing only as a mild naming-intuition signal (show/add are the conventional verbs), not a defect.
- Vendored speckit git.feature hook warns 'Git repository not detected; skipped branch creation' inside a valid git worktree, where 'git rev-parse --is-inside-work-tree' returns true in the same dir. Harmless here (already on the target branch, so the skip was a no-op) but its branch-creation contract is unreliable for any caller that depends on it -- it fails open with a warning. Suspect it tests for a .git DIRECTORY, which is a FILE in a linked worktree. Captured as TASK-455.

## session-end 2026-07-17
- execute RED-test dispatch: a subagent used a TYPE-ONLY import (import type) of the not-yet-existing module, which esbuild elides and tsc (tests excluded) never checks — the RED test passed false-green. Fix: RED-test briefs must require a VALUE import of the seam so module-not-found actually fails.
- execute RED-test dispatch: a subagent unilaterally added a @/ path alias to tsconfig+vitest.config and created mis-shaped stub src files to make its RED test load — diverging from the 1748:0-relative-import codebase and pre-empting the impl task's surface. Fix: RED-test briefs must forbid build-config edits and forbid creating impl/stub files (RED = module-missing).
- execute impl brief phrasing: telling a subagent to add an 'inline note' for an untested primitive (T088 idle-exit) yielded an untested primitive; the brief must say 'write a test', not 'note', for any primitive with no committed RED test.
