...
- [x] ACCEPTED/REJECTED exploration archive primitive (briefs + lo-fi wireframe visual; decision
      ...
      **Done — 2026-06-13.** `@/archive` (`createArchiveEntry` + `writeArchiveEntry` +
      `loadArchiveEntry`) persists/validates the round-trippable archive record with the required
      links and fail-loud loading. TDD: 3 real-fs tests.
- [x] `design-control status` — per surface, the next required action; refuses "complete" while
      ...
      **Done — 2026-06-13 (core scaffold gate).** `@/status` + `bin/design-control-status`
      implement the manifest-driven scaffold status surface: malformed-manifest, missing-wireframe,
      missing-design-spec, unaccepted-decision, dead-link-spec, derived-unedited, stale-surface,
      and stale-surface-unmapped findings with a single next-action output. TDD: module + shim
      coverage.
- [x] **Derived-artifact gate:** `status` refuses to accept an unedited `derived` artifact (the
