...
# stack-control audit-barrage cross-model at the lint source with the committed
...
# defeat -> lint fix + deterministic vitest fixture + `stackctl scope-widen`.
set -euo pipefail
...
stackctl audit-barrage --feature design-control --prompt-file "$PROMPT" "$@"
