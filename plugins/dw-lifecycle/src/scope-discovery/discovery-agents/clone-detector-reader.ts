/**
 * plugins/dw-lifecycle/src/scope-discovery/discovery-agents/clone-detector-reader.ts
 *
 * Discovery Agent 3 — clone-detector output reader.
 *
 * What it does: reads the committed `.dw-lifecycle/scope-discovery/clones.yaml`
 ...
 * `plugins/dw-lifecycle/src/scope-discovery/clone-detector.ts`. This
 ...
 *
 * CLI:
 *   tsx plugins/dw-lifecycle/src/scope-discovery/discovery-agents/clone-detector-reader.ts \
 ...
 * `.dw-lifecycle/scope-discovery/` per the Phase 2 audit (Finding 03).
 */
  ...
  if (parsed === null) {
    throw new Error(
      `clone-detector baseline at ${baselineAbs} did not parse as a clones.yaml document; ` +
        `the file may be malformed — see plugins/dw-lifecycle/src/scope-discovery/clones-yaml.ts for the contract`,
    );
  }
