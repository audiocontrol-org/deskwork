/**
 * plugins/dw-lifecycle/src/scope-discovery/doctor-rules/legacy-editor-symmetry-field-rename.ts
 *
 ...
 *   - Walks `.dw-lifecycle/scope-discovery/scope-manifest.yaml` if
 ...
 * migrate there.
 *
 * Why detection-only (not `--fix` rewrite): per
 * `plugins/dw-lifecycle/skills/doctor/SKILL.md` § Error handling, the
 * scope-discovery doctor rules are read-only in the current cut. This
...
async function collectScopeManifestPaths(repoRoot: string): Promise<string[]> {
  ...
  const projectManifest = join(
    repoRoot,
    '.dw-lifecycle/scope-discovery/scope-manifest.yaml',
  );
