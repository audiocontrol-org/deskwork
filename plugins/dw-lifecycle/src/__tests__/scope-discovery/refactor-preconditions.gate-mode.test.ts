/**
 * plugins/dw-lifecycle/src/__tests__/scope-discovery/refactor-preconditions.gate-mode.test.ts
 *
 ...
 * code, and the existing Family B tests drive `runGate` directly (which
 * is exit-code-agnostic). This file exercises the full CLI path.
 */
...
async function planFailingFixture(label: string) {
  const dir = await mkdtemp(join(tmpdir(), `dw-rfp-gate-${label}-`));
