...
function mkProject(): string {
  const root = mkdtempSync(join(tmpdir(), 'dw-doctor-schema-stale-'));
  tmpRoots.push(root);
...
function planted(root: string, name: string, body: string): void {
  writeFileSync(join(root, '.dw-lifecycle/scope-discovery', name), body, 'utf8');
}
...
describe('scope-discovery-schema-stale doctor rule', () => {
  it('passes silently when the config dir is absent', async () => {
    const root = mkdtempSync(join(tmpdir(), 'dw-doctor-no-config-'));
    tmpRoots.push(root);
