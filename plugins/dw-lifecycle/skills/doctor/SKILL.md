...
| `agent-prompt-mirror-drift` | The Step 0 fragment embedded in `.claude/agents/code-reviewer.md` and `.claude/agents/codebase-auditor.md` has drifted from the canonical fragment shipped at `plugins/dw-lifecycle/templates/scope-discovery/agent-step-0-fragment.md`. | Either re-run `/dw-lifecycle:install-agent-prompts --force` to regenerate the mirrored fragment, or document the deliberate divergence at `.dw-lifecycle/scope-discovery/agent-prompt-overrides.md` (presence of that file suppresses the finding). |
...
| `hooks-installed-missing` | The `hooks-installed.json` manifest references files that no longer exist on disk (operator deleted a managed file by hand). | Either `/dw-lifecycle:install-scope-discovery-hooks --force` to rewrite the missing files and refresh the manifest, or `/dw-lifecycle:uninstall-scope-discovery-hooks --force-uninstall` for a clean uninstall. |
