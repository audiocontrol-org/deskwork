...
Run `install-scope-discovery` once per adopting project as part of the protocol's onboarding. After this, the operator authors the registries (anti-patterns, adopter-manifests, clones baseline via `dw-lifecycle refresh-clones-baseline`) before wiring the pre-commit hook (`/dw-lifecycle:install-scope-discovery-hooks`) and the Step 0 agent mirrors (`/dw-lifecycle:install-agent-prompts`). The three install verbs are independent — operators can run only the bootstrap if they want the registries without the hook chain.
