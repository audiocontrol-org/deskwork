...
Rewrite `.dw-lifecycle/scope-discovery/clones.yaml` from a fresh jscpd run. The underlying implementation is `dw-lifecycle check-clones --refresh-baseline` (the canonical name; `detect-clones --refresh-baseline` is preserved as a back-compat alias); this skill is a thin wrapper carving the refresh mode into its own verb for operator ergonomics — the `batch-dispose` unknown-id hint cites this skill as the recovery path, and skill prose / hook docs reference it without operators having to remember the longer incantation. Per audit-log AUDIT-20260525-07 ("Heavy" option) + workplan Phase 6 Task 3.
...
The helper:
   ...
   - Dispatches to `checkClones`, which spawns jscpd, parses the output, dedupes groups by content-hashed id, merges dispositions for surviving groups, and writes the rebuilt baseline atomically.
   ...
   - `--gate-mode` is INTENTIONALLY NOT ACCEPTED — refresh is mutating by definition; gating semantics don't apply (use `/dw-lifecycle:check-clones` directly if you want the non-refresh gate behavior).
