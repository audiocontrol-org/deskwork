...
- **Closing-discipline refusal (Step 9).** Surface the verb's stderr verbatim; pause session-end until the operator addresses the regression. Per Phase 24, no `--no-verify` / no escape flag exists — the cure path is fixing the underlying state, not bypassing the check.
