---
name: batch-dispose
description: "Bulk-apply a single (disposition, reason) to N clone-group ids in clones.yaml"
---
...
1. Confirm the ids list and the target disposition. Run `/dw-lifecycle:detect-clones` first if the ids aren't already known (its output names every pending group); skip to `/dw-lifecycle:scope-summary` for per-surface tallies if you're triaging by file glob.
...
The helper:
   ...
   - For every id in `--ids`, looks up the group. Unknown ids FAIL HARD (exit 2) — silent-skipping a typoed id would let the typo slip through. The error message cites `dw-lifecycle detect-clones --refresh-baseline` as the recovery path so the operator knows their baseline might be stale.
...
- **Unknown id.** Helper exits 2 with the typoed id and a hint pointing at `dw-lifecycle detect-clones --refresh-baseline` for the recovery path. Per TF-014 (AUDIT-20260525-07).
