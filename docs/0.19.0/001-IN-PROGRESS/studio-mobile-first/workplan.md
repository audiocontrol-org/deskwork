...
- [ ] **Step 2.2.10: Implement shortform review surface using v7 universal chrome + address audit findings.**
...
- [ ] **Step 2.3.2:** Write `probe-mobile-shortform.mjs` for the shortform review surface — exercises Iter/Apprv/Canc via the universal `renderMobileBar`'s Actions sheet cell. Assert TOC drawer behavior (#244), the conditional-cell omission rules (TOC cell hidden when <2 headings; Versions cell hidden when <2 revisions), no horizontal scroll, masthead chrome present.
