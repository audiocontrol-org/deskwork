...
- [ ] **Step 2.2.7: Implement masthead ⋮ popover menu primitive.** *(Landed 2026-05-12 in `7e03d57` + review fixes in follow-up commit. ARIA role downgraded `menu` → `dialog` per review — settings panel pattern, not menubar. About-modal destination deferred to [#262](https://github.com/audiocontrol-org/deskwork/issues/262); current placeholder links to `/dev/editorial-help`.)*
...
- [ ] **Step 2.2.10: Implement shortform review surface using v7 chrome + address audit findings.**
  ...
  - **Cell-API fit check (flagged by Step 2.2.8 architecture review).** The current `Cell` discriminated-action union (`{kind:'sheet'}` or `{kind:'direct'}`) models BUTTON cells only. Shortform's existing `renderControlsRight()` at `shortform-review.ts:118-166` renders mixed content: verb chips wrapped in `.er-shortcut-chip-wrap`, pending-state `<span>` pills (e.g. `"awaiting apply…"` / `"agent iterating…"`), and per-chip keyboard-shortcut overlays (`<kbd>`). After applying G.1–G.5, decide before coding: (a) the pending pills move OUT of the bar entirely (e.g. up to the masthead's meta-inline slot) — mockup-driven; (b) the `Cell` API gains a third arm `{kind:'info'; cmd?: string}` for non-interactive cells. Pick (a) or (b) at mockup-pick time; don't expand the API ahead of need. If (b) lands, add corresponding test coverage at `packages/studio/test/mobile-bar-smoke.test.ts`.
