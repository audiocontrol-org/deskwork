...
- [x] **Step 2.4-f3 (#200):** Marginalia anchor persistence — replaced the `indexOf`-once-or-null algorithm with the W3C TextQuoteSelector model (`anchor` + new optional `anchorPrefix`/`anchorSuffix` schema fields, ~64 chars per side; rebase scores each candidate by character-boundary match against the surrounding context). Operator's reported failure modes ("re-anchored to wrong text", "anchor disappeared though text is present") are addressed. Phase 2 (diff-match-patch fuzzy fallback for minor edits to the anchor text itself) tracked as [#271](https://github.com/audiocontrol-org/deskwork/issues/271). Landed in <commit-pending>.
