...
| 2.2 | Shortform desk mobile-first → re-scoped 2026-05-12 to **v7 cross-cutting architecture** (Desk-as-hub star nav, masthead ←/⋮, zero-nav-region bar, Desk absorbs Shortform-by-platform section) | Steps 2.2.1–2.2.9 complete (v7 architecture landed: standards + archive, `renderMasthead`, ⋮ popover, universal `renderMobileBar`, Desk Shortform + Adjacent sections — all under operator review). Step 2.2.10 (shortform-review surface refactor + G.1–G.6 audit findings) pending; gated on operator pick of Shortform-1/2/3 contextual idiom. |
