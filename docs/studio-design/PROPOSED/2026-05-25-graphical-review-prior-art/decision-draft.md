...
| **CSS `flex`** | **Faithful.** The aside + svg-icon flex pair renders with the documented sub-pixel positioning. | Multi-line aside paragraph reports a positive `heightPx` matching the live DOM (~65px at Georgia 16px / 1.5 line-height on the probe machine; absolute value depends on font metrics — the probe asserts `heightPx > 0` and live-vs-captured equality, not the literal pixel value). |
...
| **Multi-line text wrapping** | **Faithful.** The aside paragraph wraps at the same column width as the live DOM (verified by `heightPx` being identical between live and captured). | Probe records `heightPx > 0` and equal between live and captured surfaces; absolute value is font-metrics-dependent (~65px on the probe machine but not asserted as a literal). |
