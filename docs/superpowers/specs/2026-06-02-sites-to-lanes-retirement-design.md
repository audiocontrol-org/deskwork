...
**Decision: workflows are scoped to the entry (`entryId`), not the site; but the *full dedup key* keeps a kind/channel discriminator.** Resolution:
...
- **An entry owns MULTIPLE workflows simultaneously** — one longform review plus one shortform workflow per platform (`/deskwork:shortform-start` creates a per-platform LinkedIn/Reddit/YouTube/Instagram workflow each). So `entryId` alone is NOT a unique dedup key — it is the **grouping axis**. The old `(site, slug)` key was effectively `(site, entry)`; the kind/channel discriminator was already implicit because longform vs shortform took different code paths. Under the entry-keyed model the dedup key MUST carry that discriminator explicitly: **`(entryId, kind, channel?)`** — `kind` ∈ {longform, shortform}, `channel` the platform for shortform. Equivalently, match on the **workflow's own `id`** when one is known (the `render.ts:75` `entryId` path), with `(entryId, kind, channel?)` as the lookup-by-attributes key.
- The `(site, slug)` match expressions (`pipeline.ts:147`, `start-handlers.ts:92,227`) switch to this composite key — NOT to bare `entryId` equality. A literal `.find(w => w.entryId === entryId)` would return the first co-resident workflow and silently shadow the others; the implementer MUST preserve the kind/channel discriminator (AUDIT-BARRAGE-claude-03, 2026-06-04).
...
- This is forced by the model (no fork on the *site* axis): removing `sites` leaves no site axis, and the entry is the only stable identity. The lane is the natural successor to `site` for *grouping* (not resolution). The kind/channel discriminator is orthogonal to the site retirement — it already existed implicitly and must survive the rekey.
...
| 22 | Review workflows are scoped to `entryId` (grouping axis), not `(site, slug)`; the full dedup key is `(entryId, kind, channel?)` — the kind/channel discriminator survives the rekey (an entry owns longform + per-platform shortform workflows); `site` validation drops; `workflow.site` → `workflow.lane` for report breakdowns | Forced by the model on the site axis; the kind/channel discriminator is orthogonal and must NOT be dropped (AUDIT-BARRAGE-claude-03). |
