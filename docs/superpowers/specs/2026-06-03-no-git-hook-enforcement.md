...
## Amendment (2026-06-18, specs/026 Decision 5): plugin-shipped PreToolUse hooks ARE permitted

The "no git-hook enforcement" principle is about what TRAVELS WITH `claude plugin install`, not about the word "hook". A **git hook** (`.husky/`, `.git/hooks/`) the adopter must discover and wire remains forbidden. A **plugin-shipped Claude Code hook** — `plugins/stack-control/hooks/hooks.json` (auto-discovered on install) → `bin/intercept` — is PERMITTED, because the adopter gets it from `claude plugin install` with no separate wiring, and it contains no decision logic (it is a thin adapter over the vendor-neutral `stackctl mediate-check`/`intercept` verb; the decision lives in the `capability/` core). The 026 capability-interceptor is exactly this: a permitted plugin-shipped enforcement surface. Its guarantee is still defense-in-depth — the load-bearing backstop is the US3 per-phase graduate gate (CLI verb + gate), not the best-effort interceptor (FR-014/FR-017). See `.claude/rules/enforcement-lives-in-skills.md` § "026 amendment".
