...
## 2026-06-09 — audit-barrage lift (20260609T032925208Z-pluggable-lifecycle-providers-after_clarify)

### AUDIT-20260609-04 — atomicReplace replaces the target inode — file mode and symlink identity are silently not preserved

Finding-ID: AUDIT-20260609-04
Status:     fixed-9293592f
Disposition: FIXED (TDD-first, commit 9293592f). `atomicReplace` now preserves identity: a symlink `docPath` is resolved via `realpathSync` (the link survives and keeps pointing at the now-updated real file); the existing target's permission mode is captured (`statSync`) and applied to the temp (`chmodSync`) before the rename. `commit-atomic.test.ts` gained explicit mode-preservation + symlink-preservation assertions.
Severity:   low
Surface:    plugins/stack-control/src/document-model/mutations-core.ts:25-33 (`atomicReplace`)

The fix for `AUDIT-20260609-02` correctly replaced the in-place `writeFileSync(docPath, …)` with a same-directory temp-write + `renameSync` to get a torn-write guarantee. But temp-then-rename has a well-known side effect the in-place write did not have: it points the path at a **brand-new inode**, so the live document inherits the *temp file's* attributes, not the original's. `writeFileSync(tempPath, contents, 'utf8')` (line 28) creates the temp with the default mode (`0o666 & ~umask`), and `renameSync` (line 29) then makes that the document's permanent mode. Any non-default mode on the original `DESIGN-INBOX.md`/`ROADMAP.md` (restricted read, a deliberately tightened group perm) is silently reset on the **first** mutation. Likewise, if the governed document is a **symlink** (e.g. an operator points their inbox at a shared location), the old in-place write followed the link; the new rename **replaces the symlink with a regular file**, severing the link without warning. This now affects both inbox and roadmap, since they share this substrate.

Blast radius is low: governed markdown documents rarely carry meaningful permission bits, and symlinked governance docs are uncommon — the default doc is a plain in-tree file. But the behavior changed silently in this diff, the test (`commit-atomic.test.ts`) asserts the *inode changed* (the very mechanism that drops the mode/symlink) without asserting either attribute is preserved, and the SKILL/README repeatedly promise the mutation is fail-safe against the live document — which a reader could reasonably read as "leaves my file as it was, only with new contents." A reasonable fix: `fstat`/`statSync` the original before the rename and `chmodSync` the temp to match (and optionally `realpathSync` to write through a symlink), or document that atomic replacement intentionally normalizes mode and de-symlinks. At minimum, add a preservation assertion to the atomic test so this stays an explicit decision rather than an accident.

---

### AUDIT-20260609-05 — Shared flag scanner refuses legitimate free-text values that begin with `--`, with a misleading "required" error

Finding-ID: AUDIT-20260609-05 (claude-02 + codex-02; cross-model)
Status:     fixed-9293592f
Disposition: FIXED (TDD-first, commit 9293592f). `scanVerbFlags`'s generic value-flag branch now accepts a `--`-prefixed free-text value; it fails usage only when there is no next token OR the next token is a RECOGNIZED boolean flag for the verb (the operator forgot the value — this preserves the `defer --until --apply` guard, AUDIT-20260608-04). The dedicated `--doc` branch keeps its stricter guard. RED→GREEN tests added for inbox (`--idea "--apply is not a flag here"`) and roadmap (`--scope "--something"`).
Severity:   medium
Surface:    plugins/stack-control/src/subcommands/document-verb-shared.ts:69-72 (`scanVerbFlags` value-flag branch); reached via inbox `--idea`/`--context`/`--surfaced`/`--reason`/`--to`

In `scanVerbFlags`, a recognized `--<name> <value>` consumes the next token as its value but rejects it when the value itself starts with `--`: `if (v === undefined || v.startsWith('--')) failUsage(verb, \`${token} <value> required\`)`. For roadmap this was acceptable — its value-flags (`--to <identifier>`, `--until <date>`, `--ref`) carry structured tokens that don't begin with `--`. This diff extends the same scanner to the inbox, whose `--idea`, `--context`, `--surfaced`, and `--reason` are explicitly **free prose** (the SKILL calls them "the idea", "background", "why"). A natural capture in a tool *about its own tooling* — `inbox capture "x" --idea "--apply should default to true"` or `inbox drop "x" --reason "--dry-run made this redundant"` — has a value beginning with `--` and is refused.

Blast radius is low and there is no data risk: the refusal is fail-loud and zero-write (the inbox is untouched). The cost is a usability trap in exactly the one-move, low-friction flow this feature exists to provide, made worse because the error message is actively misleading — it says `capture --idea <value> required` when the operator *did* supply an idea; the real cause (value can't start with `--`) is never surfaced, so the operator can't tell what to fix without reading source. A reasonable fix: distinguish "next token is missing" from "next token looks like a flag" and either accept the value after the first value-flag is seen (free-text fields should take the literal next token), or emit a specific message (`--idea value cannot begin with '--'; quote it or use --idea=…`) and support the `--name=value` form for these fields.

---

I checked and found clean: the now-anchored `INBOX_STATUS_LINE` (`mutations.ts:32`) correctly avoids the body-prose collision the prior barrage fixed — the idea/context fields render as `- **Idea:** …`, so a leading `**Status:**` token never appears and `transition` rewrites the real status bullet (the new `mutations-promote-drop.test.ts` anchoring test exercises this); the `captured`-only terminal gate in `transition` (`mutations.ts:107-111`) and the record-line splice-after-status placement are sound given `buildCaptureSection` emits status last; `validateFlags` rejects unknown flags / extra positionals / `--apply`-on-`list` before any mutation; the roadmap refactor preserves identical behavior while gaining atomic writes; and the `assertSingleLine` newline guard plus whole-document `commitCandidate` re-validation give a real zero-write-on-failure contract. The tasks.md/checklist/README/SKILL edits track the implemented behavior. If the two findings above had instead been an unanchored locator or a swallowed validation error, I'd have rated them high — they're low because each is fail-loud with no silent data loss.

### AUDIT-20260609-06 — Default inbox path points at the plugin bundle, not the project inbox

Finding-ID: AUDIT-20260609-06
Status:     dispositioned-document-and-track-9293592f
Disposition: DISPOSITIONED — document + track (operator decision, 2026-06-09; commit 9293592f). The plugin-bundled default `--doc` is a pre-existing, FAMILY-WIDE shape (`roadmap`'s default resolves identically) and is correct for this monorepo's own dogfood; the adopter scenario depends on project-relative governed-doc discovery that does not yet exist (adopter packaging is tracked under `migrate-scope-discovery`). Operator chose to keep the behavior (no divergence from the shipping roadmap verb) and instead: (1) corrected the contract + SKILL wording to accurately describe the plugin-bundled default and require adopters to pass `--doc` explicitly until discovery lands; (2) added the tracked governed item `design:gap/project-relative-doc-discovery` (part-of `design:feature/migrate-scope-discovery`) for the real cwd/config-relative resolution fix across the whole verb family. The contract/skill/code now agree (the finding's actual ask).
Severity:   high
Surface:    plugins/stack-control/src/subcommands/inbox.ts:21-23; specs/007-insight-capture/contracts/inbox-cli.md:3-5; plugins/stack-control/skills/inbox/SKILL.md:8
...
## 2026-06-09 — audit-barrage lift (20260609T042810837Z-pluggable-lifecycle-providers-after_clarify)

### AUDIT-20260609-16 — README omits the adopter "pass --doc explicitly" caveat that AUDIT-06 added to the contract, SKILL, and code comment — README-only adopters re-hit the wrong-document-write trap

Finding-ID: AUDIT-20260609-16
Status:     fixed-b748c08d
Disposition: FIXED (commit b748c08d). Completion gap in the AUDIT-06 disposition: the README (the canonical adopter-facing doc) was omitted when the contract/SKILL/code-comment were aligned on the plugin-bundled-default caveat. Added the same one-sentence caveat to the README "Insight capture" section (default is the plugin-bundled inbox; adopters MUST pass --doc or set STACKCTL_INBOX_DEFAULT_DOC until design:gap/project-relative-doc-discovery lands). README + contract + SKILL + code now all agree.
Severity:   medium
Surface:    plugins/stack-control/README.md (new "Insight capture" section + the four `inbox` table rows); cf. specs/007-insight-capture/contracts/inbox-cli.md:5, plugins/stack-control/skills/inbox/SKILL.md:9-11, plugins/stack-control/src/subcommands/inbox.ts:25-31

AUDIT-20260609-06/-10's disposition was explicit: keep the plugin-bundled `DEFAULT_DOC` behavior but make the adopter-facing surfaces *agree* that the default targets this monorepo's own inbox and that **adopters MUST pass `--doc` explicitly** until `design:gap/project-relative-doc-discovery` lands. Rounds 2–3 aligned three surfaces — the contract (`inbox-cli.md:5`: "Adopters operating on their own inbox MUST pass `--doc <path>` explicitly"), the SKILL (`SKILL.md`: the "Which inbox `--doc` targets" paragraph), and the `inbox.ts` `DEFAULT_DOC` comment. The README, which is **also in this diff** and is the canonical adopter-facing doc per `.claude/CLAUDE.md` ("follow each plugin's own README — that's the canonical adopter-facing install path"), was not given the same caveat. The new `## Insight capture` section says capture operates "against the governed [`DESIGN-INBOX.md`](./DESIGN-INBOX.md)" and the table row says "into the governed inbox in one move" — neither states that an adopter running without `--doc` writes to the *plugin's* inbox rather than their own.

The blast radius is exactly the wrong-document-write that AUDIT-06 rated HIGH, reachable by a reader of the most adopter-facing surface. An adopter who installs the plugin, opens the README (not the spec contract buried under `specs/007-insight-capture/`), and runs the documented `stackctl inbox capture "<title>" --idea "<text>" --apply` silently mutates `plugins/stack-control/DESIGN-INBOX.md` instead of their project inbox — the "one source of truth" promise broken in the wrong direction. The AUDIT-06 fix's stated goal was "the contract, skill, and code now agree" — the README was simply omitted from that list and so still under-specifies. A reasonable fix mirrors the one-sentence caveat already in the contract into the README's Insight-capture section (and ideally the table): "when `--doc` is omitted the default is the plugin-bundled inbox; adopters MUST pass `--doc` until project-relative discovery (`design:gap/project-relative-doc-discovery`) lands."

### AUDIT-20260609-17 — atomicReplace now requires write permission on the *containing directory* (a behavior change vs the prior in-place write, and it silently extended to the roadmap verb)

Finding-ID: AUDIT-20260609-17
Status:     fixed-b748c08d
Disposition: FIXED (commit b748c08d). Documented the trade-off the auditor asked be made explicit: atomicReplace's temp-then-rename requires WRITE permission on the parent DIRECTORY (stricter than the prior in-place write), a deliberate exchange for atomicity; a writable doc in a read-only dir now fails loud (EACCES/EROFS, zero write). Added the precondition to the atomicReplace doc-comment. No behavior change beyond what AUDIT-02 already introduced.
Severity:   low
Surface:    plugins/stack-control/src/document-model/mutations-core.ts:46-62 (`atomicReplace`); reached by both src/inbox/mutations.ts and src/roadmap/mutations.ts via `commitCandidate`
