...
> **F1 (front door) → design-phase block [F8 low-friction insight capture → F9 govern-the-spec → F6 *design surfaces*] → F2 parallel execution engine → F3/F4/F5 migrations → F7 retire `dw-lifecycle`.**

Notes: (1) **F1's thin skills-over-CLI architecture is unchanged** — the design surfaces are built *on* the thin front door, not a pivot to a separate web/TUI app (operator confirmed 2026-06-06). (2) **F6 splits by phase:** its *design* surfaces (spec → implementation negotiation, scope-discovery + audit-barrage surfaces, the capture/govern surfaces from F8/F9) are in the lead block; its *engine-run* surfaces remain **after F2** (they need the engine to exist). (3) **F2's spec is hardened + parked** at the `/speckit-plan` step; resuming it is deferred behind the design-phase block. Internal ordering within the design block is downstream and refinable when reached.
...
| 2 | **Parallel multi-backend execution engine** | The differentiator: worktree-isolated, cross-backend fan-out across distinct coding agents, capability-selected (survives batch/headless CLI sunset). The current `specs/002-parallel-execution-engine/`. Built *through* the front door. | **Deferred behind the design-phase block** (resequence 2026-06-06). Spec hardened + parked at `/speckit-plan` (0 NEEDS CLARIFICATION; FR-001..FR-032a; SC-001..SC-015). |
