...
| 23 | Source-shipped re-architecture (drop the bundles) | **Implementation complete on main; v0.9.0 release deferred pending blocker fixes.** Eight commits this session (`62c000a`, `bbbec30`, `8e0d851`, `b619ecd`, `196a5a4`, `23b4032`, `6dd0052`+`bf12db6`, `096b184`). All 9 sub-phases (23a–23i) implemented end-to-end: vendor symlinks via `scripts/materialize-vendor.sh` at release time; bin/ wrappers do first-run `npm install`; on-startup esbuild for client assets into `<pluginRoot>/.runtime-cache/dist/`; override resolver + `/deskwork:customize` skill; bundles + committed `public/dist/` retired; `scripts/smoke-marketplace.sh` is the new pre-tag gate per `RELEASING.md`; `MIGRATING.md` documents the upgrade for existing adopters. 680 workspace tests green (339 + 147 + 194). Senior code review surfaced 4 blockers ([#76](https://github.com/audiocontrol-org/deskwork/issues/76) bin race, [#77](https://github.com/audiocontrol-org/deskwork/issues/77) esbuild concurrent-boot race, [#78](https://github.com/audiocontrol-org/deskwork/issues/78) materialize-vendor mode/symlink, [#79](https://github.com/audiocontrol-org/deskwork/issues/79) smoke-test signal handling) and 5 polish follow-ups ([#80](https://github.com/audiocontrol-org/deskwork/issues/80)). v0.9.0 release ceremony will run after blockers ship. |
