...
## 2026-06-04 — audit-barrage lift (20260604T211015200Z-deskwork-plugin)

> **Addendum 2026-06-04 (slushed, but the cross-model finding is self-caused in the same-turn commit `361085fc` and touches the project's no-silent-fallback rule, so addressed in `<next-sha>`):**
> - **AUDIT-17 (MED, cross-model claude-01 + codex-01) — FIXED:** the best-effort `catch {}` introduced in `361085fc` was overbroad — it swallowed the common missing-lane-file case too, with no signal. Narrowed: the missing-file case is now an explicit `existsSync(laneConfigPath(...))` skip (catch-free); the residual try/catch is scoped to ONLY the present-but-won't-load shapes (unresolvable pipeline / corrupt JSON / schema-invalid), each of which means "lane unusable → skip the optional redirect"; the broken lane is surfaced by the lane-config doctor rules. The caught `err` is bound (not silently dropped) for attributability. Still pre-mutation → no partial-apply.
> - **AUDIT-18 (LOW) — OBSOLETE-by-design:** the pre-mutation-ordering regression-lock the corrupt-JSON throw-test provided is no longer meaningful because lane resolution is now NON-throwing (best-effort skip). With no throw, a partial-apply from this path is structurally impossible; the ghost / purged-pipeline / corrupt-JSON tests all assert the rename COMPLETES and skips the redirect, which covers the live contract. No test re-added because there is no throw left to order.

### AUDIT-20260604-17 — Broad `catch {}` in `renameSlug` silently drops an operator-configured 301 redirect and swallows unrelated failures, with zero signal in the result

Finding-ID: AUDIT-20260604-17 (claude-01 + codex-01; cross-model)
Status:     acknowledged-slush-pile-2026-06-04
Severity:   medium
Surface:    `packages/core/src/rename-slug.ts:202-209` (the `try { redirectsPath = loadLaneConfig(...).redirectsPath } catch { redirectsPath = undefined }` block) + the skip at `:276`

The AUDIT-13 fix correctly stopped `renameSlug` from *hard-failing* on a purged-pipeline lane, but the unbound `catch {}` it introduced is broader than the stated intent ("resolving `redirectsPath` is best-effort website metadata") and produces a silent data-loss path the disposition never examined. `loadLaneConfig` cross-validates the lane's `pipelineTemplate` (loader.ts:190-198) *before* returning the lane, so reading `.redirectsPath` requires the **entire** lane — including a field unrelated to redirects — to resolve. A lane that genuinely publishes a website and carries a valid `redirectsPath` will silently lose its 301 append on rename whenever its `pipelineTemplate` is transiently unresolvable (operator mid-edit of pipelines, a renamed preset id, a deleted template). The redirect value was right there on disk and valid in isolation; it is discarded only because an orthogonal field didn't resolve. For the website use-case this is a broken-link / SEO regression discovered only downstream.

Compounding it: there is **no operator-visible signal**. `renameSlug` returns `actions` (rename, calendar-slug-change, distribution-sync, redirect-append), but the `catch` branch pushes nothing — no `redirect-skipped` action, no warning. The operator sees a clean successful rename with a silently-missing redirect. The bare `catch` also swallows classes that are *not* "lane is gone": `assertSafeLaneId`'s path-traversal refusal (loader.ts:67-81) and genuine I/O faults like `EACCES`/`EMFILE` on the lane file — all conflated with the benign "purged lane" case. This is exactly the swallowed-exception / fallback-hides-failure bug-factory the project guidelines warn against. A narrower fix: resolve `redirectsPath` via a reader that validates only the lane schema (or pulls the field) without cross-validating the pipeline template, and when resolution genuinely fails, push a `redirect-skipped` action recording the reason so the drop is visible rather than silent.

### AUDIT-20260604-18 — Converting the corrupt-JSON throw-test to a "completes + skips" test removed the only regression-lock for AUDIT-08's pre-mutation-ordering invariant

Finding-ID: AUDIT-20260604-18
Status:     acknowledged-slush-pile-2026-06-04
Severity:   low
Surface:    `packages/core/test/rename-slug.test.ts:312-365` (the rewritten corrupt-JSON case) vs. the deleted assertions (`expect(...).toThrow()` + "artifact and calendar are untouched")

The pre-change test pinned AUDIT-08's core fix: a present-but-corrupt lane caused `renameSlug` to throw **before** mutating disk/calendar (`expect(existsSync('docs/old-post/index.md')).toBe(true)`, `expect(existsSync('docs/new-post/index.md')).toBe(false)`, calendar slug still `old-post`). That assertion was the de-facto regression-lock for the "resolve BEFORE mutation, no partial-apply" property — the property the retained code comment (rename-slug.ts:184-201) still asserts is the whole point of the placement. The rewrite deletes that test and replaces it with "rename completes + redirect skipped," which verifies the new best-effort behavior but no longer pins the *ordering* invariant at all.

Today this is moot because the `catch` makes resolution non-throwing, so ordering can't matter. But the AUDIT-08 finding was HIGH, and per the workplan's own Option D discipline (regression-lock test pinning the working-code invariant for HIGH+ findings), that lock should survive. With it gone, a future narrowing of the `catch` (e.g., to swallow only `ENOENT`) or a refactor that moves resolution back below the mutation steps reintroduces partial-apply with **no test guarding it** — the exact failure AUDIT-08 was filed to prevent. A focused fix: keep a test that asserts resolution runs before any mutation independent of throw-vs-skip — e.g., an assertion that a lane-resolution failure never leaves the artifact half-moved (old path present XOR new path present is never violated), so the ordering contract stays pinned even though the runtime no longer throws.

## 2026-06-04 — audit-barrage lift (20260604T211913837Z-deskwork-plugin)
...
The commit under audit labels AUDIT-20260604-17 "FIXED," but the fix only handles the *missing-file* case (the new `existsSync(laneConfigPath(...))` guard). AUDIT-17's headline concern was a different shape: a lane that genuinely publishes a website and carries a **valid** `redirectsPath` silently loses its 301 append whenever an *orthogonal* field — `pipelineTemplate` — fails to resolve. That path is still live. `loadLaneConfig` cross-validates the pipeline template (loader.ts:190-198) *after* reading the valid lane, and throws if the template is unresolvable (operator mid-edit of pipelines, renamed preset id, deleted template). For such a lane: `existsSync` is `true` → enter the `try` → `loadLaneConfig` throws on the pipeline cross-validation → `catch` swallows → `redirectsPath = undefined` → the redirect-append action is silently skipped. The redirect value was on disk and valid in isolation; it is discarded only because an unrelated field didn't resolve. For the website use-case this is a broken-link/SEO regression discovered only downstream — exactly what AUDIT-17 filed.

The disposition addendum's own prose admits the narrower fix AUDIT-17 recommended ("resolve `redirectsPath` via a reader that validates only the lane schema … without cross-validating the pipeline template") was *not* implemented; the existsSync guard sidesteps a different case. Marking the finding `FIXED` overstates what landed. A correct fix reads the lane's `redirectsPath` through a path that does not gate on pipeline-template resolution (e.g. `readAndValidate`-equivalent schema-only read, or a dedicated `tryReadRedirectsPath` that catches only the missing-file/schema shapes), so a valid redirect on a pipeline-broken lane still appends.
