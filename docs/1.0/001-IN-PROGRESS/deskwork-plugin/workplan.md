...
- [ ] **#218** — legacy `legacy-calendar-to-sidecars` migration rule (`MIGRATING.md` promises it; not shipped). **DECIDED 2026-05-29 (operator): defer to `feature/graphical-entries` (#301)** — it migrates the legacy per-site calendar into the entry-centric model, the exact surface the lanes work is reframing; building it now risks targeting a shape lanes will change. **Adopter-facing caveat to land on this branch:** add a `MIGRATING.md` note that the `doctor --fix=all` auto-migration of legacy `calendar.md` → sidecars is not yet available, so mid-upgrade adopters aren't sent at a silently no-op'ing command. (Doc-only; no migration code here.)
- [ ] **#218 caveat — `MIGRATING.md` note (doc-only, lands on THIS branch).** Add a note that the legacy `calendar.md` → per-entry-sidecars auto-migration (`deskwork doctor --fix=all`, promised by `MIGRATING.md`) is **not yet shipped**, so mid-upgrade adopters (v0.10→v0.11+) aren't pointed at a silently no-op'ing command while the migration rule itself is deferred to #301. Distinct task from the #218 deferral above — the rule defers, but the adopter-facing honesty note ships here.
