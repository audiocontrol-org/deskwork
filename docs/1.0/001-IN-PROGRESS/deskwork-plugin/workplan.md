...
- [ ] **39a — Lane schema + entry resolution.** Lane gains optional `host` and `scaffoldDefaults: Partial<Record<artifactKind, string>>` (**partial** — per spec + `AUDIT-20260602-04`: a lane defines defaults only for the kinds it uses; the Zod schema MUST accept a lane defining a single kind and reject only unknown keys — NOT force every `artifactKind`); remove `lane.contentDir`. Entry `artifactPath` becomes authoritative + required (post-migration) for artifact-bearing entries. TDD: lane-schema tests for the new optional fields + `.strict()` rejection + a partial-map (one kind only) accept case; entry-resolution test that `artifactPath` is the sole resolution key.
