...
- [ ] **#218** — legacy `legacy-calendar-to-sidecars` migration rule (`MIGRATING.md` promises it; not shipped). **Coupled to the calendar-surface cluster** (it migrates the legacy per-site calendar into sidecars), which is deferred to graphical-entries lanes (below). Disposition pending operator call on whether the lanes work subsumes the one-shot migration or it ships standalone here.
