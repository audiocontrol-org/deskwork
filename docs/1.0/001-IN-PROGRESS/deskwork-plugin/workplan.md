...
- [ ] **#62** (ingest no-frontmatter handling) — **DECIDED 2026-05-29 (operator).** (P1, state) When a file has no `state:` frontmatter and no `--state` flag, **refuse** — skip with a `state ambiguous` message telling the operator to pass `--state` or add frontmatter. No silent default-to-Ideas (that default IS the bug; aligns with the global no-fallback / throw-descriptively rule). (P2, `--apply`) On apply, **write a `deskwork: { id: <uuid> }` block** into the file (namespaced per the agent-discipline metadata rule) so the entry is bound from the start — no unbound entries. **Implementable, LOW-overlap per 38·0.** Verify overlap with #63 (ingest writes `deskwork.id` at apply) before coding — P2 may already be partly landed.
