...
- [ ] **39d — Doctor resolution reads `entry.artifactPath` only.** `file-presence` / `frontmatter-sidecar` / `missing-artifact-path` resolve via the stored path; no base search. Confirm `#394`-class multi-site false-positives cannot recur (regression fixture). Remove the dropped #394 search code.
