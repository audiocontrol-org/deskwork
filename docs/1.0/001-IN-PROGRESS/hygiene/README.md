...
| 11 | Stale worktree discovery + dismantle | [#356](https://github.com/audiocontrol-org/deskwork/issues/356) | Tasks 1–5 shipped on branch. Task 5 adds worktree-verbs round-trip to `scripts/smoke-hygiene.sh` (worktree-report --json + dismantle-worktrees propose + apply with all-skip dispositions); full `npm test --workspaces` 1953/1953 pass. Task 6 (dogfood) + Task 3 Step 3 (complete-gate suggestion) + Task 4 Step 3 (burndown sheet) remain as polish. |
