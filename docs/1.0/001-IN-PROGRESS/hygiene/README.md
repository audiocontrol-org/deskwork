...
| 16 | close-shipped apply — pre-flight `pending-verification` label | [#411](https://github.com/audiocontrol-org/deskwork/issues/411) | Scoped (2026-06-04) from v0.35.0..v0.36.0 dogfood. `apply` posts the comment BEFORE the label-add, so missing label leaves half-applied state. Fix: pre-flight + auto-create the label before the per-item loop. |
| 17 | close-shipped SKILL.md — replace bare `/tmp/<name>` paths | [#412](https://github.com/audiocontrol-org/deskwork/issues/412) | Scoped (2026-06-04) from v0.35.0..v0.36.0 dogfood. SKILL.md prescribes bare `/tmp/` paths violating `.claude/rules/file-handling.md`; safety classifier flagged a sub-agent for following the SKILL.md verbatim. Fix: switch to project-local `.dw-lifecycle/close-shipped/runs/<timestamp>/`. |
