...
| 11 | Stale worktree discovery + dismantle | [#356](https://github.com/audiocontrol-org/deskwork/issues/356) | Tasks 1–4 shipped on branch (worktree-report + dismantle-worktrees verbs + session-end-hygiene integration + agent-discipline rule extension + plugin README update). 70 worktree-related tests pass. Tasks 5 (smoke extension) + 6 (dogfood) + Task 3 Step 3 (complete-gate suggestion) remain. Live smoke against `/Users/orion/work/` surfaces 4 actual stale worktree candidates; 0 false-positive orphans after the `.git`-file-shape signal landed. |
| 12 | session-end-hygiene "issues filed this session" scoping fix | [#361](https://github.com/audiocontrol-org/deskwork/issues/361) | Task 1 shipped on branch (Medium fix: gh search dropped; issues derive from `#NNN` refs in `git log <sha>..HEAD`; TBD-scanner filtered to session-diff lines; helpers extracted to `session-range.ts`). 1952/1952 tests pass. Acceptance "re-run against this hygiene session" awaits a release the operator installs. Sibling infra friction (TF-003 + TF-004) tracked at [#362](https://github.com/audiocontrol-org/deskwork/issues/362). |
