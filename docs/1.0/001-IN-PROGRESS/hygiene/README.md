...
| 13 | close-shipped commit-log walker fix-keyword filter | [#366](https://github.com/audiocontrol-org/deskwork/issues/366) | Task 1 shipped on branch. `commit-scanner.ts` drops `plain` / `refs` / `parens` patterns — only `Closes` / `Fixes` / `Resolves` survive (GitHub's own auto-close grammar). `^Merge pull request #N from ` subjects unconditionally drop the whole commit's contribution. 5 new acceptance vitest cases + SKILL.md prose update + 4 subcommand-test fixtures updated to fix-keyword shapes. 2336/2336 tests pass. Verify-in-installed-release acceptance criterion awaits next ship. |
