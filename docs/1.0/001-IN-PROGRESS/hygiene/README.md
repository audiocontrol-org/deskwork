...
| 15 | close-shipped redesign — narrow mechanically, judge with Agent tool | [#374](https://github.com/audiocontrol-org/deskwork/issues/374) | Scoped (2026-05-30) from the v0.28.1 install verification + brainstorm. Design spec + 11-task implementation plan on branch (commits `52baa75` + `ad62b64`). Replaces the 4-walker prose-grammar architecture with mechanical narrowing + Agent-tool dispatch from within the agent's CC session + operator-curated `propose | apply`. Implementation pending. |
