...
| 14 | close-shipped walker accuracy follow-ups (Phase 13 dogfood) | [#369](https://github.com/audiocontrol-org/deskwork/issues/369) | Tasks 1+2 shipped on branch (configurable end-of-subject parens + audit-log Tracks-Issue field + splitter heading-level fix). Live verification against v0.27.0..v0.28.1 + reviewer pass clean. Both v0.28.1 install verification + post-implementation review confirmed the patching trajectory was unsustainable; superseded by Phase 15 redesign. |
| 15 | close-shipped redesign — narrow mechanically, judge with Agent tool | [#374](https://github.com/audiocontrol-org/deskwork/issues/374) | Shipped via PR [#375](https://github.com/audiocontrol-org/deskwork/pull/375) (merged 2026-05-30); rode out in v0.29.0+. Mechanical narrowing + Agent-tool dispatch + operator-curated `propose|apply` replace the 4-walker prose-grammar architecture. Phases 0-15 archived to `workplan-archive.md` 2026-06-04. |
| 16 | close-shipped apply — pre-flight `pending-verification` label | [#411](https://github.com/audiocontrol-org/deskwork/issues/411) | Shipped via PR [#414](https://github.com/audiocontrol-org/deskwork/pull/414) (merged 2026-06-04). `preflightLabel` auto-creates the label before the per-item loop and aborts with `InvalidProposalError` if the create itself fails — closes the half-applied-state failure mode. Audit-barrage surfaced + fixed in-PR: AUDIT-20260604-01 (cross-model claude+codex; pre-flight now skipped when all items effective-skip). |
| 17 | close-shipped SKILL.md — replace bare `/tmp/<name>` paths | [#412](https://github.com/audiocontrol-org/deskwork/issues/412) | Shipped via PR [#414](https://github.com/audiocontrol-org/deskwork/pull/414) (merged 2026-06-04). SKILL.md Phase A now uses `.dw-lifecycle/close-shipped/runs/<timestamp>/` per-run paths; smoke mirrors the convention. Audit-barrage surfaced + fixed in-PR: AUDIT-20260604-02 (smoke timestamp now uses portable python3 expression for real ms precision). |
| 18 | structural-check verbs accept + scope to `--feature <slug>` | [#417](https://github.com/audiocontrol-org/deskwork/issues/417) | Shipped on branch (Tasks 1-8 complete; 2727/2727 plugin tests pass; SKILL-chain smoke clean). Shared `resolveFeatureScope` resolver (hybrid scope-manifest+git-diff). All 6 structural verbs (`check-clones`, `check-anti-patterns`, `check-adopters`, `check-module-symmetry`, `check-refactor-preconditions`, `check-disposition-survivor`) accept `--feature <slug>` and narrow their output to feature scope; project-wide default unchanged when flag absent. Live verification awaits the next release (per verify-in-installed-release rule). |
