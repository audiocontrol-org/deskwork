...
| 18 | structural-check verbs accept + scope to `--feature <slug>` | [#417](https://github.com/audiocontrol-org/deskwork/issues/417) | Scoped 2026-06-04 (operator-selected via session-start interview: Path C real scoping + hybrid scope-manifest+git-diff narrowing). 8 tasks: shared `resolveFeatureScope` resolver + 6 per-verb `--feature` argv adds + SKILL-chain regression check. Each verb keeps current project-wide default; `--feature <slug>` is opt-in narrowing. |
