...
| `/dw-lifecycle:review` | Selects scope; independently re-runs the verification gate; runs spec + code-quality review passes; writes findings into feature-local `audit-log.md` with stable IDs and status transitions | `requesting-code-review` (how to request well); `receiving-code-review` (how to integrate findings without performative agreement); `dispatching-parallel-agents` (for spec/code-quality split on substantial changes) | `code-reviewer` (distinct passes for spec compliance and code quality) |
| `/dw-lifecycle:audit` | Synonym of `/dw-lifecycle:review` | Same as review | Same as review |
