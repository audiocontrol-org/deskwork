...
- [x] Step 1.3.3: Anchor resilience verified via `scripts/anchor-resilience.mjs`: pins three regions, then programmatically mutates the iframe DOM (id rename, sibling insertion before, class rename, total teardown of id+text). Resolver chain works as documented — id rename triggers TextQuote fallback; sibling/class shifts leave id-based CssSelectors intact; total teardown triggers FragmentSelector pixel-offset graceful degradation. All 11 anchor-resilience assertions pass.
