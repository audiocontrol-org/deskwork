...
- [x] Step 3: implement the fix
- [x] Step 4: confirm test passes
- [x] Step 5: commit with `Closes AUDIT-20260530-53 (cross-model: AUDIT-BARRAGE-codex-P5-3)` in subject

**Acceptance Criteria:**

- [x] Failing test exists at `packages/studio/test/dashboard-swimlane-storage-dedup.test.ts` (cited in Step 1)
- [x] `npx vitest run <test-file-path>` exits 0 (passes against the fix)
- [x] Audit-log Status flipped to `fixed-57bd93d` via the close-shipped-audit-findings step
...
- [x] Step 3: implement the fix
- [x] Step 4: confirm test passes
- [x] Step 5: commit with `Closes AUDIT-20260530-55 (cross-model: AUDIT-BARRAGE-claude-P6-1)` in subject

**Acceptance Criteria:**

- [x] Failing test exists at `packages/core/test/pipelines/delete-empty-reassign.test.ts` (cited in Step 1) — paired with two new CLI cases in `packages/cli/test/pipeline/delete.test.ts`
- [x] `npx vitest run <test-file-path>` exits 0 (passes against the fix)
- [x] Audit-log Status flipped to `fixed-<sha>` via the close-shipped-audit-findings step (flipped to `fixed-b034cb9`)
...
- [x] Step 3: implement the fix
- [x] Step 4: confirm test passes
- [x] Step 5: commit with `Closes AUDIT-20260530-56 (cross-model: AUDIT-BARRAGE-claude-P6-1)` in subject

**Acceptance Criteria:**

- [x] Failing test exists at `packages/core/test/pipelines/rename-migration-atomic.test.ts` (cited in Step 1)
- [x] `npx vitest run <test-file-path>` exits 0 (passes against the fix)
- [x] Audit-log Status flipped to `fixed-<sha>` via the close-shipped-audit-findings step (flipped to `fixed-cb78c6b`)
...
- [x] Step 3: implement the fix
- [x] Step 4: confirm test passes
- [x] Step 5: commit with `Closes AUDIT-20260530-57 (cross-model: AUDIT-BARRAGE-claude-P6-1)` in subject

**Acceptance Criteria:**

- [x] Failing test exists at `packages/core/test/lanes/list-graceful-degradation.test.ts` and `packages/core/test/pipelines/list-graceful-degradation.test.ts` (cited in Step 1)
- [x] `npx vitest run <test-file-path>` exits 0 (passes against the fix)
- [x] Audit-log Status flipped to `fixed-5c8ec5c` via the close-shipped-audit-findings step
...
- [x] Step 3: implement the fix
- [x] Step 4: confirm test passes
- [x] Step 5: commit with `Closes AUDIT-20260530-58 (cross-model: AUDIT-BARRAGE-claude-P6-1)` in subject

**Acceptance Criteria:**

- [x] Failing test exists at `packages/core/test/lanes/move-pre-migration.test.ts` (cited in Step 1)
- [x] `npx vitest run <test-file-path>` exits 0 (passes against the fix)
- [x] Audit-log Status flipped to `fixed-138164f` via the close-shipped-audit-findings step
...
- [x] Step 3: implement the fix
- [x] Step 4: confirm test passes
- [x] Step 5: commit with `Closes AUDIT-20260530-59 (cross-model: AUDIT-BARRAGE-claude-P6-1)` in subject

**Acceptance Criteria:**

- [x] Failing test exists at `packages/cli/test/lane/move.test.ts` (in-process rollback test under the `deskwork lane move — sidecar-write failure rollback` describe; falsified by commenting out the rollback `moveDir`/`moveFile` calls in `packages/core/src/lanes/operations/move.ts`)
- [x] `npm --workspace @deskwork/cli test -- --run test/lane/move.test.ts` exits 0 (11/11 passes against the fix)
- [x] Audit-log Status flipped to `fixed-c4f0f5c` via the close-shipped-audit-findings step
...
- [x] Step 3: implement the fix
- [x] Step 4: confirm test passes
- [x] Step 5: commit with `Closes AUDIT-20260530-62 (cross-model: AUDIT-BARRAGE-codex-P6-1)` in subject

**Acceptance Criteria:**

- [x] Failing test exists at `packages/core/test/pipelines/remove-stage-legacy-entry.test.ts` (cited in Step 1)
- [x] `npx vitest run <test-file-path>` exits 0 (passes against the fix)
- [x] Audit-log Status flipped to `fixed-0d76ec5` via the close-shipped-audit-findings step
...
- [x] Step 3: implement the fix
- [x] Step 4: confirm test passes
- [x] Step 5: commit with `Closes AUDIT-20260530-63 (cross-model: AUDIT-BARRAGE-codex-P6-1)` in subject

**Acceptance Criteria:**

- [x] Failing test exists at `packages/core/test/pipelines/delete-atomic-reassign.test.ts` (cited in Step 1)
- [x] `npx vitest run <test-file-path>` exits 0 (passes against the fix)
- [x] Audit-log Status flipped to `fixed-2c928d3` via the close-shipped-audit-findings step
...
- [x] Step 3: implement the fix
- [x] Step 4: confirm test passes
- [x] Step 5: commit with `Closes AUDIT-20260530-64 (cross-model: AUDIT-BARRAGE-codex-P6-1)` in subject

**Acceptance Criteria:**

- [x] Failing test exists at `packages/core/test/lanes/move-path-traversal.test.ts` (cited in Step 1)
- [x] `npx vitest run <test-file-path>` exits 0 (passes against the fix)
- [x] Audit-log Status flipped to `fixed-<sha>` via the close-shipped-audit-findings step
...
- [x] Step 3: implement the fix
- [x] Step 4: confirm test passes
- [x] Step 5: commit with `Closes AUDIT-20260530-66 (cross-model: AUDIT-BARRAGE-claude-P6-2)` in subject

**Acceptance Criteria:**

- [x] Failing test exists at `packages/studio/test/lanes/data-graceful.test.ts` (cited in Step 1)
- [x] `npx vitest run <test-file-path>` exits 0 (passes against the fix)
- [x] Audit-log Status flipped to `fixed-039e734` via the close-shipped-audit-findings step
...
- [x] Step 3: implement the fix
- [x] Step 4: confirm test passes
- [x] Step 5: commit with `Closes AUDIT-20260530-67 (cross-model: AUDIT-BARRAGE-claude-P6-2)` in subject — `b44f042`

**Acceptance Criteria:**

- [x] Failing test exists at `packages/studio/test/pipelines/pipelines-data-unreadable-lane.test.ts` (cited in Step 1)
- [x] `npx vitest run <test-file-path>` exits 0 (passes against the fix)
- [x] Audit-log Status flipped to `fixed-b44f042` via the close-shipped-audit-findings step
...
- [x] Step 1: write failing test exercising the bug (anchor at the file:line cited in the finding's Surface) — `packages/studio/test/lanes/lanes-page-render-project-key.test.ts`
- [x] Step 2: confirm test fails against current code (verify the bug repros)
- [x] Step 3: implement the fix
- [x] Step 4: confirm test passes
- [x] Step 5: commit with `Closes AUDIT-20260530-68 (cross-model: AUDIT-BARRAGE-claude-P6-2)` in subject — sha ba190d6

**Acceptance Criteria:**

- [x] Failing test exists at `packages/studio/test/lanes/lanes-page-render-project-key.test.ts` (cited in Step 1)
- [x] `npx vitest run <test-file-path>` exits 0 (passes against the fix)
- [x] Audit-log Status flipped to `fixed-ba190d6` via the close-shipped-audit-findings step
...
- [x] Step 3: implement the fix
- [x] Step 4: confirm test passes
- [x] Step 5: commit with `Closes AUDIT-20260530-73 (cross-model: AUDIT-BARRAGE-codex-P6-2)` in subject

**Acceptance Criteria:**

- [x] Failing test exists at `packages/studio/test/lanes/copy-disabled-when-incomplete.test.ts` + `packages/studio/test/pipelines/copy-disabled-when-incomplete.test.ts` (cited in Step 1)
- [x] `npx vitest run <test-file-path>` exits 0 (passes against the fix)
- [x] Audit-log Status flipped to `fixed-115a5383f774` via the close-shipped-audit-findings step
...
- [x] Step 3: implement the fix
- [x] Step 4: confirm test passes
- [x] Step 5: commit with `Closes AUDIT-20260530-74 (cross-model: AUDIT-BARRAGE-codex-P6-2)` in subject

**Acceptance Criteria:**

- [x] Failing test exists at `packages/studio/test/pipelines/pipelines-page-client-validation.test.ts` (Set-locked sub-form preview placeholder case)
- [x] `npm --workspace @deskwork/studio test -- --run pipelines/pipelines-page-client-validation` exits 0 (12/12 passing post-fix)
- [x] Audit-log Status flipped to `fixed-5ceee19` via the close-shipped-audit-findings step
...
- [x] Step 3: implement the fix
- [x] Step 4: confirm test passes
- [x] Step 5: commit with `Closes AUDIT-20260530-76 (cross-model: AUDIT-BARRAGE-codex-P6-2)` in subject

**Acceptance Criteria:**

- [x] Failing test exists at `packages/studio/test/chrome/folio-aria-current.test.ts` (cited in Step 1)
- [x] `npx vitest run <test-file-path>` exits 0 (passes against the fix)
- [x] Audit-log Status flipped to `fixed-056528c` via the close-shipped-audit-findings step
...
- [x] Step 3: implement the fix
- [x] Step 4: confirm test passes
- [x] Step 5: commit with `Closes AUDIT-20260530-78 (cross-model: AUDIT-BARRAGE-claude-P6-3)` in subject — `d39551c`

**Acceptance Criteria:**

- [x] Failing test exists at `packages/core/test/doctor/lane-delete-corrupt-sidecar-guard.test.ts` (cited in Step 1)
- [x] `npx vitest run <test-file-path>` exits 0 (passes against the fix)
- [x] Audit-log Status flipped to `fixed-d39551c` via the close-shipped-audit-findings step
...
- [x] Step 1: write failing test exercising the bug (anchor at the file:line cited in the finding's Surface) — `packages/core/test/doctor/lane-repair-rollback-on-journal-fail.test.ts`
- [x] Step 2: confirm test fails against current code (verify the bug repros) — pre-fix set-template left `pipelineTemplate=editorial` on disk; delete left `existsSync(laneFile)=false`
- [x] Step 3: implement the fix — snapshot+rollback pattern via `snapshotLaneFile` / `restoreLaneFile` helpers; mirrors AUDIT-20260530-13 (`bootstrapDefaultLaneIfMissing`)
- [x] Step 4: confirm test passes — full @deskwork/core suite 875 (was 873) all green
- [x] Step 5: commit with `Closes AUDIT-20260530-79 (cross-model: AUDIT-BARRAGE-claude-P6-3)` in subject — commit `7de9a07`

**Acceptance Criteria:**

- [x] Failing test exists at `packages/core/test/doctor/lane-repair-rollback-on-journal-fail.test.ts` (cited in Step 1)
- [x] `npx vitest run <test-file-path>` exits 0 (passes against the fix)
- [x] Audit-log Status flipped to `fixed-7de9a07` via the close-shipped-audit-findings step
...
- [x] Step 8.2.1: Studio's marginalia sidebar renders threads expandable; collapsed thread shows root comment + reply count badge. Landed in commit `cb5ce372`. The grouping helper (`plugins/deskwork-studio/public/src/entry-review/threads.ts`) groups comments into single-level threads (root + replies sorted by `createdAt` ascending). The renderer (`plugins/deskwork-studio/public/src/entry-review/thread-render.ts`) builds one sidebar `<li>` per thread: the root card via the existing `buildSidebarItem` + a reply-count badge inside the root's `.er-marginalia-actions` row + a collapsed `.er-marginalia-thread-replies` `<ul>`. Click the badge to toggle visibility + `aria-pressed`. Orphan replies (replyTo doesn't resolve to a known root in this batch) render with `data-orphan-reply="true"` + an `.er-thread-orphan-label` chip — operator sees the broken-thread state explicitly rather than a silently-dropped comment. Per-reply Resolve / Edit / Delete chrome reuses `buildSidebarItem` so the existing affordances (including Step 8.6.1's addressed-badge inline diff expansion) keep working on reply cards. Test coverage in `packages/studio/test/entry-review/threads-grouping.test.ts` (11 unit tests, edge cases enumerated) + `packages/studio/test/entry-review/thread-render.test.ts` (7 jsdom tests asserting badge placement, default-collapsed, click toggle, orphan markers, reply-chrome continuity, addressed-badge-on-reply).
- [x] Step 8.2.2: Per Phase 9's design pick, threads are either inline-on-pin or sidebar-grouped (operator picks the direction in Phase 9; this task translates). Phase 9 hasn't run yet; per the autonomous-mode bias ("make the reasonable call and keep going") this task picked **sidebar-grouped** because it's simpler than inline-on-pin (no new on-document pin DOM, no scroll-positioning logic, no conflict with the existing per-comment highlight markup) and reuses the existing marginalia sidebar's single-column layout. The choice is documented in `plugins/deskwork-studio/public/src/entry-review/threads.ts`'s file header so a future operator can override in Phase 9 by extending `thread-render.ts`'s render path (the grouping helper is reusable either way).
- [x] Step 8.2.3: Comment-thread permalinks per PRD § Implied scope captured: `/dev/editorial-review/entry/<uuid>#comment/<comment-id>` scrolls to the thread. Landed in commit `93198a02`. The `maybeApplyHashPermalink` private function in `annotations.ts` parses the hash with `/^#comment/(.+)$/`, decodes the id, looks it up in `sidebarIndex`, walks to the enclosing thread root via `findThreadRootByCommentId`, expands the root's replies via `expandThreadForRoot`, then scrolls + highlights the targeted card. Called once at the end of `loadAnnotations` (initial load) AND from a `hashchange` listener (subsequent permalink clicks within an already-loaded session). Permalinks to reply cards expand the enclosing thread first so the operator lands on a visible card. Test coverage in `packages/studio/test/entry-review/thread-permalink.test.ts` (7 jsdom tests covering the lookup + expand primitives + end-to-end sequence; the hash-parsing path inside the controller is exercised via the two primitives it composes).

### Task 8.3: Screenshot capture mechanism

- [x] Step 8.3.1: Per Phase 1's decision, implement screenshot capture (`html-to-image`-based DOM-to-canvas — the 2025/2026 consensus successor to `html2canvas` per the Phase 1 decision brief). Landed in commit `1112bf12` — `plugins/deskwork-studio/public/src/entry-review/screenshot-capture.ts` exposes `captureElementToPng(element, options)` returning `{ blob, suggestedFilename }`. The suggested filename follows the PRD shape: `<commentId>-<ISO-timestamp>.png` when a valid UUID `commentId` is supplied (entry-anchored case), `<ISO-timestamp>-<hash>.png` otherwise (orphan case). Filesystem-safe timestamp replaces `:` and `.` with `-`; hash is the first 8 hex chars of a SHA-256 over the captured bytes via `crypto.subtle` (works identically in browsers and node 20+). Malformed `commentId` values (non-UUID) fall back to the orphan form so a bad client can't smuggle path-traversal characters into the filename. `toBlob()` returning null throws — per project rules, no fallback. `html-to-image ^1.11.13` added to `@deskwork/studio` dependencies; resolves up through the hoisted root `node_modules`. Test coverage in `packages/studio/test/entry-review/screenshot-capture.test.ts` (11 jsdom tests with `html-to-image` mocked at the module boundary via `vi.hoisted` + `vi.mock`).
- [x] Step 8.3.2: Selection-rectangle UI for region capture; full-frame capture is the alternative path. Landed in commit `21b5de14` — `plugins/deskwork-studio/public/src/entry-review/screenshot-region.ts` exposes three primitives: (a) `selectRegion(options)` mounts a `position: fixed; inset: 0; role=dialog; z-index 9000` overlay, listens for `mousedown → mousemove → mouseup`, resolves with `{ pageX, pageY, width, height }` on a successful drag (cancels on Escape, on click-and-no-drag below the minimum-size threshold, or on explicit teardown); the overlay is removed from the DOM BEFORE the Promise resolves so the caller can immediately fire the capture without the overlay appearing in the screenshot; (b) `computeCropGeometry(input)` translates a page-coordinate region into source-canvas pixel coordinates with the capture-time `devicePixelRatio` applied — pure math, no DOM, no canvas; regions extending past the source element are silently clamped to the intersection, regions entirely outside resolve to zero dimensions; (c) `cropCanvasToRegion(source, input)` drives `drawImage` to produce a fresh canvas of just the cropped pixels (no resampling) — throws on zero-pixel crops. Affordance placement per `.claude/rules/affordance-placement.md`: the trigger button that opens the overlay lives ON the entry-preview component, not in a generic toolbar; button wire-up lives downstream in Task 8.4 / Phase 10. Test coverage in `packages/studio/test/entry-review/screenshot-region.test.ts` (13 jsdom tests covering successful drags in all four directions, Escape cancel, sub-threshold drags, overlay teardown before resolution, role + z-index assertions, custom minPx threshold; geometry tests covering page→source-canvas pixel translation with `devicePixelRatio`, clamping past the source bounds, fully-outside region, no-scroll origin case; crop-canvas tests using a `drawImage` spy because jsdom does not ship a 2d-context without the optional `canvas` npm package).
- [x] Step 8.3.3: Captured bytes saved to `<entryDir>/scrapbook/screenshots/<comment-id>-<timestamp>.png` (entry-anchored) OR `<projectRoot>/.deskwork/screenshots-orphan/<timestamp>-<hash>.png` (capture-then-attach flow). Landed in commit `a461206d`. Server-side: `packages/studio/src/lib/screenshot-persistence.ts` exposes `persistEntryScreenshot` / `persistOrphanScreenshot` (+ `assertSafeScreenshotFilename`, `entryScreenshotsDir`, `orphanScreenshotsDir`); filename validation is a tight regex with belt-and-braces rejection of slashes, parent-dir hops, leading dots, non-png extensions, empty strings, and >200-char names; both writers `mkdir -p` the target dir, refuse to overwrite (throws → 409), and use the atomic tmp+rename pattern. Two new Hono routes in `packages/studio/src/routes/api.ts`: `POST /api/dev/editorial-review/entry/:entryId/screenshot` (400 on malformed entryId / missing file / bad filename; 404 on unknown entry; 409 on collision; 200 with `{ writtenPath, relativeWrittenPath }`) and `POST /api/dev/editorial-review/screenshots/orphan` (same shape minus 404). Both routes share parsing + error-mapping plumbing in `packages/studio/src/routes/screenshot-upload-helper.ts` (`extractScreenshotUploadFile`, `mapScreenshotErrorToResponse`); the new routes triggered the pre-commit clone-detection gate, which prompted a small `api.ts` refactor extracting `readValidEntryId` / `readValidCommentId` / `readValidEntryAndCommentIds` / `lookupEntrySidecar` private helpers — five existing call sites consolidate against the sidecar lookup helper. Client-side: `plugins/deskwork-studio/public/src/entry-review/screenshot-persist.ts` exposes `postEntryScreenshot` / `postOrphanScreenshot` with hand-rolled response-shape parsing (no `as` casts; every field read with a typeof guard). Test coverage: `screenshot-persistence.test.ts` (15 tests), `screenshot-upload-helper.test.ts` (7 tests), `entry-screenshot-route.test.ts` (10 integration tests against real tmp project trees), `screenshot-persist-client.test.ts` (5 jsdom tests). Full suite at this commit: 1194 passing, 11 skipped. Task 8.4 (attach-to-comment workflow + cross-entry attachment + clipboard / drag-drop) is out of scope for this task; the binding from a persisted screenshot path into the comment's `attachments[]` field + composition with the `spatialAnchor` schema field added in Step 8.1.1 lives there.
...
- [x] Step 8.5.1: Update `/deskwork:iterate` skill prose (`plugins/deskwork/skills/iterate/SKILL.md`) to require a free-text `reason` for every `addressed` disposition. The dispositions file format becomes: `{ "<commentId>": { "disposition": "addressed", "reason": "addressed by adding § X at line N" } }`. Landed in commit `bea8539e` — SKILL.md's dispositions-file-format block shows reason verbatim on every shape, a constraint paragraph states the per-disposition rule (addressed required, deferred + wontfix optional), the legacy read path (Step 8.5.3) is named, and the CLI-parse-time refusal (Step 8.5.2) is forward-pointed so the agent knows the gate fires before any journal-write.
- [x] Step 8.5.2: The `deskwork iterate` CLI refuses dispositions files missing the `reason` field for `addressed` entries (clear error with example). Landed in commit `602e2b64` — two CLI-parse-time gates pair with the Step 8.1.2 write-side schema (commit `91954561`): (a) `loadDispositionsFile` in `packages/cli/src/commands/iterate-dispositions.ts` refuses any `addressed` entry that lacks a non-empty `reason` with exit 2 (usage error, per the OutOfRangePositionError / Task 0.66 precedent) and the error names the offending `commentId` + shows the expected shape inline; (b) `runLongformIterate` in `packages/cli/src/commands/iterate.ts` refuses `--auto-dispositions=addressed` unconditionally because the flag has no per-comment reason input. Both gates fire BEFORE `iterateEntry` runs, so a malformed file or wrong flag never reaches the journal-write boundary. New test `packages/cli/test/iterate/disposition-reason-required.test.ts` pins the four scenarios from the task spec plus an ordering test (parse-time gate runs BEFORE the orphan-skip). Existing `iterate-entry-centric-dispositions.test.ts` updated: the orphan test uses `deferred` so the parse gate doesn't fire and the orphan-skip path is exercised; the `--auto-dispositions=addressed` test pins the new parse-time refusal shape; the empty-comments test exercises `--auto-dispositions=deferred`.
- [x] Step 8.5.3: Existing `addressed` dispositions in the journal without `reason` (legacy) render with "no reason recorded" in the studio — backward compatible read; only new writes are gated. Landed in commit `2afffa1c` — `plugins/deskwork-studio/public/src/entry-review/sidebar-render.ts` (`buildAddressStamp`) now surfaces a "no reason recorded" inline marker when an `addressed` annotation lacks a non-empty `reason`. The legacy-marker span carries a `data-legacy-missing-reason="true"` attribute so diagnostic / triage scripts can grep for it without matching the placeholder text exactly. New data renders without the attribute. Scoping: only `addressed` annotations get the legacy marker — `deferred` and `wontfix` have always had optional `reason`. Test `packages/studio/test/entry-review/addressed-badge-legacy.test.ts` covers six scenarios (missing reason → marker, empty string → marker, substantive reason → verbatim, deferred / wontfix without reason → no marker, and CSS-class continuity between the legacy marker span and the substantive-reason span).

### Task 8.6: Per-comment inline diff expansion ("addressed" badge → diff)

- [x] Step 8.6.1: Studio: clicking a comment's "addressed" badge expands inline to show two things — the disposition's `reason` text AND the slice of the prior-vs-new-revision diff that intersects the comment's anchor region. Landed in commit `b7aa3a63` — `buildAddressStamp` in `plugins/deskwork-studio/public/src/entry-review/sidebar-render.ts` accepts an optional `fetchDiffSlice` callback; when present AND the disposition is `addressed`, the stamp becomes click-interactive (`role="button"`, `aria-pressed`, `tabindex`, `cursor:pointer`) and click / Enter / Space toggles an inline `.er-marginalia-diff-expansion` placed directly after the stamp. `attachDiffToggle` debounces in-flight clicks via `dataset.fetching` and re-uses the rendered expansion on second-click toggle-off. The controller wires the fetcher against the new `/diff-slice` route with full per-field type-guarded response parsing (no `as` casts). Per `.claude/rules/affordance-placement.md`, the affordance is component-attached.
- [x] Step 8.6.2: Diff-slicing logic: compute the diff between revision N-1 and revision N (server-side, via diff library); for each comment with `disposition: addressed` on revision N, intersect the diff hunks with the comment's `range` (markdown) or `spatialAnchor` region (graphical) and return that subset. Landed in commit `39be53ab` — `packages/core/src/entry/diff-slice.ts` exports `computeDiffSlice(projectRoot, entryId, commentId, revision)`; uses `structuredPatch` from the `diff` npm package (added to `@deskwork/core` deps) with `context: 0` for tight hunk boundaries. Markdown comments: character offsets converted to line numbers against revision N's full file, intersected with each hunk's `newStart..newStart+newLines-1` range via the exported `intersectHunksWithLineRange`. Spatial-anchor comments: returns empty hunks + a `notes` value naming the gap (Phase 10/11 surface). New route `GET /api/dev/editorial-review/entry/:entryId/diff-slice?commentId=<id>&revision=<n>` serializes the result.
- [x] Step 8.6.3: Render the slice as a side-by-side mini-diff inside the expanded comment, with the disposition reason as a header line. Landed in commit `b7aa3a63` — `renderDiffExpansion` (in `sidebar-render.ts`) builds: `.er-marginalia-diff-reason` (header — verbatim disposition reason, or "no reason recorded" for legacy data with the `data-legacy-missing-reason` attribute), followed by `.er-marginalia-diff-body` containing one `<pre class="er-marginalia-diff-hunk">` per hunk. Each hunk's lines render as `<span class="er-marginalia-diff-line" data-kind="add|del|ctx">` so the CSS layer can paint the side-by-side view independently. Lines include their unified-diff `+/-/' '` prefix verbatim to match the operator's mental model.
- [x] Step 8.6.4: When the diff slice is empty (the comment was on a region that didn't change in the new revision), surface "addressed without local diff — see the disposition reason" so the operator knows to read the reason text. Landed in commit `dd00ec75` — `renderDiffExpansion` detects the `hunks.length === 0 && notes === undefined` case and renders an inline `.er-marginalia-diff-empty` element with the exact text from the task spec. The element carries `data-empty-slice="true"` so diagnostic / triage scripts can find the case without text-matching. The reason header stays ABOVE the fallback in DOM order so "see the disposition reason" points at something the operator has already read. The `notes` branch (server-side explanation — first iteration, spatial-anchor surface, etc.) takes precedence when set; the empty fallback fires only when the server didn't provide a more specific explanation.

### Task 8.7: Cross-cutting markdown review benefit

- [x] Step 8.7.1: Verify the existing markdown review surface picks up threads + attachments + inline diff expansion for free (the schema change is additive; the render layer reads the same data). Pre-flight audit of `plugins/deskwork-studio/public/src/entry-review/sidebar-render.ts` + `thread-render.ts` + `state.ts` confirms render-side coverage by Phase 8 field: `replyTo` rendered via Task 8.2's `buildSidebarThread` (reply-count badge + collapsed `<ul>`); `addressed reason` rendered via Step 8.5.3's `buildAddressStamp` (`.er-marginalia-stamp-reason` span with legacy-marker back-compat); inline diff expansion rendered via Step 8.6.1's `attachDiffToggle` + Step 8.6.3's `renderDiffExpansion` + Step 8.6.4's empty-slice fallback. `attachments` and `spatialAnchor` reach the parsed `CommentAnnotation` object via the additive schema but have no render-side yet — Task 8.3/8.4 (attachments) + Phase 10/11 (spatialAnchor) close those gaps.
- [x] Step 8.7.2: Smoke test: leave a threaded comment + screenshot attachment on an existing markdown entry; confirm the existing review surface renders both correctly. Landed in commit `221f30e3` — new integration test `packages/studio/test/entry-review/markdown-benefits-phase-8.test.ts` (5 scenarios) covers the cross-cutting flow on a single markdown entry: (a) root + 2 replies + reply-count badge (collapsed → expanded toggle) + addressed badge with non-empty reason + inline diff expansion with del/add line `data-kind` attribution; (b) `attachments` field survives into the parsed `CommentAnnotation` after grouping (Task 8.3/8.4 render-side gap documented in test); (c) `spatialAnchor` discriminated-union shape survives into the parsed object (Phase 10/11 graphical-render gap documented in test); (d) per-reply addressed badge renders with its own reason + empty-slice fallback (Step 8.6.4) on a reply card; (e) legacy addressed annotation (no reason) renders the `data-legacy-missing-reason="true"` marker on a threaded markdown entry. All 5 scenarios green; full entry-review suite stays at 53/53 (no regression).
