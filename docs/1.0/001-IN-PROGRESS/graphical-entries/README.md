...
| 6 | Lane + pipeline CRUD skills + studio management surfaces | [#307](https://github.com/audiocontrol-org/deskwork/issues/307) | **In progress** — Tasks 6.1 (`/deskwork:lane` skill family — SKILL.md + CLI + 45 tests + path-traversal hardening + atomic write + move-rollback), 6.2 (`/deskwork:pipeline` skill family — SKILL.md + CLI + 64 tests + rename-migration sidecar + customize wrapper + 3 BLOCKING bugs caught at quality-review), 6.3 (studio lane-management page — server-render + clipboard-builder client + 30 tests + 7 quality polish), 6.4 (studio pipeline-editor page — server-render + Phase-2 follow-up error rows + 70 tests + 7 quality polish + 6 audit followups) all shipped on `feature/graphical-entries`. Tests 706 → 893 studio + 320 cli + 711 core. Builds exit 0. Tasks 6.5 (doctor rule: orphan-pipeline-reference) and 6.6 (integration test) still pending. |
