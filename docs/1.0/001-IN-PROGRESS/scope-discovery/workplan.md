...
## Phase 12: Close-shipped-issue lifecycle — closure as structural gate

**Parent issue:** (TBD — file via `/dw-lifecycle:issues` after this phase merges into the workplan)
**Source:** Operator design conversation 2026-05-28 — the "how can we institutionalize closure" thread following the accumulated-debt audit. Triggered by three converging observations: (1) ~100 open GH issues across the deskwork repo of which 17 are stale phase parent metadata + 3 are confirmed-shipped-but-not-closed (#284, #289, #292) + a long tail of multi-release deferrals; (2) the canary's TF doc grew from 3 entries to 10 across one week of dogfood with the close-rate trailing the open-rate; (3) STUB-tracked algorithmic deferrals (#318 clustering algorithm; `pattern-handlers/semantic.ts` LLM dispatcher) sit in production code with TODO + tracking-issue link but no recurring closure pressure.

**Problem the phase addresses (captured exhaustively per capture-mode rule; scoping is a separate pass):**

The scope-discovery lifecycle has a structural asymmetry: **shipping is rewarded; closure isn't.** Every cycle, substantive work lands + careful follow-ups get filed; the *next* cycle has its own work to ship, and the previous cycle's follow-ups rot. The audit-log already carries `fixed-<sha>` / `verified-<date>` markers; `tooling-feedback-import` already promotes closure-marked TF entries into the audit-log; the project rule "Issue closure requires verification in a formally-installed release" already names the closure contract. **What's missing is the inverse promotion path:** `audit-log fixed-<sha> + <sha> reachable in tagged release + corresponding GH issue still open → surface for operator-confirmed closure.`

The fix-shape is composition over existing primitives, not new infrastructure:

- The closure-evidence sources already exist (audit-log, TF entries, commit subjects, workplan checkboxes with embedded issue links).
- The shipment gate already exists (`/release` knows the tag SHA + the commits it carries).
- The session-end gate already exists (`/dw-lifecycle:session-end` walks the just-touched features).
- The doctor verb already exists with rule infrastructure for `--fix` flows.
- The orchestrator-agent already has the `gh issue close --comment "<provenance>"` invocation pattern from prior PRs.

**Scope expansion notice (per agent-discipline § "Capture mode vs scope mode"):** the phase intentionally captures every dimension the design conversation surfaced — closure of shipped issues, doctor-rule visibility, release-notes auto-gen, phase parent closure gate, cross-reference lint, one-time catch-up sweep. Operator scoping decides which sub-tasks ship in v0.25.0 vs deferred; agent does not pre-cut.

### Task 1: `/dw-lifecycle:close-shipped` skill (the inverse promotion path)

NEW skill at `plugins/dw-lifecycle/skills/close-shipped/SKILL.md` + library at `plugins/dw-lifecycle/src/scope-discovery/close-shipped/`. Mirrors `tooling-feedback-import`'s pattern but in reverse direction (audit-log → GH issue closure).

- [ ] **T1.1**: Library `close-shipped-evidence.ts` — walks four evidence sources per feature: (a) audit-log entries with `Status: fixed-<sha>` where `git tag --contains <sha>` is non-empty; (b) per-feature TF doc entries with `Status: Closed | <closing-commit>`; (c) commit subjects on `main` matching `(closes|fixes|resolves) #\d+` within tagged releases; (d) workplan items `[x]` with embedded `· [#NNN](url)`. Each source produces a `ClosureCandidate { issueNumber, evidenceSource, commitSha, releaseTag?, auditEntryId?, tfEntryId?, draftedComment }`.

- [ ] **T1.2**: Library `close-shipped-cross-reference.ts` — merges candidates by issue number; for each issue surfaces the full provenance trail (audit-log entry + TF entry + commit + release tag). If multiple sources contradict (e.g. audit says `fixed-aa6d680`, commit says `closes #289` but commit is from a different SHA), the cross-reference surfaces the discrepancy and refuses to draft an auto-closeable status (operator-only path).

- [ ] **T1.3**: CLI verb `dw-lifecycle close-shipped` at `plugins/dw-lifecycle/src/subcommands/close-shipped.ts` + register in `src/cli.ts`. Flags: `--feature <slug>` (default: all in-progress features); `--release-tag <tag>` (default: latest tag); `--dry-run` (surface only, don't call `gh issue close`); `--yes` (skip operator confirmation; for automation); `--json` (emit ClosureCandidate[] to stdout for machine consumption).

- [ ] **T1.4**: SKILL.md at `plugins/dw-lifecycle/skills/close-shipped/SKILL.md` documenting the operator gate flow: the skill prints the closeable list + drafted closure comments per issue; operator types `y`/`n` per issue OR `all`/`none` to bless the batch. On confirmation, the skill invokes `gh issue close <num> --comment "<provenance>"` per accepted candidate. Stdout reports per-issue outcome; exit 0 on success; 1 on `gh` failures.

- [ ] **T1.5**: Tests at `plugins/dw-lifecycle/src/__tests__/scope-discovery/close-shipped.test.ts`. Fixture covers: happy path (one closeable issue with all four evidence sources agreeing); orphan-audit case (audit-entry references issue not on GitHub); orphan-issue case (GH issue says `closes <sha>` but no audit-entry); contradiction case (multiple sources disagree on which SHA closes); --dry-run skips the gh call; --json emits valid JSON. Mock `gh` via the subprocess test pattern that scope-discovery's existing tests use.

### Task 2: Integration into `/release` (the high-leverage shipment moment)

The `/release` skill's Pause 5 (final push confirmation) is the natural extension point — the just-shipped commits are now reachable in a tagged release, and that's the moment closeability flips from "pending" to "yes."

- [ ] **T2.1**: Modify `.claude/skills/release/SKILL.md` Pause 5 — after `atomic-push` reports success, append a prompt: `"Release v<version> pushed. Run /dw-lifecycle:close-shipped --release-tag v<version> to surface issues this release closes? [Y/n]"`. On `Y`, the agent invokes the skill against the just-pushed tag.

- [ ] **T2.2**: Auto-generate release-notes body from the closeable list. Currently the `release` workflow creates an empty release; this task wires the workflow OR the release skill itself to call `dw-lifecycle close-shipped --release-tag v<version> --json` and inject the result into `gh release edit v<version> --notes "<body>"`. The body lists closed issues + a one-line summary per. Adopters running `gh release view v<version>` see the closure trail.

- [ ] **T2.3**: Acceptance — `/release` end-to-end against a synthetic shipping scenario: tag a release with one commit that contains `closes #NNN`; the release prompt surfaces #NNN as closeable; on confirm, `gh issue close #NNN` fires; release-notes body lists #NNN.

### Task 3: Integration into `/dw-lifecycle:session-end`

Session-end is the natural per-feature closure gate (one feature's worth of work just landed; check if any of its TF / audit entries now close).

- [ ] **T3.1**: Modify `plugins/dw-lifecycle/skills/session-end/SKILL.md` — after the existing journal-append + commit step, invoke `dw-lifecycle close-shipped --feature <slug>` against the just-worked feature. Operator confirms or declines per issue.

- [ ] **T3.2**: Cross-feature pass — if the session touched multiple features (multi-worktree session), session-end invokes close-shipped per touched feature in sequence.

- [ ] **T3.3**: Acceptance — synthetic session-end against scope-discovery (which has shipped-but-open #284, #289, #292) surfaces those three as closeable; operator confirms; issues close.

### Task 4: NEW doctor rule `shipped-issue-still-open`

The doctor verb already has rule infrastructure for `--fix` flows. Add a rule that makes shipped-but-not-closed issues visible on every doctor run, not just after release.

- [ ] **T4.1**: NEW rule at `plugins/dw-lifecycle/src/scope-discovery/doctor-rules/shipped-issue-still-open.ts`. Walks `audit-log.md` files under `docs/<v>/001-IN-PROGRESS/<slug>/`; for each `Status: fixed-<sha>` line, tests whether `<sha>` is in `git tag --contains` AND queries `gh issue view <ref> --json state`. Open issues with shipped fixes surface as `severity: medium, surface: <audit-log-entry>, file: <issue-url>`. Suggested fix: `dw-lifecycle close-shipped --feature <slug>`.

- [ ] **T4.2**: `--fix` mode invokes close-shipped per affected feature.

- [ ] **T4.3**: Tests at `plugins/dw-lifecycle/src/__tests__/scope-discovery/doctor-rules/shipped-issue-still-open.test.ts`. Stub gh + git tag responses; verify the rule fires + the fix runs the right close-shipped invocation.

### Task 5: NEW doctor rule `audit-log-issue-bidirectional` (cross-reference linter)

The closure trail is provable from either end (audit-log or GH issue). The lint forces the bidirectional invariant.

- [ ] **T5.1**: NEW rule at `plugins/dw-lifecycle/src/scope-discovery/doctor-rules/audit-log-issue-bidirectional.ts`. For every audit-log entry that references a GH issue number AND every GH issue tagged as scope-discovery, assert: (a) audit-entry exists in the canonical audit-log; (b) GH issue references the same commit SHA or audit-entry ID. Mismatches surface as findings (orphan-audit vs orphan-issue).

- [ ] **T5.2**: Tests at `plugins/dw-lifecycle/src/__tests__/scope-discovery/doctor-rules/audit-log-issue-bidirectional.test.ts`. Cover: bidirectional happy path, orphan-audit, orphan-issue, mismatched SHA. Verify the finding shape includes the missing-half hint.

### Task 6: Phase parent issue closure gate in `/dw-lifecycle:complete`

The 17 stale phase parent issues (e.g. #273 scope-discovery parent, #301 graphical-entries parent, the per-Phase issues #274–#283) have no closure gate. Codify one.

- [ ] **T6.1**: Modify `plugins/dw-lifecycle/skills/complete/SKILL.md` — after the existing doc-move + ROADMAP-update steps, walk the feature's GH issues (parent + per-phase). For each parent issue, test whether (a) all child issues are closed OR (b) the feature reached feature-complete per this skill's invocation. If either holds, close the parent issue with a closure comment citing the feature-complete commit + the closed children.

- [ ] **T6.2**: Library `phase-parent-closure.ts` factoring the closure-eligibility check from `complete.ts`.

- [ ] **T6.3**: Tests against a synthetic parent issue + per-phase children fixture.

### Task 7: Acceptance test + dogfood against accumulated debt

The retroactive validation: run close-shipped against the existing audit-log entries with `fixed-<sha>` across all in-progress features. Verify the surfaced list matches the operator's known shippable set.

- [ ] **T7.1**: Dry-run `dw-lifecycle close-shipped --dry-run --json` against scope-discovery. Expected output: at minimum #284 (batch-dispose), #289 (check-disposition-survivor), #292 (scope-widen) — all confirmed-shipped-but-open per the design conversation. May include audit-log AUDIT-20260525-02..07 closures if those reference open issues.

- [ ] **T7.2**: Operator-confirmed pass: invoke the skill non-dry-run; close the issues the operator blesses. Update the audit-log to mark each as `verified-2026-MM-DD` (the closure date) per the existing closure ritual.

- [ ] **T7.3**: One-time catch-up sweep — run close-shipped against every in-progress feature in the repo (`docs/*/001-IN-PROGRESS/*/`). Surface every shippable-and-open issue across all features. Operator triages.

### Task 8: Cross-references + documentation

- [ ] **T8.1**: Update `.claude/rules/agent-discipline.md` § "Memory-vs-rule placement" with a new section: *"Closure is a structural step, not aspirational."* The rule cites `/dw-lifecycle:close-shipped` + the post-release + session-end gates. Documents that the agent posts evidence and the operator decides per-issue closure — the existing verification rule's "agent doesn't close issues" clause stays load-bearing.

- [ ] **T8.2**: Update `plugins/dw-lifecycle/README.md` with a new section on the closure-of-shipped workflow + the four evidence sources + the surfaces that auto-invoke it.

- [ ] **T8.3**: Workplan + README's Phase 12 row flipped to `[x]` when acceptance passes (T7.2 closes the bootstrap set of confirmed-shippable issues).

**Acceptance Criteria:**

- [ ] `dw-lifecycle close-shipped` runs against scope-discovery, surfaces #284 + #289 + #292 (the canonical confirmed-shippable trio), assembles a provenance trail per issue citing the audit-log entry + commit SHA + release tag.
- [ ] `/release` end-of-flow prompts to close the closeable issues post-push; operator confirmation gate works as designed.
- [ ] `/dw-lifecycle:session-end` invokes close-shipped per just-touched feature.
- [ ] Doctor rule `shipped-issue-still-open` fires + the `--fix` flow closes confirmed-shippable issues.
- [ ] Doctor rule `audit-log-issue-bidirectional` flags the orphan cases.
- [ ] `/dw-lifecycle:complete` closes the phase parent when child issues all close.
- [ ] One-time catch-up sweep closes ~70 stale issues across the repo's accumulated backlog (number TBD pending T7.3 dry-run).
- [ ] Agent-discipline rule + README docs updated; the closure-as-structural-gate pattern is documented for future sessions.

**Open scoping questions (operator decides):**

1. Should `close-shipped` auto-close issues when ALL FOUR evidence sources unanimously agree (no operator confirmation), reserving the prompt for cases with discrepancies? Stricter alternative: the rule is "agent posts evidence; operator decides" — never auto-close. The strictness vs ergonomics trade-off is open.

2. Should the doctor rule run in `--fix` mode auto-invoke close-shipped for confirmed-shippable issues, or always require operator confirmation? Same auto-vs-manual axis as #1.

3. Should phase parent closure (T6) fire when ANY child closes (incremental) or only when ALL close (terminal)? Incremental risks premature closure if a phase parent is still load-bearing for in-flight follow-ups; terminal risks the same accumulation-problem at a smaller scale.

4. Should the cross-reference lint (T5) include audit-log-to-non-GH-issue references (e.g. internal tracker IDs from the audiocontrol pilot's pre-import era)? Or only the GitHub surface?

5. Should release-notes auto-gen (T2.2) include closed issues only, or also include `verified-<date>` audit-log entries that don't have a corresponding GH issue (legacy items)?

**Out of Scope (for this phase):**

- Closing issues outside the scope-discovery / dw-lifecycle domain (e.g. studio + doctor + ingest bugs). The infrastructure built here generalizes to any feature with an audit-log, but the catch-up sweep (T7.3) scopes only to in-progress feature directories — other open issues stay manually triaged.
- Auto-categorizing the existing 100 open issues (e.g. tagging by feature, by category). That's a separate hygiene cycle that this phase doesn't depend on.
- Closing the algorithmic STUB deferrals (#318 clustering, `semantic.ts` LLM dispatcher). Those are different — algorithm-implementation work, not closure ritual. A separate Phase 13 (or new feature) would address them.

**Existing primitives this phase composes:**

- `audit-log.md` per-feature with `fixed-<sha>` / `verified-<date>` markers.
- `tooling-feedback.md` per-feature with `Status: Closed | <closing-commit>` line.
- `tooling-feedback-import` skill — the forward direction (TF → audit-log).
- `/release` skill — the shipment gate.
- `/dw-lifecycle:doctor` — the existing audit verb.
- `/dw-lifecycle:complete` — the feature-complete gate.
- `gh issue close --comment` — the closure invocation.
- Workplan-issues back-fill (v0.24.1) — the `· [#NNN](url)` embedding evidence source.
