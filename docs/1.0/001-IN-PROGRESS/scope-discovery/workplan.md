...
- [ ] Orchestrator auto-dispositions candidates at high confidence; escalates at low confidence. **OPEN — operator decision needed.** The dampener-based slush vs promote disposition exists (Phase 16 `check-barrage-dampener`), but the 4-factor composite confidence calibration named below (line 1132) is not yet implemented; today's auto-disposition uses the dampener's 2-rule policy (N-quiet + single-clean-run) rather than `judge-confidence × policy-match × skills-exhaustion × auditor-correction-rate`. Resolving the calibration question (ship as dampener-only vs build the 4-factor composite) would close this acceptance criterion.
...
- [ ] Pattern-type vocabulary supports at minimum the 4 v1 operator-named patterns from #315 (Tailwind/utility-class catch-all, hardcoded-color, hover-only-affordance, negative-space-no-canonical-consumer). **OPEN — verification needed.** Per-pattern registration check not yet performed against the live `pattern-matrix-patterns.yaml`; the polymorphic dispatcher landed in Task 1 supports the four pattern *classes* (regex / negative-space / coverage / outlier / semantic) but specific operator-named v1 catalog entries should be enumerated to confirm.
...
- [x] [#350](https://github.com/audiocontrol-org/deskwork/issues/350) — **`validate-return` refactor-cue false-positives.** Tracked via GH issue [#350](https://github.com/audiocontrol-org/deskwork/issues/350) (open). Workplan checkbox closed because the disposition is "issue-tracked, downstream patch cycle"; the workplan no longer carries duplicate tracking when the GH issue is the source of truth. Per project rule "issue tracking flows through GitHub, not workplan checkboxes."
- [x] [#351](https://github.com/audiocontrol-org/deskwork/issues/351) — **`session-start`/`session-end` helper-subcommand availability check.** Tracked via GH issue [#351](https://github.com/audiocontrol-org/deskwork/issues/351) (open). Workplan checkbox closed; disposition same shape as #350.
- [x] [#352](https://github.com/audiocontrol-org/deskwork/issues/352) — **Pre-commit gate chain skipped on docs-only commits.** Tracked via GH issue [#352](https://github.com/audiocontrol-org/deskwork/issues/352) (open). Note: Phase 24's no-git-hook-enforcement retirement may render #352 partially moot — the bookkeeping-only-commit detector now lives in `implement-hook` Step 6b (`check-barrage-tip` short-circuits the barrage on bookkeeping-only diffs), which closes the docs-only-friction half. The structural-chain gates still fire on docs-only commits per the agent-discipline `check-clones --gate-mode` etc.; that's intentional (catches docs-side regression of structural invariants). Workplan checkbox closed; the GH issue can be re-scoped against the current implementation.
- [x] **#318 validation milestone — cross-feature, owned by graphical-entries.** Tracked at graphical-entries' Phase 7 acceptance criteria; this entry is the scope-discovery side of the cross-reference. Workplan checkbox closed because the milestone lives on the OTHER feature's workplan; this row is a navigational pointer, not a deliverable for scope-discovery.
