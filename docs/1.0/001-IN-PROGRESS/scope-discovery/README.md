...
| 12 | Close-shipped-issue lifecycle — closure as structural gate (extension proposed 2026-05-28) | Not started — captured in workplan; PRD iteration pending. Triggered by accumulated-debt audit (100 open issues; 3 confirmed shipped-but-open: [#284](https://github.com/audiocontrol-org/deskwork/issues/284), [#289](https://github.com/audiocontrol-org/deskwork/issues/289), [#292](https://github.com/audiocontrol-org/deskwork/issues/292)). 8 tasks captured: NEW `/dw-lifecycle:close-shipped` skill + library; `/release` post-push closure prompt + auto-generated release notes; `/dw-lifecycle:session-end` per-feature closure gate; NEW doctor rules `shipped-issue-still-open` + `audit-log-issue-bidirectional`; phase-parent closure gate in `/dw-lifecycle:complete`; acceptance + dogfood (catch-up sweep of accumulated backlog); cross-references + docs. Closes the structural asymmetry between shipping and closing; mirrors `tooling-feedback-import` as its inverse promotion path. |
