...
| 6 | CLI subcommands (~20 new verbs) | In progress — 15 of ~20 verbs landed (`check-clones` (with `detect-clones` back-compat alias), `check-anti-patterns`, `check-adopters`, `check-refactor-preconditions`, `scope-inventory`, `check-editor-symmetry`, `batch-dispose`, `check-disposition-survivor`, `scope-summary`, `check-deprecations` (full scanner port; closes [#287](https://github.com/audiocontrol-org/deskwork/issues/287)), `validate-scope-discovery`, `scope-export`, `refresh-clones-baseline`, `dispose-clone`, `scope-widen` (closes [#292](https://github.com/audiocontrol-org/deskwork/issues/292))); `--gate-mode` flag landed on all four check-* subcommands; Task 4 install/migrate/uninstall verbs pending. |
| 7 | Slash command skill prose (~18 new + 5 updated) | In progress (18 of 18 skills authored; 4 install-related landed in Phase 8; `scope-widen` skill landed alongside the verb implementation per [#292](https://github.com/audiocontrol-org/deskwork/issues/292)). |
...
| 11 | Pattern discovery loop with self-correcting controller ([#316](https://github.com/audiocontrol-org/deskwork/issues/316)) | Captured (not yet started). First dogfood-cycle finding ([#315](https://github.com/audiocontrol-org/deskwork/issues/315)) surfaced that the agents act as inventory, not discovery. Phase 11 captures the general fix: pattern-type widening (negative-space + coverage + outlier + clustering + semantic), status-marked catalog entries with provenance, orchestrator-mediated dispositions, self-correcting controller (measures codebase-state + auditor-correction-rate), `/dw-lifecycle:implement` augmented with per-turn audit/judge stack, multi-source LLM ensemble (internal judge + external auditor + audit-log memory), wrong-decision recovery primitives. Captured exhaustively per capture-mode rule; scoping pass decides what ships in v1.1. |
