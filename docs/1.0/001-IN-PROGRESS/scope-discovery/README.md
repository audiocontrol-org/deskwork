...
| 11 | Pattern discovery loop with self-correcting controller ([#316](https://github.com/audiocontrol-org/deskwork/issues/316)) | In progress — Tasks 1 + 2 + 14 landed. Task 1: polymorphic pattern-handler dispatcher (regex / negative-space / coverage / outlier / semantic) with KeygroupSummary repro detection from [#315](https://github.com/audiocontrol-org/deskwork/issues/315); G5 + G6 stubs at [#318](https://github.com/audiocontrol-org/deskwork/issues/318) / [#319](https://github.com/audiocontrol-org/deskwork/issues/319). Task 2: Loop foundation — `status:` + `provenance:` on every registry-driven catalog entry; 6-status discriminator; reversibility primitive; new doctor rule `catalog-entry-missing-status`; 30 new vitest scenarios. Task 14: tooling-feedback closure → audit-log import workflow — `/dw-lifecycle:tooling-feedback-import` skill + verb; new doctor rule `tooling-feedback-stale` (14-day default threshold, configurable via `.dw-lifecycle/scope-discovery/config.yaml`); idempotent `imported-as: AUDIT-<id>` watermark on promoted TF entries; cross-referenced AUDIT entries preserve the literal closure status; 30 new vitest scenarios. Tasks 3–13 remain. |
