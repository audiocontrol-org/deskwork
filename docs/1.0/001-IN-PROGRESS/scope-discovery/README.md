...
| 11 | Pattern discovery loop with self-correcting controller ([#316](https://github.com/audiocontrol-org/deskwork/issues/316)) | In progress — Tasks 1 + 2 landed. Task 1: polymorphic pattern-handler dispatcher (regex / negative-space / coverage / outlier / semantic) with KeygroupSummary repro detection from [#315](https://github.com/audiocontrol-org/deskwork/issues/315); G5 + G6 stubs at [#318](https://github.com/audiocontrol-org/deskwork/issues/318) / [#319](https://github.com/audiocontrol-org/deskwork/issues/319). Task 2: Loop foundation — `status:` + `provenance:` on every registry-driven catalog entry (anti-patterns / adopter-manifests / clones / pattern-matrix / deprecations); 6-status discriminator (pending / blessed / cursed / ignore / tracked-holdout / withdrawn); reversibility primitive (`withdrawn` requires `provenance.context: audit-finding-<id>`); scanners filter to actively-enforced (`blessed` + `cursed`); new doctor rule `catalog-entry-missing-status`; 30 new vitest scenarios; 861/861 tests pass. Tasks 3–14 remain. |
