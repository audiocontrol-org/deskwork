...
| 7 | Slash command skill prose (~18 new + 5 updated) | In progress (19 of 19 new skills authored — adds `migrate-from-pilot` for [#291](https://github.com/audiocontrol-org/deskwork/issues/291); 4 install-related landed in Phase 8; `scope-widen` skill landed alongside the verb implementation per [#292](https://github.com/audiocontrol-org/deskwork/issues/292); Task 2 updated-skill prose now 5/5 — `/dw-lifecycle:implement` documents auto-scope-widen + dispatch-wrapper engagement + `--no-scope-widen`). |
...
| 11 | Pattern discovery loop with self-correcting controller ([#316](https://github.com/audiocontrol-org/deskwork/issues/316)) | In progress — Tasks 1 + 2 + 11 + 14 landed. Task 1: polymorphic pattern-handler dispatcher (regex / negative-space / coverage / outlier / semantic) with KeygroupSummary repro detection from [#315](https://github.com/audiocontrol-org/deskwork/issues/315); G5 + G6 stubs at [#318](https://github.com/audiocontrol-org/deskwork/issues/318) / [#319](https://github.com/audiocontrol-org/deskwork/issues/319). Task 2: Loop foundation — `status:` + `provenance:` on every registry-driven catalog entry; 6-status discriminator; reversibility primitive; new doctor rule `catalog-entry-missing-status`; 30 new vitest scenarios. Task 11: cross-surface uniformity — editor-symmetry-matrix filters non-active adopter-manifest rows; regime-holdout-detector stamps `status_provenance` on every finding + `actively_enforced_count` / `candidate_count` rollup; manifest schema gains `status_provenance:` (required) on every regime-holdout entry; `mergeDelta` re-derives `by_status` from merged sections; new test `cross-surface-loop.test.ts` (11 scenarios) asserts uniform filter behavior across anti-patterns / adopter-manifests / editor-symmetry / pattern-matrix / clones / regime-holdout-detector. Task 14: tooling-feedback closure → audit-log import workflow — `/dw-lifecycle:tooling-feedback-import` skill + verb; new doctor rule `tooling-feedback-stale` (14-day default threshold, configurable via `.dw-lifecycle/scope-discovery/config.yaml`); idempotent `imported-as: AUDIT-<id>` watermark on promoted TF entries; cross-referenced AUDIT entries preserve the literal closure status; 30 new vitest scenarios. Tasks 3–10, 12, 13 remain. |
