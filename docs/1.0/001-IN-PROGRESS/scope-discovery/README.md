...
| 13 | Audit-finding lifecycle — anti-deferral discipline + workplan promotion ([#355](https://github.com/audiocontrol-org/deskwork/issues/355); extension 2026-05-29) | Not started — captured in workplan; PRD approved (pending /deskwork:approve). NEW `/dw-lifecycle:promote-findings` skill walks audit-log for `Status: open` entries; default disposition is "scope into workplan as TDD-first fix task"; agent CANNOT pick deferral. `/dw-lifecycle:implement` refuses to advance when ≥1 open finding; NO escape hatch (operator chose rigidity). Mechanical TDD doctor rule + commit-msg gate enforce passing test for `Closes AUDIT-<id>` commits. Closure-side automation flips `open` → `fixed-<sha>` → `verified-<date>`. 6 tasks captured. |
