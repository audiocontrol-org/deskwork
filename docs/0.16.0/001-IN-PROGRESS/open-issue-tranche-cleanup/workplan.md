...
**T5 partial — mobile-first review surface rebuild fix-landed 2026-05-08 (commits `2c1a15a` → `d2fed62`, 12 commits).** Pending v0.17+ marketplace-walk verification on a real iPhone. The rebuild covers the cross-cutting "review surface is desktop-shaped on phone" concerns directly; addresses #240 + #241 + adjacent bugs as a single architectural pass instead of N independent fixes.

- [x] **Strip compaction on phone** (`b9edb0a`) — desktop f7c3266 compaction mirrored to <30rem; 6 decision buttons single-row; phone strip 157→78px (-50%).
- [x] **Strip sticky restored** (`b862c32`) — `body { overflow-x: hidden }` was breaking sticky positioning of descendants on iOS; replaced by the existing `clip` rule. Strip now pins below folio while reading.
- [x] **Strip row-1 collapse on scroll** (`089e255`) — at <30rem, scrolling collapses row-1 (back-link + stage stamp) so only the 6-button row stays pinned. CSS transition + reduced-motion respect.
- [x] **Dual-viewport verification rule + smoke** (`2c1a15a`) — `.claude/rules/ui-verification.md` adds a "Dual-viewport verification" subsection requiring before/after measurements at BOTH ≥1280 and ≤390 viewport classes. `scripts/smoke-er-viewport-regressions.mjs` is the regression check (Chromium, 4 entries × 2 viewports, asserts no html-overflow + strip-height invariant + no fixed-position offenders).
- [x] **Mobile mockups** (`3e33ceb`) — three opinionated direction mockups under `/static/mockups/` (bottom-sheet / FAB / inline-reveal). Operator picked Mockup 1.
- [x] **Mockup 1 implementation — bottom tab bar + sheets** (`759bbff`) — 64px persistent bottom bar with 3 tabs (Outline / Notes / Actions); sheets slide up from bottom with drag-handle dismissal; Outline + Notes content sourced from existing renderers; Actions sheet renders 4 decision buttons with per-action color accents that copy `/deskwork:<verb> <slug>` to clipboard. All desktop side-rails (marginalia tab, outline tab, outline-drawer, marginalia element, strip-right) hide at <48rem. Top strip drops to back-link + stage stamp only (43px).
- [x] **Bidirectional mark↔note linking** (in `759bbff`) — tap a `<mark>` in article → open Notes sheet, scroll to matching note, brief flash; tap a note in sheet → close sheet, scroll article to mark, flash.
- [x] **Composer wired into mobile Notes sheet** (`0c07aa5`) — composer was trapped inside hidden `.er-marginalia`; relocated into Notes sheet at boot on phone (gated on matchMedia). Add-new-note flow works.
- [x] **Edit/Resolve/Delete on existing notes** (`792e37d`) — root cause was `cloneNode` not copying event listeners. Refactored to MOVE the actual `[data-sidebar-list]` element into the sheet instead of cloning. All existing handlers fire correctly.
- [x] **iOS auto-zoom slosh fix** (`d2fed62` + `a44feab`) — added `maximum-scale=1` to viewport meta (universal fix); also swept all review-surface form inputs to `font-size: 16px` (defense-in-depth).
- [ ] **Verify on real iPhone post-release** — Tailscale URL walks, on-device review of all sheets + composer + edit form + editor.
- [ ] **Mobile editor + scrapbook (2026-05-09 session)** — Mockup 2 editor (commits `cdc184a` + `5baa042`) and Mockup 1 scrapbook (commits `821bc07` + `74e42eb`) shipped behind /frontend-design + dual-viewport probes. Code review (commit `23cb111`) applied 7 findings; one deferred:
    - [ ] [#245](https://github.com/audiocontrol-org/deskwork/issues/245) — mobile scrapbook sheet: cloned items have inert event handlers (lightbox + future per-item actions). Three options surfaced (move-not-clone, rebind via `initScrapbookLightbox(slot)`, or accept native link behavior); operator decision needed on the long-term shape. Recommended minimum: rebind via the existing root-parameter API.
- [ ] **Other T5 issues remaining** — [#173](https://github.com/audiocontrol-org/deskwork/issues/173), [#174](https://github.com/audiocontrol-org/deskwork/issues/174), [#175](https://github.com/audiocontrol-org/deskwork/issues/175), [#176](https://github.com/audiocontrol-org/deskwork/issues/176), [#216](https://github.com/audiocontrol-org/deskwork/issues/216), [#217](https://github.com/audiocontrol-org/deskwork/issues/217). Not addressed by the mobile rebuild; need separate evaluation.
- [ ] **Outline tab text-doesn't-fit on iOS Safari (operator-reported, not yet fixed)** — Chromium @ 390 renders fine; reproducer requires real iOS or Playwright WebKit. Deferred.
- [ ] **#193 induct picker on Final + pipeline stages** — picker is hidden on phone via the strip-compaction work; needs separate decision on whether desktop-only is acceptable or needs sheet-surfacing.
- [ ] **Original cross-page concerns** ([#177](https://github.com/audiocontrol-org/deskwork/issues/177), [#178](https://github.com/audiocontrol-org/deskwork/issues/178), [#179](https://github.com/audiocontrol-org/deskwork/issues/179), [#180](https://github.com/audiocontrol-org/deskwork/issues/180)) — review whether the mobile rebuild addresses any of these.
