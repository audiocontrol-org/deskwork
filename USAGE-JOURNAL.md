...
## 2026-05-07: studio-bridge Phases 1–8 — infrastructure-only session, plugin not exercised

**Arc:** Pure feature build of the studio ↔ Claude Code bridge through all 8 workplan phases in one auto-mode session. Drove subagent-driven-development with two-stage review (spec compliance + code quality) on every phase, fix-up commits between. 15 commits, 1293 tests passing. See [DEVELOPMENT-NOTES.md](./DEVELOPMENT-NOTES.md) entry of the same date for the build-side narrative.

**This session did NOT dogfood the deskwork plugin** — no `/deskwork:add`, no `/deskwork:approve`, no real editorial work. The infra built here (chat panel, MCP listen loop, affordance routing) is *the surface* a future dogfood would exercise; today's session built it.

**Deferred to next session: Phase 8 manual smoke (M1–M10).** This is the real adopter-experience test — the moment the bridge meets a phone, a real editorial collection (writingcontrol.org), and an operator who has to actually finish a thought without a terminal. Until that walkthrough happens, *"Phase 8 done"* means *"the automated smoke passes"*, not *"the bridge survived contact with reality."* The manual checklist captures specifically what to look for: Cmd-Enter on phone keyboards, tool-use card readability on small screens, draft-restore across reconnects, the single-agent-invariant 409 when a second CC tries to claim the bridge.

**insight** — **Build sessions and dogfood sessions are different shapes; they shouldn't be conflated.** The Phase 4 + Phase 5 review-loop caught real correctness bugs (publish-before-persist drift, silent-failure on panel-destroyed-mid-await), but those are *contract* bugs — the kind a careful subagent + reviewer can find statically. The bugs the *manual* M1–M10 smoke is going to find are *embodied* bugs — keyboard-doesn't-fit, scroll-doesn't-stick, two-thumbs-can't-reach-this-button. No amount of static review surfaces those. They surface when the operator picks up a phone, opens `/dev/chat`, and tries to drive a real Approve+Iterate flow on a real entry. That's the gate Phase 8 represents and why the workplan's exploratory framing has it as a separate validation step, not a checkbox bundled with implementation.

**friction** (anticipated, not yet experienced) — **The MCP client config snippet in `plugins/deskwork-studio/README.md` is hedged because we never registered the bridge with a live CC against this branch.** The README points operators at the studio's startup-banner URL (`http://localhost:47321/mcp`) and uses generic-looking `mcpServers` JSON, but Claude Code's exact config shape varies across versions. If the M1 step (boot studio + register MCP server) hits a schema rejection, the README's hedging language ("the typical shape is...") is the get-out — but it's a get-out, not an answer. Phase 8's smoke walkthrough is also where this gets pinned down.
